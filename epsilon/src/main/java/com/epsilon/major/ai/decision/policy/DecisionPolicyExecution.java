...
/**
 * 一回の方策推論で使う実行計画と出力の利用権をまとめ、転送が終わるまで保持する。
 *
 * <p>AFFINE_SUM、候補特徴量の先頭部分 連結、INDEXED_AFFINE、候補種類
 * 集約、4家牌別文脈、二分岐文脈集約は独立した実行構成部分だが、同じ方策順伝播の出力を再利用してよい時点は共通である。 {@link
 * Forward#seal()}が返す依存オブジェクトを各バッチの最終D2H完了まで保持することで、
 * 各実行計画の永続出力を早く再利用しない。開始、確定、依存解放、解放は外部の投入スレッドが直列化する。
 */
public final class DecisionPolicyExecution implements AutoCloseable {
  ...
  public Forward beginForward(NDManager workingManager) {
    ...
    try {
      ...
      try {
        // 構成部分開始中にはデバイス投入がないため、構築済み順伝播を逆順にキャンセルする。
  ...
  /** 実行系を利用不能にした最初の失敗を返す。正常なら{@code null}を返す。 */
  ...
  public static final class Forward {
    ...
    public AutoCloseable seal() {
      ...
      try {
        ...
        // 確定済み側も内部Forwardが利用権を所有している。完了前に閉じず、再利用不可に設定回収へ渡す。
        poison(failure);
        throw failure;
      }
    }
