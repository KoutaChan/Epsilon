 ...
 *
 * <p>生の履歴トークンに加えて、ネットワークが複数構成要素を暗黙に突き合わせなくても済むよう、初巡の継続、立直宣言位置、リーチ後の見逃しによるフリテン、相手別河枚数、立直者自身の河と立直後に全プレイヤーが通した現物を決定論的に導出する。裏ドラ、牌山順、他家の副露に含まれない手牌は公開観測ではないため入力しない。
 */
