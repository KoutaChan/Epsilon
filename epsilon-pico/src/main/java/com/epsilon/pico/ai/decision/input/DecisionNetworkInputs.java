...
/**
 * Decision ネットワークへ渡すテンソルを、用途ごとの名前で保持する。
 *
 * <p>NDList 内の順序、初期化時の形状、連続バッファからのビュー構築を一か所で管理する。モデル、推論サーバー、学習処理が位置番号から入力の意味を解釈し直すことを防ぐ。
 *
 * @param stateCategories 局・プレイヤー・牌・河・面子のカテゴリ値フィールド
 * @param actionCategories 判断対象の行動ごとのカテゴリ値フィールド
 * @param actionRoutes 方策グラフの条件付き経路 ID
 * @param transitionCategories 行動-遷移ごとのカテゴリ値フィールド
 * @param transitionTiles 行動直後の状態の牌種別フィールド
 ...
 * @param actionNumerics 判断対象の行動ごとの数値フィールド
...
public record DecisionNetworkInputs(
  ...
  /**
   * 転送済み2 連続バッファから標準形式の論理ビューを構築する。
   *
   * <p>返すNDArrayは入力連続バッファのスライス/形状変更ビューであり、テンソルごとの追加複製を行わない。
   *
