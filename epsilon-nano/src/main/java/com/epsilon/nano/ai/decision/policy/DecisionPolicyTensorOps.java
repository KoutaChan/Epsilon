...
/**
 * 方策入力から有効な要素を集め、埋め込み参照とマスク付きの遷移集約を行う。
 *
 * <p>学習パラメーターは持たない。保存 ID の0をパディングとし、それ以外は埋め込み表のインデックスに1を足した値として扱う。
 */
final class DecisionPolicyTensorOps {
   ...
   *
   * <p>牌表自体は複製せず、同じバッチの全行動・遷移へバッチ IDだけを繰り返す。返却テンソルはクエリと同じ 管理元/デバイスに置かれ、学習対象ではない。
   */
  ...
  /**
   * 幅1の有効要素のみの遷移行を行動配置へ直接戻す。
   *
   * <p>一つの行動に実在遷移が一つだけなら、条件付きsoftmaxの重みは必ず1になる。したがって実在行を対応位置へ書き込み、
   ...
   *
   * <p>通常経路ではスコアへ遷移マスクを乗算する際にこのデータ型へ昇格する。幅1の直接対応位置へ書き込む経路でも同じ境界を明示し、
   * 後段の候補文脈Fusionが容量区分ごとに一貫したデータ型を受け取るようにする。
   *
   * @param scores 有効要素のみの遷移スコア
   * @param transitionEmbeddings 対応する有効要素のみの遷移表現
  ...
  /**
   * 有効要素のみの遷移を条件付き方策重みで行動単位へ集約する。
   *
   * <p>GPUでは実在行だけを{@code index_add}し、CPUでは同じ値を密な対応位置へ書き込んで集約する。
   */
  ...
  /** 応答元の牌文脈と、手番中KANの主となる-牌文脈を最初の行動へ割り当てる。 */
  ...
  /** 有効要素のみの遷移のバッチ IDと牌IDから、元の34牌表を複製せず対象牌だけを集める。 */
  ...
  /** 有効要素のみの遷移のバッチ IDと牌IDから、元の全プレイヤー牌文脈を複製せず対象牌だけを集める。 */
  ...
  /** マスク外を0にし、候補ロジットの共通ずらす量に不変な遷移重みを返す。 */
  ...
  /** 保存された IDの0をゼロベクトル、1以降を表インデックス + 1としてバッチごとに埋め込みを引く。 */
  ...
  /** 有効要素のみの行ごとの元バッチ IDを使い、共有表から直接保存された IDを引く。 */
