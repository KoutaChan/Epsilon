...
/**
 * 未正規化スコアを、麻雀の条件付き意思決定構造に沿った合法手の確率へ合成する。
 *
 ...
 * 分解する。CALLとKANの総確率は専用の二値分岐だけで決まり、合法なCHI 組み合わせ数やKAN 種類数を増減しても上位分岐の確率は変化しない。
 *
 * <p>最終行動の対数確率は到達経路上の条件付き対数確率の和である。選択行動損失は 選択した行動が通過した節点だけへ勾配を流し、未選択分岐へ反実仮想ラベルを与えない。パディング枠の確率は0である。
 */
public final class EpsilonDecisionPolicyGraph {
  ...
  /**
   * スコア計算処理出力と符号化済みの行動メタデータから最終合法手分布を作る。
   *
   * @param scores 二値の分岐、種類、具体候補、RIICHI/DAMA 分岐の未正規化スコア
   * @param actionCategories {@code [batch, legalActionCapacity, ACTION_INT_STRIDE]} の行動 メタデータ
   * @param actionRoutes 同じ物理打牌の代表枠と RIICHI/DAMA 枠を指す参照先の対応メタデータ
   * @return 各行の合法手候補の位置上で総和1となる合法手分布
   */
