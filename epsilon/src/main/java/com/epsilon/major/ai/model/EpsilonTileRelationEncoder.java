 ...
 * フィードフォワード残差接続で構成し、層の組末尾の層正規化で候補採点器へ渡す牌トークンの尺度を固定する。
 */
public final class EpsilonTileRelationEncoder extends AbstractBlock {

  /** 積み重ねる関係-を考慮した注意機構ブロック数。 */
  public static final int BLOCK_COUNT = 2;

  /** 各ブロックの Attentionヘッド数。 */
  public static final int ATTENTION_HEADS = 4;

  private static final int MAXIMUM_ATTENTION_WIDTH =
      EpsilonDecisionArchitecture.TILE_ATTENTION_WIDTH;
  private static final int MAXIMUM_FEED_FORWARD_WIDTH =
      EpsilonDecisionArchitecture.TILE_FEED_FORWARD_WIDTH;
  ...
  /** 関係集合とエンコーダー構成を要約したチェックポイント互換性識別子。 */
   ...
   * 指定幅の牌関係エンコーダーを構築する。
   *
   * @param hiddenSize 牌トークン幅。Attentionヘッド数で割り切れる正数
   */
   ...
   *
   * <p>ブロック自体は公開せず、パラメーターを保持するNDArray 参照だけを名前付きのレコードへ写す。重みを複製しないため、 返却値の寿命は指定管理元とパラメーター保存先に従う。
   */
   ...
   *
   * <p>通常順伝播の定数ホストからデバイスへの転送を除き、HIP グラフの記録中にもページ固定されていないホストメモリを参照しない。
   *
  ...
  /** Backendが利用可能な融合演算を選び、一つの牌トークンバッファを全ブロックで再利用する。 */
  private NDArray encodeInference(
    ...
    // 通常項目では共有入力を守り、所有権を引き取る項目だけが呼び出し元から譲られたバッファを再利用する。
