...
import com.epsilon.nano.ai.decision.policy.EpsilonDecisionPolicyHead;
 ...
 * EpsilonMahjongStateReadout}だけを分離する。共有エンコーダーは方策パラメーターであり、価値 特徴量の集約へ渡すメモリは常に勾配を切り離す。
 * したがって価値損失は共有表現を一切更新せず、方策分布を暗黙に移動させない。
 *
 * <p>方策または価値だけが必要な処理では専用順伝播により不要な特徴量の集約/出力層を省略する。両方が必要な通常順伝播では、高価な 構成要素文脈化を一度だけ実行する。
 */
public final class EpsilonDecisionNetwork extends AbstractBlock {
   ...
   * 指定隠れ層幅でネットワークを構築する。
   *
   * @param hiddenSize 状態・行動埋め込み幅。注意ヘッド数で割り切れる正数
   */
  ...
  public static String architectureSummary(int hiddenSize) {
    return EpsilonDecisionConstants.ARCHITECTURE_ID
        ...
        + "+value-readout-c64-f256+value-detached"
        ...
        + "+shared-discard+broadcast-aware-linear-fusion+binary-branch-gates"
  ...
  /** ホストで有効な要素だけに集約した逐次実行用の入力から方策と価値を推論する。 */
  ...
  /**
   * ホストで有効な要素だけに集約した入力を、実行枠を管理するな方策推論実行境界とともに推論する。
   *
   * <p>入力テンソルの管理元は長寿命でもよい。順伝播中だけ入力を{@code outputManager}配下の一時有効範囲へ借用し、 返却テンソルだけを{@code
   * outputManager}へ残す。これにより固定した再生でも反復回数一時テンソルが入力管理元へ蓄積しない。
   */
  public EpsilonDecisionOutput forwardInferenceDecision(
      ParameterStore parameterStore,
      DecisionInferenceDeviceBatch batch,
  ...
  /** 価値特徴量の集約を起動せず、ホストで有効な要素だけに集約した逐次実行用の方策入力だけを実行する。 */
   ...
   *
   * <p>価値ネットワークは行動/遷移を参照しないため、このオーバーロードは通常の{@link DecisionDeviceBatch}経路と同じ状態
   ...
   *
   * <p>固定した再生の長寿命入力と、反復回数ごとに破棄する出力・一時テンソルの寿命を分離するための推論境界である。
   *
   * @param parameterStore 順伝播に使うパラメーターの取得先
   * @param inputs 状態テンソルだけを持つ価値専用入力
  ...
  /** ホストで確定したプレイヤーごとの履歴表現インデックスを使い、GPUで存在行を再集計せず価値を推論する。 */
  public NDArray forwardInferenceValue(
      ParameterStore parameterStore,
      DecisionStateInputs inputs,
      NDArray playerMemoryPresentIndices,
   ...
   *
   * <p>方策中間表現の計算部と候補スコア計算処理は実行しない。牌譜などの固定データによる検証で1候補行の方策統計を作らず、価値表現だけを監査するために使う。
   *
   * @param parameterStore 順伝播に使うパラメーターの取得先
   * @param inputs 状態テンソルだけを持つ価値専用入力
   * @param training 学習固有のブロック挙動を有効にするか
  ...
  /** 有効要素のみのプレイヤーごとの履歴表現インデックスを再利用する価値専用診断経路。 */
  public ValueDiagnosticOutput forwardValueWithDiagnostics(
      ParameterStore parameterStore,
      DecisionStateInputs inputs,
      NDArray playerMemoryPresentIndices,
  ...
  /**
   * 両ネットワークの出力に加え、それぞれの状態埋め込みを監査用に返す。
   *
  ...
  /** ホストで有効な要素だけに集約した逐次実行用の入力から方策/価値と診断埋め込みを推論する。 */
  ...
  /** 有効要素のみの推論バッチの論理ビューと疎インデックスを順伝播有効範囲へ一時的に借用する。 */
  ...
  /** 固定 GRP の期待効用をガウス分布の事前予測にし、101 区間の状態残差を加える。 */
  private NDArray forwardValueHeads(
    ...
    // ログだけを下限値し、遠い区間にも有限の残差で確率を移せるようにする。
    NDArray prior =
        priorProbabilities
  ...
  /** 重みを固定したGRP の局境界周辺分布を学習計算グラフから分離する。 */
  ...
  /**
   * パラメーター名を自己対局学習方策モデルオプティマイザーが所有するか判定する。
   *
   * @param name DJL パラメーターの階層名
   * @return 自己対局学習方策モデルの更新対象なら {@code true}
   */
  ...
  /**
   * 方策と価値を同時に更新する牌譜事前学習で更新可能なパラメーターか判定する。
   *
   * @param name DJL パラメーターの階層名
  ...
  /**
   * 牌譜などの固定データによる方策のみ事前学習で更新するパラメーターか判定する。
   *
   * @param name DJL パラメーターの階層名
  ...
  /**
   * 牌譜などの固定データによる価値のみ事前学習で更新するパラメーターか判定する。
   *
   * @param name DJL パラメーターの階層名
  ...
  /**
   * 方策と価値を同時に更新する自己対局学習のオプティマイザーが所有するパラメーターか判定する。
   *
   * @param name DJL パラメーターの階層名
   * @return 方策モデルまたは価値の更新対象なら {@code true}
   */
  ...
  /**
   * 方策/価値のブロックを牌譜などの固定データによる事前学習対象に応じて切り替える。
   *
  ...
  /** 方策と価値を同時に更新する自己対局学習で更新する方策/価値ブロックをすべて学習可能に戻す。 */
