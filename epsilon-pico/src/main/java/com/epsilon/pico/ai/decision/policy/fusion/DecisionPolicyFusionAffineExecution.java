...
import com.epsilon.pico.ai.decision.policy.DecisionPolicyAffineExecution;
...
public final class DecisionPolicyFusionAffineExecution extends DecisionPolicyAffineExecution {
  ...
  private final NDArray alternativeOffsets;
  ...
  public DecisionPolicyFusionAffineExecution(
    ...
    try {
      ...
      this.alternativeOffsets =
          parameterStore.getValue(alternativeOffsets, manager.getDevice(), false);
  ...
  private NDArray submitAlternative(
    ...
    try {
      ...
      // AFFINE_SUMのバイアスは一次元なので、分岐別オフセットは射影の和へ直接加える。
      NDArray activated = EpsilonMahjongStateEncoder.silu(result.add(alternativeOffsets));
  ...
  private AffineProgram createAlternativeProgram(int capacity, DataType[] inputDataTypes) {
    ...
    FusionRecipe.Value[] sources = {state, candidates};
    ...
    FusionRecipe.Output output = builder.addOutput("alternativeHidden", affine.build());
    FusionRecipe recipe = builder.build();
  ...
  private void validateParameterTypes() {
    ...
    requireType(alternativeOffsets, "alternativeOffsets");
  }
