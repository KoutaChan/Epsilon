...
/**
 * 収集中は大きな入力と方策分布を外部ファイルに置き、必要になった時点で読み込む学習サンプル。
 *
 * <p>半荘終了時には教師値と小さな観測情報だけを保持する。学習用ファイルには入力本体を複製せず、保存先と位置、長さを記録する。
 *
 * <p>入力本体は {@link EpsilonDecisionTrajectoryPayloadCodec}
 * の形式で非圧縮のまま保存し、学習時にサンプルを必要とするときだけ読み込む。収集中の再エンコードや二重書き込みを避ける。
 */
final class EpsilonDecisionDeferredSample implements EpsilonDecisionSampleRecord {
  ...
  /** 学習データファイル符号化・復号処理向けの検証済み有効要素のみの合法手ビュー。 */
  ...
  /** 学習データファイル/境界検証向けの共有読み取り専用のビュー。パッケージ外へ公開しない。 */
  ...
  /** 学習データファイル読み取り処理向け。外部データ本体のI/O失敗を隠さず伝播する。 */
