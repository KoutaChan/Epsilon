...
/**
 * {@link DecisionHostBatch} の各行を構築し、入力と学習用の教師値を同じ順序で格納する。
 *
 * <p>推論では実在する遷移だけを保持し、学習では固定長バッファを使う。どちらも同じエンコーダーで入力を生成する。指定した行数をすべて書き込んだ場合だけ {@link #build()}
 * を呼べる。構築後の再利用はできない。
 */
public final class DecisionBatchBuilder {

  /** 並列エンコーダー実行処理が所有し、バッチをまたいで再利用する行を符号化するための作業領域。 */
  ...
  /**
   * 行動適用後の状態と価値を表す、合法手を伴わない1行のバッチを構築する。
   *
   * @param state 符号化する局状態
   * @param player 観測主体の席番号（0-3）
   * @return 行動軸にダミーの1枠を設けた状態のみのバッチ
   */
  ...
  /** 対局の借用観測を一度だけ解析し、系列固有の入力行へ書き込みます。 */
  public void encodeObservedInferenceRow(
      int rowIndex, PublicObservation observation, List<Action> legalActions) {
    try (var session = openEncoding()) {
   ...
   * @param chosenSlot 実際に選択された候補枠
   * @param behaviorPolicy 候補枠順の探索後の確率
