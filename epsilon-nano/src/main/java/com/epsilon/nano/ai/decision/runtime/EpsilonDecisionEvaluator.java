...
public interface EpsilonDecisionEvaluator extends AutoCloseable {
  ...
  /**
   * 一行の合法手集合を実際に評価する個々の推論器を返します。
   *
   * <p>通常の評価器は自身を返す。複数方策を振り分ける振り分け処理は、受付前に安定した推論器の識別情報へ解決することで、 一つの論理的な
   ...
   * 型付きバッチをパイプラインへ投入します。
   *
   * <p>GPU実装はこのメソッドを上書きします。小さなCPU テスト評価器は同期評価を完了済みFutureへ包みます。
   *
   * @param batch ホスト上に構築済みの型付きバッチ
  ...
  /** ホスト符号化用格納枠を待機せずにで取得し、失敗時は正確な解放通知を返します。 */
  ...
  /** この推論器が利用できるデバイスへ、次のバッチを供給する必要があるかを返す。 */
  ...
  /**
   * Streaming 対局が一度に構築する標準論理バッチ行数を返す。
   *
  ...
  /**
   * Streaming 対局が指定型付き容量区分について一度に構築する論理バッチ行数を返す。
   *
   * @param bucket 行動・遷移容量を表す型付き容量区分
   * @return ホスト側で一度に構築する行数
   */
