...
import com.epsilon.major.ai.decision.policy.EpsilonDecisionPolicyGraph;
import com.epsilon.major.ai.decision.training.DecisionOnlineLossConfig;

/**
 * 牌譜による方策の模倣学習、スカラー価値の学習、選択行動の方策勾配学習の損失を計算する。
 *
 * <p>PPO教師信号は実行資格を持つ選択最終行動に対してだけ与える。探索後の方策と探索寄与を混ぜた方策の重なりを
 * 正規化した方策を Actor/Value の評価対象とし、Actor はその方策を探索前方策の相対変化で更新する。エントロピー
 * 追加項は反実仮想ラベルを使わず、現在の合法最終行動分布全体へ勾配を流す。探索前方策間の {@code KL(piRollout || piCurrent)}
 ...
 *
 * <p>Decision 価値は期待効用を Gaussian ヒストグラムへ変換した HL-Gauss 交差エントロピーで学習する。GRP 境界分布の期待
...
public final class EpsilonDecisionLoss {
  ...
  /**
   * 保存データによる方策ネットワーク部分専用のサンプル-重み付き模倣学習損失を計算する。
   *
   * @param output ネットワークが出力した候補・選択肢スコア
   * @param input 合法候補、経路、教師実行枠、サンプル重みを持つデバイス側バッチ
  ...
  /**
   * 完全な入力検査を有効にして自己対局による損失を計算する診断向け引数の異なるメソッド。
   *
   * <p>本番頻繁に実行する処理ではモデルに固定した効用の定義を渡し、検査済み学習データ片を CPU へ戻さない。
   *
   * @param output 方策と価値のネットワーク出力
   * @param input 自己対局による教師値を持つデバイス側バッチ
   * @param config PPO クリップ・探索分の学習への寄与混合率・エントロピー係数の設定
   * @param hasPositiveActorWeight バッチに方策対象サンプルが存在するか
   * @return 逆伝播対象損失と全診断量
   */
  ...
  /** ネットワークの方策スコアを損失と同じFLOAT32境界で最終行動分布へ合成する。 */
  ...
  /** モデル生成時の効用の定義を使い、CPU 転送なしで自己対局による損失を計算する。 */
  public static TrainingLossResult computeOnlineTrainingLoss(
      EpsilonDecisionOutput output,
      DecisionDeviceBatch input,
      DecisionOnlineLossConfig config,
  ...
  public static TrainingLossResult computeOnlineTrainingLoss(
    ...
    // 方策対象外の行は選択確率ゼロを許す。重みを掛ける前に0除算を除き、対象行の比率は変えない。
    ...
    NDArray explorationCreditWeight =
        rawExplorationRatio
            ...
            .minimum(1.0f)
            .stopGradient();
    // beta_old は mu_old と ((1-alpha) pi_old + alpha mu_old) の重なりを正規化した方策。
    // PPO 更新中は beta_old を固定し、pi_theta/pi_old で再重み付けして正規化する。
    NDArray oldOverlap =
        rolloutPolicy
            .mul(1.0f - config.explorationCreditMix())
  ...
  /** 不正な方策は epsilon/クリップで救済せず、収集契約違反として検証に失敗した場合は処理を止めるにする。 */
  ...
  /**
   * 方策診断ベクトルを一つの行列へまとめ、同じ重みによる合計/min/maxを各一回だけ実行する。
   *
   * <p>診断値はすべて勾配から切り離されており、PPO 損失の計算グラフには接続しない。
   */
  ...
  /** 実行系所有の固定区間境界を共有し、教師ラベルの定数転送を省くHL-Gauss 目的関数。 */
  public static NDArray computeValueLoss(
      NDArray valueLogits,
      NDArray valueTarget,
      NDArray sampleWeight,
  ...
  /** 方策グラフの全二択分岐と候補正規化を損失精度で評価できるスコアへ変換する。 */
   ...
   * 保存データによる方策事前学習の集約値。
   *
   * @param total 選択最終行動の重み付き負の対数尤度
   * @param entropy 標準形式の最終的な行動の確率分布の重み付きエントロピー
   * @param chosenProbability 教師行動に割り当てた重み付き平均確率
   */
  ...
  /**
   * 未加工の探索比、探索分の学習への寄与、方策更新、実効方策比を混同しない選択行動診断。
   *
   * @param rawExplorationRatio {@code piRollout/muBehavior}。診断専用
   * @param explorationCreditWeight 重なり方策の非正規化質量と探索後方策の比
   * @param policyUpdate 重なり方策の更新比率
   * @param effectiveActorRatio {@code explorationCreditWeight * policyUpdate}
   * @param policyUpdateClipFraction クリップ範囲外だった方策学習の重みの比率
   * @param effectiveActorRatioMeanSquare 小バッチを跨いで厳密なESSを集約するための加重二次移動平均
   */
  ...
  /**
   * 方策、エントロピー、価値と監査指標を一つのデバイス上の結果にまとめる。
   *
   * @param total 選択段階の合計損失
   * @param policyGradientLoss 選択行動の方策勾配損失
   * @param entropyBonusLoss 最終的な行動分布のエントロピーを支える {@code -coefficient * entropy}
   * @param behaviorCloningLoss 模倣学習損失
   * @param valueLoss Decision スカラー価値の HL-Gauss 交差エントロピー
   * @param meanRolloutPolicyKl 行動選択プレイヤー-重み付き {@code KL(piRollout || piCurrent)} 平均
   ...
   * @param behaviorProbMean 選択行動の探索適用後の平均確率
   * @param behaviorProbMin 選択行動の探索適用後の最小確率
   * @param actorWeight 方策損失に寄与した重み総量
   * @param policyRatioDiagnostics 未加工の探索比と実効探索分の学習への寄与を分離した比率診断
   */
