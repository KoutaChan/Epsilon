 ...
 *
 * <p>局・プレイヤー・牌などの特徴表現の先頭トークンをクエリの初期値としてネイティブ処理段階内で選ぶため、非連続ビューや中間複製を作らない。方策のみでは方策
 * 特徴量の集約一つ、方策と価値が必要な場合は二つの集約を同じ投入で計算する。出力利用権は後段方策、価値、最終D2Hがすべて完了するまで保持する。
 */
public final class EpsilonMahjongStateReadoutGroupExecution implements AutoCloseable {
  ...
  /**
   * 凍結済み特徴量の集約を結び付け、一つまたは二つの特徴量の集約を実行する永続実行計画を作る。
   *
  ...
  /** 空き循環バッファ実行枠を予約し、一つのネットワーク順伝播を開始する。 */
