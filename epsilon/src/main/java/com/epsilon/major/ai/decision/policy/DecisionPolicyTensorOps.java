...
/**
 * 方策入力のコンパクト化、埋め込み参照、マスク付き遷移集約を担う形状演算群。
 *
 * <p>学習パラメーターを持たず、学習と推論で同じ経路選択を使う。保存された IDは0をパディング、1以降を実際のテーブルインデックス + 1として扱う。
 */
final class DecisionPolicyTensorOps {
   ...
   *
   * @param scores コンパクト遷移スコア
   * @param transitionEmbeddings 対応するコンパクト遷移表現
  ...
  /** 応答元の牌文脈と、手番中KANの基準牌文脈を行動候補へ割り当てる。 */
  ...
  /** コンパクト遷移のバッチ IDと牌IDから、元の34牌テーブルを複製せず対象牌だけを集める。 */
  ...
  /** コンパクト遷移のバッチ IDと牌IDから、元の全プレイヤー牌文脈を複製せず対象牌だけを集める。 */
  ...
  /** マスク外を0にし、候補ロジットの共通定数加算に不変な遷移重みを返す。 */
  ...
  /** 保存された IDの0をゼロベクトル、1以降をテーブルインデックス + 1としてバッチごとに埋め込みを引く。 */
  ...
  /** コンパクト行ごとの元バッチ IDを使い、共有テーブルから直接保存された IDを引く。 */
