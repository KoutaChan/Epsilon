...
import com.epsilon.major.ai.decision.policy.EpsilonBinaryBranchGate;
...
public final class DecisionPolicyFusionIndexedAffineExecution
  ...
  private GateProgram createProgram(
    ...
    long gateWidth = weight(hidden).getShape().get(0);
    FusionRecipe.Constant hiddenWeight =
        builder.addConstant(
            "hiddenWeight", FusionRecipe.TensorSpec.fixed(dataType, gateWidth, hiddenSize * 3L));
    FusionRecipe.Constant hiddenBias =
        builder.addConstant("hiddenBias", FusionRecipe.TensorSpec.fixed(dataType, gateWidth));
    FusionRecipe.Constant scoreWeight =
        builder.addConstant("scoreWeight", FusionRecipe.TensorSpec.fixed(dataType, 1, gateWidth));
  ...
  private static final class FusionForward extends Forward {
    ...
    /** 有効な位置がない処理箇所を実行済みにし、永続Fusion バッファを借用せずに完了させる。 */
