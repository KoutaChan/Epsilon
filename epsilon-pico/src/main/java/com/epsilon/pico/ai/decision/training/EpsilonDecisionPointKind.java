...
/**
 * 合法手の集合から分類する、判断機会の種類。
 *
 * <p>打牌だけなら DAHAI、リーチ宣言が可能なら RIICHI、鳴き・ロンへの応答や和了・槓などを含む判断は REACTION とする。実際に選択した行動だけでは分類しない。
 */
public enum EpsilonDecisionPointKind {
  ...
  /** 応答、和了、途中流局、カンのいずれかを含む希少決定。 */
  REACTION;

  /**
   * 一行の合法行動集合から決定機会の種別を分類する。
   *
   * @param input 詰めた合法手メタデータを持つホスト側バッチ
   * @param row 分類するバッチ行
  ...
  /** 入力と確率分布の本体をデータを復元せず、対局単位の学習データ付随情報の詰めた合法手 IDだけで分類する。 */
