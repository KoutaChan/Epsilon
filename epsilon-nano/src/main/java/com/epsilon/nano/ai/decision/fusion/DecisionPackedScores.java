 ...
 * {@link FusionOutputLease}を保持し、非同期D2Hが完了するまで枠の再利用を防ぐ。
 */
public final class DecisionPackedScores implements AutoCloseable {
   ...
   *
   * @param activeRows 有効行だけを持つ連結したスコアテンソル
  ...
  /** EAGER 連結したテンソルと、その計算が消費した上流利用権を束ねる。 */
  ...
  /**
   * 連結後の例外経路で、このスコアを生成するデバイス処理の完了を待つ。
   *
