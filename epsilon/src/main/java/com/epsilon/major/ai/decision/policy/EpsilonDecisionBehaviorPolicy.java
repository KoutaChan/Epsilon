...
/**
 * 正規化済み標準形式の最終的な行動の確率分布に、方策グラフ節点単位の探索と最終行動確率下限を適用する純粋計算。
 *
 * <p>上位節点の探索確率の重みは下位種類数から独立する。最大40個の合法位置は一つのビットマスクで扱い、推論ごとの {@code
 * List<Integer>}、ストリーム、mapを作らない。返す分布と正規化規則は学習時の方策グラフ階層と一致する。
 */
public final class EpsilonDecisionBehaviorPolicy {
   ...
   *
   * @param policyProbability モデルが返した正規化済み最終行動確率
   * @return 入力を複製した探索適用後の・対局生成方策
   */
   ...
   *
   * <p>配列は正値化・正規化され、返す探索後と探索前の方策は同一の不変配列を共有する。
   *
   * @param ownedPolicyProbability この呼び出しへ所有権を移す最終行動確率配列
   * @return 同一配列を探索適用後の・対局生成に持つ混合
   */
  ...
  /**
   * 合法行動と節点別探索率に従って探索を適用した最終行動の方策を構築する。
   *
   * @param policyProbability モデルが返した正規化済み最終行動確率
   * @param legalActions 確率位置と同順の合法行動
   * @param config 方策グラフ節点別の探索確率の重み
   * @return 探索後探索適用後のと探索前対局生成の混合
   */
  ...
  /**
   * 合法行動と節点別探索率へ判断固有の倍率を掛けて探索を適用した最終行動の方策を構築する。
   *
   * <p>倍率は節点探索確率の重みだけへ適用し、最終行動確率下限は固定する。
   */
  public static Mixture distribution(
   ...
   * @param legalActions 確率位置と同順の合法行動
   * @param config 方策グラフ節点別の探索確率の重み
   * @return 新規探索適用後の配列と所有権移譲された対局生成配列の混合
   */
   ...
   *
   * @param explorationMultiplier 全方策グラフ節点の探索確率の重みへ掛ける有限の非負倍率
   */
  public static Mixture distributionInPlace(
      float[] ownedPolicyProbability,
      List<Action> legalActions,
  ...
  /** 階層節点の条件付き確率が掛け合わさっても、各合法最終行動を正の確率で選択できる下限を付与する。 */
  ...
  /** 受諾側の条件付き確率。存在しない側へ確率の重みを割り当てない。 */
