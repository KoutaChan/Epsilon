 ...
 *
 * <p>状態、合法行動、行動適用後に選べる遷移を固定の項目幅のカテゴリ値フィールドと数値フィールド
 * に分離する。各カテゴリ値フィールドは自身の正確なカテゴリ数だけを持ち、その累積和を名前空間オフセットとして単一埋め込み
 * テーブルへ渡す。値0はbooleanのfalseや数値カテゴリのゼロにも使い、可変長構成要素の未使用位置かどうかはIDまたはPRESENT
 * フィールドの0で判定する。合法候補数と一候補あたりの遷移数だけは宣言済み容量区分に丸め、未使用位置をゼロ初期値のまま保持する。
 *
 * <p>{@link #fingerprint()} はフィールド順、容量区分、カテゴリ幅に加え、通し番号を格納する外部列挙型の名前と順序も含む。したがって
 * フィールドの追加・並べ替えだけでなく、{@link Action.Type} などの並べ替えも学習用に変換したデータセットとチェックポイント
 * の互換性を明示的に失わせ、意味の異なる入力を黙って読み込むことを防ぐ。
 */
public final class DecisionInputSchema {

  /** 学習用に変換したデータセットとチェックポイントの互換性判定に使うスキーマ世代。 */
  public static final int VERSION = 20;

  /** 可変長位置の未使用値。カテゴリ埋め込み上の実値とはフィールドごとの符号化・復号処理で分離する。 */
  public static final int PAD_ID = 0;

  /** 対局エンジンが一判断で生成し得る合法手の最大数。 */
   ...
   * 4人打ちの一人あたり河上限。
   *
   * <p>通常の最大18打牌に、最大4面子ぶんの鳴き/加槓後追加打牌を足した22以下。境界検査用に24を確保し、超過は切り捨てず不整合を検出して直ちに例外を送出する。
   */
  ...
  /** 判断対象の行動適用後の、具体的かつ自己完結した遷移の種類。 */
  public enum ActionTransitionKind {
    /** PASSなど、手牌を変えずに判断対象の行動だけで完結する遷移。 */
    IDENTITY,
  ...
  /** 共有の打牌スコア計算処理へ、同じ物理牌の打牌がどの判断から生じたかを伝える文脈。 */
  ...
  /** 判断対象の行動だけで確定し、後続打牌によって変わらないカテゴリ値フィールド。 */
  public enum ActionInt {
    ...
    /** CHI順子内で鳴いた牌が占める位置。 */
    CHI_CALLED_POSITION,
    /** 同じ物理牌の打牌をDAMA/RIICHI間で結ぶ識別情報 ID。 */
    DISCARD_IDENTITY,
    /** 成立面子に含まれる赤牌の由来。 */
  ...
  /** 判断対象の行動だけで確定する費用・和了・順位射影の数値フィールド。 */
   ...
   *
   * <p>値は埋め込みへ入れない。0は該当位置なし、それ以外は行動候補の位置インデックス + 1である。同じ物理牌の打牌に属する
   * DAMA/RIICHIをネットワーク内で全候補同士比較せず、入力生成時に一度だけ解決して直接指定位置から抽出する。
   */
  public enum ActionRoute {
    /** 同じ物理牌の打牌識別情報群を代表する識別情報 ID。 */
    DISCARD_IDENTITY_REPRESENTATIVE,
    /** 同じ物理牌の打牌識別情報群の代表行動候補の位置。 */
    DISCARD_REPRESENTATIVE_SLOT,
    /** この物理牌の打牌に対応するDAMA 行動候補の位置。 */
    DAMA_SLOT,
    /** この物理牌の打牌に対応するRIICHI 行動候補の位置。 */
    RIICHI_SLOT
  }

  /** 判断対象の行動適用後の一つの具体遷移に付与するカテゴリ値フィールド。 */
  public enum ActionTransitionInt {
    /** 識別情報・打牌・保留中の・終端の遷移種別。 */
    KIND,
  ...
  public enum ActionTransitionFloat {
    ...
    /** 現在の役の有無を問わない待ちに残る未観測物理牌枚数の正規化値。 */
    ...
    /** 1シャンテンから次の自摸・打牌でフリテンでない通常RON役のある聴牌へ進める自摸牌種数。 */
    NORMALIZED_NEXT_FURITEN_FREE_RON_YAKU_TENPAI_UKEIRE_KINDS,
    /** 1シャンテンから次の自摸・打牌でフリテンでない通常RON役のある聴牌へ進める残り物理牌枚数。 */
    ...
    /** 1シャンテンから次の自摸・打牌で通常TSUMO役のある聴牌へ進める自摸牌種数。 */
    NORMALIZED_NEXT_TSUMO_YAKU_TENPAI_UKEIRE_KINDS,
    /** 1シャンテンから次の自摸・打牌で通常TSUMO役のある聴牌へ進める残り物理牌枚数。 */
  ...
  /**
   * フィールド順・外部列挙型順・容量区分幅を含むスキーマ互換性互換性識別子を返す。
   *
   * <p>カテゴリ 配置はこのクラスの定数を参照するため、クラス初期化中には計算しない。遅延holderにより、どちらのクラスが先に参照されても完全に初期化された配置だけを使用する。
   */
