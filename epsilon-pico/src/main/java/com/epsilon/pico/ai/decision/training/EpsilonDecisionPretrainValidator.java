...
/**
 * 学習に使用しない検証データで、方策の一致率と価値予測の誤差を測定する。
 *
 * <p>価値については教師値への適合と実際の対局結果に対する誤差を分けて集計する。合法手が一つだけの行は方策の正解率、NLL、一様分布との比較、判断種別の件数、方策表現の統計から除外する。価値指標には全行を含め、全サンプル数と方策評価のサンプル数を別々に報告する。
 */
  ...
  public static ValidationMetrics evaluate(
    ...
    try (Model model =
            EpsilonDecisionCheckpointManager.loadForInference(
                ...
                config.bind(com.epsilon.pico.config.settings.DecisionInferenceSettings.class));
      ...
      try {
        samples =
            EpsilonDecisionPretrainTargets.prepare(
                ...
                grpSession,
                config
                    .bind(com.epsilon.pico.config.settings.DecisionSettings.class)
                    .utilityProfile());
