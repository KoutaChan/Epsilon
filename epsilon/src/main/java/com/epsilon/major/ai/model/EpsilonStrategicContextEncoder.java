...
final class EpsilonStrategicContextEncoder extends AbstractBlock {
  ...
  private static final int BLOCK_COUNT = EpsilonDecisionArchitecture.STRATEGIC_BLOCK_COUNT;
  ...
  private static final int MAXIMUM_FEED_FORWARD_WIDTH =
      EpsilonDecisionArchitecture.STRATEGIC_FEED_FORWARD_WIDTH;
  ...
  /** 34牌種を自家手牌トークンへ集約し、局・手牌・4席の6 トークンを3層で文脈化する。 */
  ...
  /** 34牌種と4席の要約を作り、推論時だけ永続戦略文脈の層の組実行計画を利用する。 */
  StrategicContext encode(
      ParameterStore parameterStore,
      NDArray roundEmbedding,
      NDArray tileEmbeddings,
  ...
  /** 6 トークンだけを対象に、128幅注意機構と960幅FFNで残差更新するブロック。 */
  private static final class StrategicBlock extends AbstractBlock {
    ...
    StrategicBlock(int hiddenSize, int attentionHeads) {
      ...
      feedForwardWidth = Math.min(hiddenSize * 5 / 2, MAXIMUM_FEED_FORWARD_WIDTH);
