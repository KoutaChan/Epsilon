...
/**
 * 1対局分の学習データを、現行のバイナリ形式で符号化・復号する。
 *
 * <p>最終順位、GRP
 * の局履歴、4席・4順位の分布を対局ヘッダーに一度だけ保存する。各判断は局境界の番号とプレイヤーごとの判断番号で補助データを参照する。入力と方策分布は外部ファイルに置き、旧形式のデータは明示的に拒否する。
 */
final class EpsilonDecisionFragmentCodec {
  ...
  /** GRP 付随情報・サンプル本体・入力と確率分布の本体を読まず、学習器の選抜に必要な接頭部分だけを返す。 */
  ...
  /**
   * 指定global位置の軽量サンプルの参照情報だけを復元する。
   *
   * <p>選ばれなかった行は配列を生成せずに読み飛ばし、選ばれた行も密な入力と方策入力と確率分布の本体は外部ファイルに残す。
   */
