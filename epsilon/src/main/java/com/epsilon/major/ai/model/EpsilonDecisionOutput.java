 ...
 * @param valueLogits 自家の効用-区間ロジット [バッチ,101]。順位確率ではなく区間中心の期待値へ復号する
 */
public record EpsilonDecisionOutput(DecisionPolicyScores policyScores, NDArray valueLogits) {
  ...
  /**
   * 推論向けに最終行動の対数確率だけを構築する。
   *
   * @param actionCategories 行動種類・グループを持つカテゴリ値テンソル
   * @param actionRoutes 同一打牌と RIICHI・DAMA 対応を持つ経路選択テンソル
   * @return パディングに有限な大負値を持つ最終行動対数確率
   */
  ...
  /**
   * 汎用 Block・チェックポイント API 用の固定順テンソル一覧へ変換する。
   *
   * @return 方策スコア群の後ろに価値ロジットを置いたテンソル一覧
   */
