...
/**
 * 行動 ID、牌種ごとの関係、役のビット集合を、パディングを含む保存用カテゴリ値へ変換する。
 *
 * <p>個々の値の符号化と復号だけを担当し、連続バッファの位置やテンソルの形状は扱わない。
 */
public final class DecisionFeatureCodec {
  ...
  /**
   * 対局エンジン行動 IDを、0をパディングに予約した保存カテゴリへ変換する。
   *
   ...
   *
   * <p>0はパディング専用。実値は結果副露していない手牌枚数、改善牌残り枚数、役の有無を問わない待ち、RON役あり待ち、TSUMO役あり待ち、打牌、
   * 新設面子への使用、赤5和了牌の残存可能性の直積を使う。
   *
   * @param concealedCount 遷移後の副露していない手牌にある同牌種枚数
   ...
   * @param calledIntoMeld 判断対象の行動で新しい面子へこの牌種を使うなら {@code true}
  ...
  /**
   * 遷移牌カテゴリから結果副露していない手牌枚数を復元する。
   *
   * @param storedCode {@link #transitionTile} が返したカテゴリ
   * @return 同牌種の副露していない手牌枚数
   */
  ...
  /**
   * 遷移牌カテゴリが役の有無を問わない待ちを表すかを返す。
   *
   * @param storedCode {@link #transitionTile} が返したカテゴリ
   * @return 役の有無を問わない待ちなら {@code true}
   */
   ...
   *
   * @param storedCode {@link #transitionTile} が返したカテゴリ
   * @return 判断対象の行動で面子へ使う牌種なら {@code true}
   */
