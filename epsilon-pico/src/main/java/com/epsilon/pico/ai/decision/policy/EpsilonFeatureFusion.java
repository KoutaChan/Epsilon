 ...
 * メモリとの通信表現は小さいコンテキスト幅のまま受け取れるため、隠れ層の幅へ事前展開する行列積と一時テンソルを作らない。
 * 先頭構成要素だけがバイアスを持ち、加算後のバイアスが構成要素数に依存しないようにする。
 *
 * <p>ここでのFusionは学習モデル上の特徴合成を意味し、djl-rocmのFusion 実行計画ではない。重みを固定したモデルの推論用のネイティブ実装は {@code
 * com.epsilon.pico.ai.decision.policy.fusion} が所有する。
 */
public final class EpsilonFeatureFusion extends AbstractBlock {
  ...
  /** チェックポイント名を変えず既存射影を結び付けるため、指定構成要素のLinearを返す。 */
   ...
   * 候補軸へブロードキャストする構成要素は接頭部分へ含めず、候補数分の不要なFLOPsを増やさない。
   */
