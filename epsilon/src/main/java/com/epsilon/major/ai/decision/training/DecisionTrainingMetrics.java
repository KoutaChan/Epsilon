 ...
 *
 * <p>損失の構築と逆伝播は学習処理が所有し、このクラスは学習結果を変更しない観測専用です。価値・BCはサンプル確率の重み、方策固有値は有効な方策 確率の重みで集約します。
 */
final class DecisionTrainingMetrics {
  ...
  /** まとめたスカラーを保持し、オプティマイザー確定より前の明示的有限値検査に使います。 */
