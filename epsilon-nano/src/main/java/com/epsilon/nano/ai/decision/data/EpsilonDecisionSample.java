 ...
 *
 * <p>{@code valueTarget}は直後の次局GRPで閉じる探索補正付きスカラー価値、{@code
 * advantage}は因果関係のある行動の先読み価値と対局生成予測の効用差である。探索後の / 対局生成方策は合法手候補の位置順の正規化済み確率である。
 *
 * @param input 1 判断行の型付きホスト入力
 * @param chosenLegalSlot 動的合法候補列で実際に選択した枠
 * @param chosenActionId 選択枠に対応する安定行動 ID
 * @param behaviorLogProb 探索を含む探索後の方策での選択行動対数確率
...
public record EpsilonDecisionSample(
  ...
  /**
   * 完走牌譜の一行を、初期化用の教師値生成前の事前学習サンプルとして作る。
   *
   * <p>選択方策はワンホット、Decision 価値は終局効用、席通し番号は未割当である。{@link
   * EpsilonDecisionPretrainTargets#prepare}がゲーム全体を見て局境界を一意化し、共通形式の教師値へ置換する。
   */
  ...
  /**
   * 合法手候補の位置順のワンホット方策を作る。
   *
   * @param legalActionCount 動的合法候補数
   * @param chosenLegalSlot 1にする有効要素のみの合法枠
   * @return 長さ {@code legalActionCount} のワンホット配列
   */
