...
public final class EpsilonDecisionLoss {
  ...
  /**
   * 保存データによる方策出力用ネットワーク専用のサンプル-重み付き模倣学習損失を計算する。
   *
   * @param output ネットワークが出力した候補・選択肢スコア
   * @param input 合法候補、経路、教師位置、サンプル重みを持つデバイス側バッチ
  ...
  /**
   * 完全な入力検査を有効にして自己対局による損失を計算する診断向けoverload。
   *
   * <p>本番頻繁に実行する処理ではモデルに固定した効用の定義を渡し、検査済み対局単位の学習データを CPU へ戻さない。
   *
   * @param output 方策と価値のネットワーク出力
   * @param input 自己対局による教師値を持つデバイス側バッチ
   * @param config PPO クリップ・探索による選択の学習への寄与混合率・エントロピー係数の設定
   * @param hasPositiveActorWeight バッチに方策更新の対象サンプルが存在するか
   * @return 逆伝播対象損失と全診断量
   */
  ...
  /** ネットワークの方策スコアを損失と同じFLOAT32境界で末端の行動分布へ合成する。 */
  ...
  public static TrainingLossResult computeOnlineTrainingLoss(
    ...
    // 方策更新の対象外の行は選択確率ゼロを許す。重みを掛ける前に0除算を除き、対象行の比率は変えない。
  ...
  /** invalid 方策は epsilon/クリップで救済せず、収集契約違反として検証に失敗した場合は処理を止めるにする。 */
  ...
  /** 学習ワーカー所有の固定区間境界を共有し、教師ラベルの定数転送を省くHL-Gauss 目的関数。 */
  public static NDArray computeValueLoss(
      NDArray valueLogits,
      NDArray valueTarget,
      NDArray sampleWeight,
  ...
  /** 方策グラフの全二択の判定と候補正規化を損失精度で評価できるスコアへ変換する。 */
   ...
   * 保存データによる方策事前学習の集約値。
   *
   * @param total 選択末端の行動の重み付き負の対数尤度
   ...
   * @param policyRatioDiagnostics 未加工の探索比と実効探索による選択の学習への寄与を分離した比率診断
   */
