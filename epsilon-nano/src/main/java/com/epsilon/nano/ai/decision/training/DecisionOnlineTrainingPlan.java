...
public record DecisionOnlineTrainingPlan(
  ...
  /** テストや小規模な実行向けに遷移-格納枠上限を実質無制限とする最小計画。 */
  ...
  /** クリップ幅を明示し、探索学習への寄与には標準値を使うテストや小規模な実行向け最小計画。 */
  ...
  /** この新しく生成した学習の反復から実行する方策モデル・価値オプティマイザー確定総数。 */
  ...
  /** 探索学習への寄与混合率は標準値を使い、指定した更新段階数を1 エポックだけ実行する。 */
  public DecisionOnlineTrainingPlan(
      int microBatchSize,
      int maximumDeviceTransitionCells,
      float policyUpdateClipRange,
  ...
  /** 探索学習への寄与混合率を明示し、指定した更新段階数を1 エポックだけ実行する。 */
  public DecisionOnlineTrainingPlan(
      int microBatchSize,
      int maximumDeviceTransitionCells,
      float policyUpdateClipRange,
