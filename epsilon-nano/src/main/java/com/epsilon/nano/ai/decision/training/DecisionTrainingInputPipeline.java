...
import com.epsilon.nano.ai.decision.input.DecisionTrainingTargetLayout;
...
/**
 * 学習ワーカーごとに、復号とホストからデバイスへの転送を容量制限付きで並列に進める。
 *
 * <p>容量は二小バッチに固定する。単一のワーカーが外部データ本体セッションとホスト書き込み処理を専有し、記述情報から最終ホスト 連続バッファへ直接復号する。{@link
 * DecisionTrainingDeviceTransferSchedule#ON_DEMAND}では確定済みホスト バッチを返し、計算用ワーカーが取得した時点で転送する。{@link
 * DecisionTrainingDeviceTransferSchedule#OVERLAPPED}では復号直後に実行単位専用ストリームへ
 * H2Dをキューへ追加する。呼び出し側のAPIは両方式で同一である。
 *
 * <p>ページ固定のホスト枠は順伝播終了ではなくH2D 完了完了通知で回収する。デバイス側バッチは独立した子管理元へ所有権を 移すため、ホスト枠を次バッチの復号へ先に再利用できる。
 */
final class DecisionTrainingInputPipeline implements AutoCloseable {

  /** 現在のバッチと先読みバッチを同時に保持する固定容量。 */
   ...
   * 各学習ワーカー専用のパイプラインを作る。
   *
   * @param laneIndex データ並列の学習ワーカーの固定インデックス
   * @param owner 実行単位のモデルデバイスを所有する管理元
  ...
  /**
   * H2D完了済み完了通知を回収し、現在復号へ再利用できるホスト枠数を返す。
   *
   * <p>デバイス利用権の寿命とは独立したホスト再利用をテストと診断用の集計から確認するための各学習ワーカー専用の値である。
   */
  ...
  /**
   * 記述情報小バッチを復号キューへ一度だけ投入する。
   *
   * <p>二件が未完了（キュー、準備済み、デバイス利用権のいずれか）なら三件目を受理せず直ちに例外を送出する。これにより実行処理キューやホスト・デバイス メモリが学習速度とは無関係に増えない。
   *
   * @param plan コピーせず所有権を借りる作業計画
   * @return 復号と任意の先行H2Dを経て準備済みバッチで完了を通知する CompletableFuture
   */
   ...
   * 準備済みバッチを現在の演算ストリームへ一度だけ取得する。
   *
   * <p>OVERLAPPEDでは転送イベントのGPU 側の待機だけを現在のストリームのキューへ追加する。ホストスレッドは複製完了を待たない。
   *
   * @param ready このパイプラインが返した準備済みバッチ
  ...
  /**
   * ワーカーキューを完了待ちと回収した後、同じワーカー上でキャッシュ済みデータ本体チャネルを閉じる。
   *
   * <p>学習反復の区切りで呼ぶと、削除済みデータ本体ファイルの領域を次の反復まで保持しない。
   */
  ...
  /**
   * ワーカー、準備済みバッチ、H2D 完了通知、ホスト記憶領域の順で実行単位資源を閉じる。
   *
   * <p>デバイス利用権は独立した子管理元を所有するため、このパイプラインより後に閉じてもよい。
   */
  @Override
  public void close() {
    synchronized (this) {
  ...
  /** 二つのホスト枠の一つ。状態遷移は外側パイプラインが直列化する。 */
  ...
  /** 一種類の再利用可能なページ固定のまたは直接参照のホスト側の連続バッファ。 */
  ...
  /** H2D 完了通知、元のデバイス連続バッファ、ホスト枠再利用をまとめる内部所有権。 */
