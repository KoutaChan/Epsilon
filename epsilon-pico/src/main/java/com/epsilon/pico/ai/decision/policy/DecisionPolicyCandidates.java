...
public final class DecisionPolicyCandidates {
  ...
  /**
   * CONTINUE後の「物理牌の打牌識別情報 → DAMA/RIICHI」階層を構成するマスクを返す。
   *
   * <p>同一識別情報にDAMAがあればDAMA 位置を識別情報代表とし、RIICHIしか合法でない場合だけRIICHI 位置を代表にする。
   */
  ...
  /**
   * 物理牌の打牌識別情報とDAMA/RIICHI分岐を表す不変マスク群。
   *
   ...
   * @param representativeSlots 各行動と同一識別情報の代表位置 + 1
   * @param damaSlots 各行動と同一識別情報のDAMA 位置 + 1。存在しなければ0
   * @param riichiSlots 各行動と同一識別情報のRIICHI 位置 + 1。存在しなければ0
   * @param damaPresent 各行動識別情報にDAMA候補が存在する場合1
   * @param riichiPresent 各行動識別情報にRIICHI候補が存在する場合1
   * @param representativeMask 各物理牌の打牌識別情報につき一つだけ立つ代表位置マスク
   */
