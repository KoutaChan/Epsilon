...
/**
 * Decision の入力を1行ずつ、特徴量の項目名を指定して書き込む。
 *
 * <p>項目番号やバッファ内の位置計算を呼び出し側から隠す。値は入力エンコーダーが検証済みであることを前提とし、書き込むたびには再検証しない。
 */
public final class DecisionInputWriter {
   ...
   * @param riverIndex その席の河の中のインデックス
   * @param field 書き込むフィールド
   * @param value フィールド固有カテゴリ数内のカテゴリ ID
   */
  public void river(
  ...
  /**
   * 方策グラフ用の枠参照先の対応メタデータを書き込む。
   *
   * @param actionSlot 参照経路を所有する行動候補の位置
   * @param field 参照経路種別
  ...
  /**
   * 待ち牌のRON・TSUMO別役マスクチャンクを書き込む。
   *
   * @param actionSlot 親最初の行動候補の位置
   * @param transitionSlot 行動内の遷移候補の位置
   * @param waitSlot 待ち集合内の枠
   * @param feature RON・TSUMOを通したチャンクインデックス
