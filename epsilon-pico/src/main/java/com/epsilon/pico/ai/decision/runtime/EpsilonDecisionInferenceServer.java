...
import com.epsilon.pico.ai.decision.fusion.DecisionInferenceOutputPacker;
import com.epsilon.pico.ai.decision.fusion.DecisionPackedScores;
...
import com.epsilon.pico.ai.decision.input.DecisionStateInferenceBatch;
import com.epsilon.pico.ai.decision.input.DecisionStateInputs;
import com.epsilon.pico.ai.decision.policy.DecisionPolicyInferenceComposer;
import com.epsilon.pico.ai.decision.policy.DecisionPolicyScores;
import com.epsilon.pico.ai.model.DecisionInferenceExecution;
 ...
 *
 * <p>ホスト側バッチがデバイス上限を超える場合は非所有の連続 {@link DecisionHostBatch.RowSlice} に分割する。各スライスは {@link
 * DecisionBatchTransfer.Workspace}のページロックした/直接参照のバッファへ直接詰められるため、中間のJava配列を作らない。GPU推論では複数の非所有行
 * ビューを{@link DecisionDevicePipeline}へ渡し、Fusion/方策利用権とバッチ
 * 管理元を最終D2H完了まで位置に保持する。返す方策は合法手候補の位置順の確率または自然対数確率、価値はHL-Gaussから復号した期待効用である。
 */
public final class EpsilonDecisionInferenceServer implements EpsilonDecisionGreedyEvaluator {
  ...
  /** 複数循環バッファ位置から同時に参照できる凍結方策・価値実行コンテキストを作ります。 */
  ...
  /** 行動だけを消費する対局生成用に、重みを固定した方策のみサーバーを作る。 */
  ...
  /** 複数循環バッファ位置から同時に参照できる凍結方策のみ実行コンテキストを作ります。 */
  ...
  /** 対戦評価の明示上限だけを適用し、元のグラフ行数設定の検証は維持する。 */
  ...
  /** 対局実行処理が所有する物理位置数で、モデル固有の実行作業領域を構築します。 */
  public static EpsilonDecisionInferenceServer forPolicySession(
      Model model,
      DecisionInferenceSettings settings,
      DecisionInferenceFusionSettings fusion,
   ...
   *
   * <p>順伝播と方策グラフ合成は通常の方策のみ推論と同じであり、ホスト出力の表現だけを詰めたにする。
   */
  @Override
  ...
  /**
   * 所有連続バッファを複製せず推論行ビューから最大確率の行動を選ぶ方式位置だけを返す。
   *
   * <p>呼び出し元は完了までビューの所有権を保持する。
   */
  ...
  /**
   * 単一デバイス診断経路の詰めた出力をホストへ読み、読み終えた後だけFusion/方策利用権を返します。
   *
   * <p>ホスト readが失敗した場合も、スコアが表す最終デバイス処理の完了を確定してから利用権を閉じます。
   */
   ...
   *
   * <p>診断経路はサーバーごとに直列実行されるため、イベント完了時点でこの実行コンテキストに属する未完了順伝播はすべて安全に回収できます。
   */
  ...
  /** 物理バッチの診断コールバックを解除します。通常推論では観測者参照一回以外の処理を行いません。 */
  ...
  /** 固定入力の一バッチを診断セッション付きで実行する。呼出側は完了を待ってから次を投入する。 */
  public CompletableFuture<DecisionInferenceResult> submitProfile(
      DecisionHostBatch.RowBatch rows, InferenceProfile profile) {
    requireReplayRows(rows);
    RingWork work = new RingWork(rows, DecisionInferenceResult.Kind.PREDICTIONS, profile);
    submitReplayWork(work);
    return work.future;
  }

  /** 一つの所有バッチを固定入力として、D2Hとホスト確率の合成を除いた循環バッファパイプラインへ投入します。 */
  ...
  /** このモデルコンテキストを含む物理GPU上の推定保留中の処理を返します。 */
   ...
   *
   * <p>循環バッファの失敗バッチは各位置が最終イベントを待っています。パイプライン利用権の処理の回収後に限り、途中失敗が保持したFusion出力位置と方策 利用権を一括回収します。
   */
  @Override
   ...
   *
   * <p>通常推論の詰めたスコア転送には使わず、診断を要求した呼び出しだけで必要な型変換を行う。
   *
  ...
  /** 一つの物理循環バッファ位置上で処理段階からホスト確率の合成までを進めるバッチ所有権です。 */
  private final class RingWork implements DecisionDevicePipeline.Work {
    ...
    @Override
    public InferenceProfile profile() {
      return profile;
    }
    ...
    public void submit(DecisionDevicePipeline.Slot slot) {
      ...
      try {
        ...
        try (PtStreamScope ignored = slot.computeStream().openScope();
          ...
          try (var section = InferenceProfile.section("network.forward")) {
            packedScores =
                stagedState == null
    ...
    public boolean abort(DecisionDevicePipeline.Slot slot, Throwable failure) {
      ...
      // バッチ単位で安全に回収できないため、位置を再利用せずパイプライン全体を不整合を検出して直ちに例外を送出するさせる。
      if (computeStarted) {
        fail(failure);
        return false;
      }
  ...
  /** 一度だけデバイスへ移した固定入力を共有し、順伝播と出力詰める処理だけを反復するセッションです。 */
  public final class FixedReplay implements AutoCloseable {
    ...
    /** H2DとD2Hを行わず、本番と同じ順伝播と出力詰める処理を一回投入します。 */
  ...
  /** 固定入力を一度だけ位置のページロックしたバッファから長寿命デバイステンソルへ移す処理です。 */
  ...
  /** 長寿命デバイス入力へ本番と同じ順伝播と出力詰める処理を適用する処理です。 */
   ...
   * #policyLogProbabilities()}で変換し、無作為抽出直前の不要なログ/exp往復を避ける。公開コンストラクターは従来どおり自然対数確率を受け取る。
   */
