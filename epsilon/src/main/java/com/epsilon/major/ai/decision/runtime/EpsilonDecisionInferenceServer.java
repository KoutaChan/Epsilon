...
import com.epsilon.major.ai.decision.fusion.DecisionInferenceOutputPacker;
import com.epsilon.major.ai.decision.fusion.DecisionPackedScores;
...
import com.epsilon.major.ai.decision.input.DecisionStateInferenceBatch;
import com.epsilon.major.ai.decision.input.DecisionStateInputs;
import com.epsilon.major.ai.decision.policy.DecisionPolicyInferenceComposer;
import com.epsilon.major.ai.decision.policy.DecisionPolicyScores;
import com.epsilon.major.ai.model.DecisionInferenceExecution;
 ...
 *
 * <p>ホスト側バッチがデバイス上限を超える場合は非所有の連続 {@link DecisionHostBatch.RowSlice} に分割する。各部分領域は {@link
 * DecisionBatchTransfer.Workspace}のページ固定の/ダイレクトバッファへ直接詰められるため、中間のJava配列を作らない。GPU推論では複数の非所有行
 * ビューを{@link DecisionDevicePipeline}へ渡し、Fusion/方策利用権とバッチ
 * 管理元を最終D2H完了まで実行枠に保持する。返す方策は合法手候補の位置順の確率または自然対数確率、価値はHL-Gaussから復号した期待効用である。
 */
public final class EpsilonDecisionInferenceServer implements EpsilonDecisionGreedyEvaluator {
  ...
  /** 複数循環バッファ実行枠から同時に参照できる凍結方策/価値実行コンテキストを作ります。 */
  ...
  /** GPU 一致性検証用に、構成部分別Fusion設定を指定して凍結方策/価値サーバーを作ります。 */
  static EpsilonDecisionInferenceServer forFrozenModel(
      Model model,
      int maxBatch,
      int executionSlots,
  ...
  /** 複数循環バッファ実行枠から同時に参照できる凍結方策のみ実行コンテキストを作ります。 */
  ...
  /** 対局実行処理が所有する物理実行枠数で、モデル固有の実行作業領域を構築します。 */
  public static EpsilonDecisionInferenceServer forPolicySession(
      Model model,
      DecisionInferenceSettings settings,
      DecisionInferenceFusionSettings fusion,
  ...
  /** GPU 一致性検証用に、構成部分別Fusion設定を指定して凍結方策のみサーバーを作ります。 */
  static EpsilonDecisionInferenceServer forFrozenPolicyModel(
      Model model,
      int maxBatch,
      int executionSlots,
  ...
  /** 構成部分別Fusion設定が、この推論コンテキストのデバイス・パラメーター契約を満たすことを検証する。 */
  ...
  /**
   * 方策のみモデルを評価し、全候補分布を保持せずに各行の最大確率の行動候補の位置だけを返す。
   *
   * <p>順伝播と方策グラフ合成は通常の方策のみ推論と同じであり、ホスト出力の表現だけをコンパクトにする。
   */
  @Override
  ...
  /**
   * 所有連続バッファを複製せず推論行ビューから最大確率の行動を選ぶ方式で選ぶ候補位置だけを返す。
   *
   * <p>呼び出し元は完了までビューの所有権を保持する。
   */
  ...
  /**
   * 単一デバイス診断経路のまとめた出力をホストへ読み、読み終えた後だけFusion/方策利用権を返します。
   *
  ...
  /** 物理バッチの診断コールバックを解除します。通常推論では観測コールバック参照一回以外の処理を行いません。 */
  ...
  /** 一つの所有バッチを固定入力として、D2Hとホスト方策の合成を除いた循環バッファパイプラインへ投入します。 */
   ...
   *
   * <p>循環バッファの失敗バッチは各実行枠が最終イベントを待っています。パイプライン利用権の処理完了まで回収後に限り、途中失敗が保持したFusion出力実行枠と方策 利用権を一括回収します。
   */
  @Override
   ...
   *
   * <p>通常推論のまとめたスコア転送には使わず、診断を要求した呼び出しだけで必要な型変換を行う。
   *
  ...
  /** 一つの物理循環バッファ実行枠上で処理段階からホスト方策の合成までを進めるバッチ所有権です。 */
  private final class RingWork implements DecisionDevicePipeline.Work {
    ...
    public boolean abort(DecisionDevicePipeline.Slot slot, Throwable failure) {
      ...
      // バッチ単位で安全に回収できないため、実行枠を再利用せずパイプライン全体を例外を送出して停止させる。
      if (computeStarted) {
        fail(failure);
        return false;
      }
  ...
  /** 一度だけデバイスへ移した固定入力を共有し、順伝播と出力連結処理だけを反復するセッションです。 */
  public final class FixedReplay implements AutoCloseable {
    ...
    /** H2DとD2Hを行わず、本番と同じ順伝播と出力連結処理を一回投入します。 */
  ...
  /** 固定入力を一度だけ実行枠のページ固定のバッファから長寿命デバイステンソルへ移す処理です。 */
  ...
  /** 長寿命デバイス入力へ本番と同じ順伝播と出力連結処理を適用する処理です。 */
   ...
   * #policyLogProbabilities()}で変換し、無作為抽出直前の不要な対数・指数変換往復を避ける。公開コンストラクターは従来どおり自然対数確率を受け取る。
   */
