...
public final class NetworkDevices implements Iterable<Device> {
   ...
   * 指定順を維持したデバイス集合を作る。
   *
   * <p>同じデバイスを複数実行系へ割り当てるCPU テスト等を許すため、重複は保持する。
   *
   * @param devices 主デバイスから並んだ1台以上のデバイス
   * @return 変更不可なデバイス集合
   */
  ...
  /**
   * 集合内の指定デバイスを主デバイスへ移した順序を返す。
   *
   * <p>元の集合とデバイス重複数は変えず、指定デバイスと現在の先頭だけを入れ替える。
   *
   * @param primary 新しい主デバイスデバイス
   * @return 主デバイスだけを並べ替えた集合
   */
