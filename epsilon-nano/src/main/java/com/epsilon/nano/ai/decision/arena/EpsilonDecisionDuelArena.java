...
public final class EpsilonDecisionDuelArena {
   ...
   *
   * <p>同じ牌山の乱数シードごとに候補を0..3席へ一度ずつ置き、残り3席を対戦相手にする。{@code games} は半荘数で、1評価単位の4半荘へ切り上げる。
   *
   ...
   * は4席が揃った牌山ごとにスケジューラースレッドから呼ばれるため、進捗表示や固定標本統計を対局の停止なしで更新できる。
   */
  ...
  private static DuelRun runDuel(
    ...
    Totals totals =
        new Totals(
            collectOutcomes,
            wallOutcomeSink,
            config.bind(com.epsilon.nano.config.settings.DecisionSettings.class).utilityProfile());
   ...
   * 順位順列について厳密に0になる。したがって候補1人と比較元3人の対戦におけるの4席平均は、追加の 比較元同士の対局を行わずに候補から比較元を引いた差として使える。
   */
  ...
  /**
   * 対戦比較の統計結果と、その根拠となる席順を入れ替えた対局／牌山観測。
   *
   * @param result 集約した対応をそろえた対戦比較指標
   * @param metrics 対局のバッチ処理・推論指標
  ...
  /**
   * 1つの牌山と評価対象の席に対応する対戦結果。計測した指標を保持する。
   *
