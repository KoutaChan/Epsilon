...
/**
 * カテゴリ特徴量の値域と、共有埋め込みテーブル内の配置を定義する。
 *
 * <p>各フィールドは自身が実際に使うカテゴリ数だけを確保する。以前の全フィールド共通固定ストライドは使わず、フィールドごとのカテゴリ数の累積和を名前空間オフセットとする。構成要素が複数存在するプレイヤー、牌、河、面子は同じフィールド名前空間を共有するため、同じ意味の値は
 * 構成要素位置によらず同じ埋め込みパラメーターを参照する。
 *
 * <p>Encoderは{@link DecisionInputWriter}を通して値域検査済みの値だけを書き込む。これにより、あるフィールドの値が隣のフィールド
 * 名前空間へ入り込む破損を入力生成時に即座に検出する。
 */
public final class DecisionCategoryLayout {
  ...
  /** 行動候補・遷移フィールドが共有する候補埋め込み辞書の総要素数。 */
  ...
  /**
   * 一次元にした状態テンソルの各位置に加えるフィールド名前空間オフセットを返す。
   *
   * @return 呼び出し側から変更できる新しいオフセット配列
   */
  public static int[] stateOffsets() {
    int[] result = new int[DecisionInputSchema.PUBLIC_HISTORY_RELATION_OFFSET];
    int cursor = 0;
