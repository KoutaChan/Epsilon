...
import com.epsilon.pico.ai.decision.training.EpsilonDecisionPretrainTargets;
 ...
 * @param chosenActionId 選択位置に対応する安定行動 ID
 * @param behaviorLogProb 探索を含む実際の行動選択に使う方策での選択行動対数確率
...
public record EpsilonDecisionSample(
  ...
  /**
   * 完走牌譜の一行を、事前学習の開始用教師値生成前の事前学習サンプルとして作る。
   *
  ...
  /** 形状、識別情報、確率分布、教師値のサンプル契約を不整合を検出して直ちに例外を送出するで検証する。 */
  ...
  /** 固定検証処理で使う参照方策だけを置き換え、収集時の探索適用後のと学習教師値は保持する。 */
   ...
   * @param chosenLegalSlot 1にする詰めた合法位置
