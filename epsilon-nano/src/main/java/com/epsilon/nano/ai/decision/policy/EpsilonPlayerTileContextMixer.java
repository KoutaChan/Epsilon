...
/**
 * 同じ牌種に対する4人の局所文脈を、牌種ごとに独立して非線形に合成する残差ブロック。
 *
...
public final class EpsilonPlayerTileContextMixer extends AbstractBlock {
  ...
  /**
   * CPU の通常 Linear は自動混合精度を適用した入力と、FLOAT32 の更新用重みを受け付ける。 独自の MLP の CPU 代替処理
   * は計算前に同データ型を要求するため、同じ式を標準演算で評価する。 パラメーター/入力の toType は使わず、元の学習計算グラフへの勾配を保つ。
   */
