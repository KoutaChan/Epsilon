...
public final class EpsilonGrpTrainingSession implements AutoCloseable {
   ...
   *
   * @param decisionCheckpointDir GRP 子ディレクトリを持つ Decision チェックポイントの親ディレクトリ
  ...
  /**
   * 学習データ片ヘッダーに保存する、現在開いている固定した教師モデルの内容識別情報を返す。
   *
   * @return チェックポイント世代と推論保存物 SHA-256
   * @throws IOException 有効セッションが保存済みチェックポイント識別情報を持たない場合
   */
