...
/** 一つの選択行動の方策勾配学習の評価間隔と直後の対局実行処理対戦評価結果。 */
public record DecisionSelectedPgDuelResult(
  ...
  /** 候補が対局収集用の採用モデルへ昇格したか。 */
  public boolean promoted() {
    return status == Status.PROMOTED;
  }

  /** 固定牌山対戦評価の判定と一連の学習実行枠から、評価間隔の終端状態を決める。 */
  ...
  /** 一つの学習評価間隔後の対局実行処理判定。 */
  public enum Status {
    /** 固定 CI下限が0を超え、対局収集用の採用モデルへ昇格した。 */
    PROMOTED(true),

    /** 固定 CI上限が0未満となり、一連の学習実行を停止した。 */
    HARMFUL(false),

    /** CIが0をまたぎ、同じ学習の継続状態とオプティマイザーで次評価間隔へ進む。 */
    UNRESOLVED(true),
    ...
    /** 現在のモデル、AdamW、KL 制御処理を次評価間隔へ保持するか。 */
