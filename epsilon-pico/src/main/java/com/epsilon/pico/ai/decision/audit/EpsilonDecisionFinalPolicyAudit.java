...
import com.epsilon.pico.ai.decision.training.DecisionPolicySignalMultipliers;
import com.epsilon.pico.ai.decision.training.DecisionPolicyTrustRegion;
import com.epsilon.pico.ai.decision.training.EpsilonDecisionTrainer;
import com.epsilon.pico.ai.decision.training.EpsilonDecisionTrainingBatch;
...
/**
 * 候補モデルの学習対象データを全件再評価し、方策の更新比率と収集時の方策からの KL ダイバージェンスを測る。
 *
 * <p>メモリ内の入力は一括で、逐次読み込む入力は件数を区切って評価する。方策更新対象外のサンプルも入力と出力の有限性を検査し、方策・KL 指標の集計だけから除外する。
 */
public final class EpsilonDecisionFinalPolicyAudit {
  ...
  /**
   * 与えられた順序を変えず、固定幅バッチごとに方策更新と対局生成 KL 検証条件の診断量を計算する。
   *
   ...
   * @return 入力元全体のKL・探索による選択の学習への寄与・方策更新比率監査結果
   */
  ...
  /**
   * 判断機会別行動選択プレイヤー重みを本番更新と同じ定義で適用する全件監査。
   *
   * @param model 監査対象 Decision モデル
   * @param samples 省略せず評価する選択した入力元
   * @param batchSize 一回の論理バッチ行数
  ...
  /**
   * 対局単位の学習データ読み取り処理が返す選択した入力元を省略せず全件監査する逐次処理版。
   *
   * <p>{@code maxSamplesPerChunk} は監査対象総数の上限ではなく、同時に保持するチャンク幅である。各チャンクの平均 KL は方策学習の重み
   * で再集約するため、入力元全体の判定を保つ。
   *
   * @param model 監査対象 Decision モデル
   * @param fragmentPaths 読み込む対局単位の学習データの順序付きパス
   * @param reader 対局単位の学習データから選択したサンプルを読むコールバック
   * @param batchSize 一回の論理バッチ行数
   * @param maxSamplesPerChunk 同時にデータを復元する最大サンプル数
   * @param config 本番損失と同じ更新設定
   * @return 全対局単位の学習データを集約した監査結果
  ...
  /**
   * 判断機会別行動選択プレイヤー重みを本番更新と同じ定義で適用する逐次処理全件監査。
   *
   * @param model 監査対象 Decision モデル
   * @param fragmentPaths 読み込む対局単位の学習データの順序付きパス
   * @param reader 対局単位の学習データから選択したサンプルを読むコールバック
  ...
  /**
   * 行動選択プレイヤー重みとデバイス遷移-格納枠上限を本番更新と揃える逐次処理全件監査。
   *
   * @param model 監査対象 Decision モデル
   * @param fragmentPaths 読み込む対局単位の学習データの順序付きパス
   * @param reader 対局単位の学習データから選択したサンプルを読むコールバック
   ...
   * @param maximumObservedPolicyUpdateRatio 観測した重なり方策の更新比の最大値
