...
public class CompactReadoutTest {
  ...
  public void compactProjectionPreservesReadoutAndGradients(int rows, int width, boolean dense) {
    try (NDManager manager = NDManager.newBaseManager(Device.cpu())) {
      var readout = new EpsilonMahjongStateReadout(width, 128, 128);
  ...
  public Object[][] gpuLayouts() {
    return new Object[][] {{3, false}, {17, false}, {3, true}};
  }

  /** BF16 の重みと autocast を使い、有効行の順序変更による丸め誤差と勾配経路を検証する。 */
  @SuppressWarnings("try")
  @Test(groups = "rocm", dataProvider = "gpuLayouts")
  public void gpuBfloat16CompactProjectionPreservesReadoutAndGradients(int rows, boolean dense) {
    ...
    try (NDManager manager = engine.newBaseManager(device)) {
      ...
      Assert.assertTrue(
          gradientMagnitude > 0, "Detached value readout still learns its own weights");
      System.out.printf(
          "CompactReadout BF16 rows=%d dense=%s maxOutputAbs=%g maxInputGradientAbs=%g"
              + " maxParameterGradientAbs=%g%n",
          rows, dense, outputError, inputGradientError, parameterGradientError);
    }
  }
