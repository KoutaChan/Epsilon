 ...
 *
 * <p>候補・型・二択分岐の重みは実際の方策分布と共有し、上位損失から子ロジットへの直接勾配だけを遮断する。 独立した注意機構や反実仮想教師値は導入しない。
 */
public final class DecisionPolicyContextPool {
  ...
  /** 二択分岐の条件付き分布で二分岐を集約し、片側だけ合法ならそのコンテキストを維持する。 */
   ...
   *
   * <p>学習時は候補表現への勾配だけを維持し、方策スコアは従来どおり勾配を遮断する。ROCm推論時は種類ごとの マスク付きソフトマックス・積・集約を単一カーネルへまとめる。
   */
  ...
  /** 二分岐の存在量と選択された確率から、片側だけ存在する場合も質量を保つ勾配を遮断した重みを作る。 */
