...
/**
 * 学習の診断値をページロックしたバッファへ非同期転送し、逆伝播と並行して回収する。
 *
 * <p>損失の計算と逆伝播は学習処理が担当する。価値損失と模倣学習の損失はサンプル重みの合計、方策固有の指標は方策更新対象の重みの合計で集約する。
 */
final class DecisionTrainingMetrics {
  ...
  record Batch(
    ...
    /** 方策検証条件に必要なスカラーをページロックしたバッファへ非同期転送する。 */
  ...
  /** 逆伝播と重ねているページロックした指標転送。解放は未完了転送も安全に回収する。 */
  ...
  /** 詰めたスカラーを保持し、オプティマイザー確定より前の明示的有限値検査に使います。 */
