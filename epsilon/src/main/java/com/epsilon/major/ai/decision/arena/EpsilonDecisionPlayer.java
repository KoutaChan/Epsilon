...
import com.epsilon.major.ai.decision.data.EpsilonDecisionLegalActions;
...
public class EpsilonDecisionPlayer implements Player {
   ...
   * @param causalTraceLambda 価値/因果関係を持つ方策更新用の遷移列のスカラー値のトレース係数
  ...
  public static final class Builder {
    ...
    /**
     * 対局中の行動履歴に記録する収集開始時の学習側モデルのスナップショット ID を設定する。
     *
     ...
     *
     * @param fullSupport 探索適用後の方策に混ぜる探索設定
     * @return このビルダー
     */
    ...
    /**
     * 対局中の行動履歴の価値計算用と方策更新用の二つの遷移列スカラー値のトレース係数を設定する。
     *
     ...
     *
     * @param randomSeed 行動選択プレイヤー固有の乱数シード
     * @return このビルダー
     */
    ...
    /**
     * 選択方式、探索に割り当てる確率、スカラー値のトレース係数、探索分の学習への寄与混合率をまとめて設定する。
     *
    ...
    /**
     * 収集・更新単位境界で固定された不確実性CDFと収集診断用の集計を設定する。
     *
  ...
  /**
   * 半荘終了時の対局中の行動履歴を、データ本体を読み戻さずに学習サンプルビューへ変換する。
   *
   * <p>型付き入力 / behaviorPolicy / rolloutPolicy は処理中の一時保存領域から読まず、選択時に保存した合法手メタデータ
   * と軽量統計を使って遅延サンプルを作る。学習データ片書き込み処理はデータ本体ファイルテーブルとオフセット/長さ参照だけを保存する。
   */
   ...
   *
   * <p>重みを固定したGRPが利用可能で直後の局境界が存在する場合はその席周辺分布の期待効用を使い、それ以外は半荘終局効用へ戻す。
   */
