...
/**
 * CPU 上にある Decision 入力の行順、有効範囲、行動・遷移の対応関係を管理する。
 *
 * <p>入力記憶領域は{@link DecisionHostInputs}、学習教師値は{@link
 * DecisionTrainingTargets}へ分離する。この型は構築済み行の順序・有効範囲・行動/遷移連続性だけを管理し、特徴量 オフセットや教師値連結処理を所有しない。
 *
 * <p>{@link #copyRows}は独立所有複製、{@link #sliceRows}は入力元を強参照し、非同期処理の完了まで記憶領域寿命を保持する非所有ビューである。
 */
public final class DecisionHostBatch {
  ...
  /**
   * {@link DecisionInputSchema#fingerprint()}に一致する1行データ本体から復元する。
   *
  ...
  /**
   * 復号済み入力/教師値連続バッファの所有権を受け取り、複製せずに学習バッチを復元する。
   *
   * <p>呼出元は返却後に配列を読み書きしてはならない。永続化符号化・復号処理など、配列を新規確保して全要素を書き終えた境界だけで使用する。
   *
   * @param rowCount データ本体に含まれる行数
   * @param bucket データ本体の行動・遷移容量区分
   * @param inputCategories 所有権を渡す標準形式の入力カテゴリ値連続バッファ
   * @param inputNumerics 所有権を渡す標準形式の入力数値連続バッファ
   * @param targetCategories 所有権を渡す標準形式の教師値カテゴリ値連続バッファ
   * @param targetNumerics 所有権を渡す標準形式の教師値数値連続バッファ
   * @return 渡された四つの配列を直接所有する学習バッチ
   */
  public static DecisionHostBatch takeEncodedTrainingBatch(
      int rowCount,
      DecisionBucket bucket,
      short[] inputCategories,
      float[] inputNumerics,
      int[] targetCategories,
      float[] targetNumerics) {
    DecisionHostBatch batch =
        new DecisionHostBatch(
            DecisionHostInputs.takeEncoded(rowCount, bucket, inputCategories, inputNumerics),
            DecisionTrainingTargets.takeEncoded(
                rowCount, bucket, targetCategories, targetNumerics));
    for (int row = 0; row < rowCount; row++) {
      batch.commitActionRow(row);
    }
    return batch;
   ...
   *
   * <p>非同期推論パイプラインはこの型をCPU側の軽量記述情報として保持し、GPU 格納位置が空いた時点で各入力元から実行枠専用のページ固定の
   * バッファへ直接抽出する。元バッチの所有権は移さないため、呼び出し側は推論完了まで入力元を保持します。
   */
  public static final class RowBatch {
    ...
    /** プレイヤーの履歴表現インデックスだけを集約バッチ上の位置へ直して書き込みます。 */
    ...
    /** 全入力元からコンパクト推論入力を、集約配置上の最終位置へ直接書き込みます。 */
    void copyInferenceInputsTo(
        ShortBuffer categoricalDestination,
        FloatBuffer numericDestination,
        DecisionInferenceInputLayout layout) {
    ...
    /** 全入力元のプレイヤーの履歴表現、遷移、方策実行インデックスを集約順に書き込みます。 */
    void copyInferenceIndicesTo(IntBuffer destination, InferenceIndexLayout layout) {
      int start = destination.position();
      copyPlayerMemoryPresentIndicesTo(
  ...
  /** 推論用INT32本の連続バッファ内の三領域を、ホスト側の要素オフセットと件数で固定する。 */
