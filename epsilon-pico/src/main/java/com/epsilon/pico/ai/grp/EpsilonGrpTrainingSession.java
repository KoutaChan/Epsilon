...
public final class EpsilonGrpTrainingSession implements AutoCloseable {
   ...
   *
   * @param decisionCheckpointDir GRP 配下の保存先を持つ Decision チェックポイントの親ディレクトリ
   * @param device モデルを配置するデバイス
   * @return オプティマイザー状態を持つ学習可能なセッション
  ...
  /**
   * 行動選択プレイヤー用の推論専用セッションを開く。オプティマイザーと学習処理状態は確保しない。
   *
   * @param decisionCheckpointDir GRP 配下の保存先を持つ Decision チェックポイントの親ディレクトリ
   * @param device モデルを配置するデバイス
   * @return 固定したチェックポイントを読む推論-only セッション
   ...
   *
   * @return 推論。無効な状態セッションでは {@code null}
   */
  ...
  /**
   * 対局単位の学習データヘッダーに保存する、現在開いている固定した教師モデルの内容識別情報を返す。
   *
   * @return チェックポイント世代と推論保存物 SHA-256
   * @throws IOException enabled セッションが保存済みチェックポイント識別情報を持たない場合
   */
