 ...
 *
 * <p>合法行動が一つしかない行は選択問題ではないため、方策のtop-1、NLL、一様分布との比較、Decision種別件数、方策状態
 * モーメントの分母から除外する。価値指標には全行を含め、{@link ValidationMetrics#samples()}と{@link
...
public final class EpsilonDecisionPretrainValidator {
  ...
  public static ValidationMetrics evaluate(
    ...
    try (Model model =
            EpsilonDecisionCheckpointManager.loadForInference(
                ...
                config.bind(com.epsilon.nano.config.settings.DecisionInferenceSettings.class));
  ...
  /** 4順位分布の累積3成分と、実順位ワンホットの累積3成分との平均二乗誤差。 */
