...
import com.epsilon.pico.ai.decision.policy.DecisionPolicyIndexedAffineExecution;
import com.epsilon.pico.ai.decision.policy.EpsilonBinaryBranchGate;
...
/**
 * djl-rocm の INDEXED_AFFINE で、有効な位置だけを対象に方策の二択のスコアを計算する。
 *
 * <p>リーチ、鳴き、ロン、槓、九種九牌、ツモ和了をそれぞれ独立した計画で実行する。出力の最大行数と有効位置用の作業領域は別々に拡張し、有効位置がない場合は計画を生成せず0のテンソルを返す。
 */
public final class DecisionPolicyFusionIndexedAffineExecution
  ...
  private GateProgram createProgram(
    ...
    NDArray hiddenWeightValue = weight(hidden);
    NDArray hiddenBiasValue = bias(hidden);
    NDArray scoreWeightValue = weight(score);
    NDArray scoreBiasValue = bias(score);
    FusionRecipe.Constant hiddenWeight =
        builder.addConstant(
            "hiddenWeight",
            FusionRecipe.TensorSpec.fixed(dataType, hiddenWeightValue.getShape().getShape()));
    FusionRecipe.Constant hiddenBias =
        builder.addConstant(
            "hiddenBias",
            FusionRecipe.TensorSpec.fixed(dataType, hiddenBiasValue.getShape().getShape()));
    FusionRecipe.Constant scoreWeight =
        builder.addConstant(
            "scoreWeight",
            FusionRecipe.TensorSpec.fixed(dataType, scoreWeightValue.getShape().getShape()));
    FusionRecipe.Constant scoreBias =
        builder.addConstant(
            "scoreBias",
            FusionRecipe.TensorSpec.fixed(dataType, scoreBiasValue.getShape().getShape()));
    ...
    bindings
        .bind(hiddenWeight, hiddenWeightValue)
        .bind(hiddenBias, hiddenBiasValue)
        .bind(scoreWeight, scoreWeightValue)
        .bind(scoreBias, scoreBiasValue);
    return prepare(
        site,
        actionCapacity,
        batchCapacity,
  ...
  private static final class FusionForward extends Forward {
    ...
    /** 有効な位置がない適用箇所を実行済みにし、永続Fusion バッファを借用せずに完了させる。 */
  ...
  private static final class GateProgram implements AutoCloseable {
    ...
    /** 収容上限拡張後は新規投入を止め、最後の利用権返却時にネイティブ資源を解放する。 */
    private void retire() {
      retired = true;
