...
/** 合法手の配列位置と行動 ID の対応を検証し、必要な要素を取得する。 */
public final class EpsilonDecisionLegalActions {
  ...
  /**
   * 先頭の合法枠だけを残した行動 ID 配列を返す。
   *
   * @param legalActionIdBySlot 枠順の行動 ID 配列
   * @param legalActionCount 有効な先頭枠数
   * @return 有効枠だけを含む配列。元配列が既に同じ長さならその参照
   */
  ...
  /**
   * 有効要素のみの合法手配列から指定枠の安定行動 ID を取得する。
   *
   * @param legalActionIdBySlot 有効要素のみの枠順の行動 ID
   * @param slot 取得する 0始まりの枠
   * @return 指定枠の行動 ID
   */
