 ...
 *
 * <p>終局順位、局開始時点までの GRP 入力、GRP 4x4分布は対局ヘッダーへ一度だけ保存する。Decision行は境界インデックスと席判断
 * 通し番号で付随情報を参照し、旧形式の行ごとのGRP・終局順位重複は持たない。
 *
 * <p>型付き入力と方策分布は外部ファイルに保存する。学習データファイルに埋め込む形式は扱わず、現行形式の識別値だけを受理する。 旧学習データファイルは明示的に拒否する。
 */
final class EpsilonDecisionFragmentCodec {
  ...
  /** GRP 付随情報・サンプル本体・データ本体を読まず、学習器の選抜に必要な先頭部分だけを返す。 */
  ...
  /**
   * 指定した全体インデックスの軽量記述情報だけを復元する。
   *
   * <p>選ばれなかった行は配列を生成せずに読み飛ばし、選ばれた行も密な入力と方策のデータ本体は外部ファイルに残す。
   */
