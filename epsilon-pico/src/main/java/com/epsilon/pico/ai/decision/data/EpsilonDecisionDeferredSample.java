...
/**
 * 収集中の入力と確率分布を外部ファイルに残し、学習時に必要となった段階で読み戻すサンプル。
 *
 * <p>半荘終了後は教師値と軽量なメタデータだけを保持する。学習データには外部ファイルのパス、オフセット、長さを記録し、大きな入力の復号、再符号化、二重書き込みを避ける。外部ファイルは圧縮せず、確定した学習バッチを構築する際にだけ読み込む。
 */
final class EpsilonDecisionDeferredSample implements EpsilonDecisionSampleRecord {
  ...
  /** 対局単位の学習データ符号化・復号処理向けの検証済み詰めた合法手ビュー。 */
  ...
  /** 対局単位の学習データ/境界検証向けの共有読み取り専用のビュー。パッケージ外へ公開しない。 */
  ...
  /** 対局単位の学習データ読み取り処理向け。外部入力と確率分布の本体のI/O失敗を隠さず伝播する。 */
