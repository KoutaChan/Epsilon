...
/** 単一モデルまたはチェックポイントの複製から、学習・評価用の推論器を生成する。 */
public final class EpsilonDecisionEvaluatorFactory {
  ...
  /** 固定対戦評価用に方策のみモデルの複製とコンパクト最大確率の行動を選ぶ方式出力境界を開く。 */
  public static GreedyHandle openGreedyPolicyCheckpointEvaluator(
      Path checkpoint,
  ...
  /** 新しく生成した行動選択プレイヤーパラメーターを各デバイスが所有する独立した凍結モデルへ直接複製する。 */
  ...
  /** 読み込み処理所有権を受け取り、固定/cast完了後だけ呼び出し側へモデルの複製を移す。 */
  ...
  public static final class Handle implements AutoCloseable {
    ...
    /** 単一デバイス診断を行うサーバーを返し、非同期循環バッファなら物理GPU全体を先に処理完了まで回収します。 */
  ...
  /** 方策のみ評価器の資源所有権を保った対戦評価用コンパクト最大確率の行動を選ぶ方式参照。 */
