...
/**
 * 正規化済み最終的な合法手の確率分布に、方策グラフ節点単位の探索と合法手の確率下限を適用する純粋計算。
 *
 * <p>上位節点の探索に割り当てる確率は下位種類数から独立する。最大40個の合法手候補の位置は一つのビットマスクで扱い、推論ごとの {@code
 * List<Integer>}、ストリーム、マップを作らない。返す分布と正規化規則は学習時の方策グラフ階層と一致する。
 */
public final class EpsilonDecisionBehaviorPolicy {
  ...
  /**
   * 探索を加えず、丸め誤差によるゼロだけを正値に補正し、行動抽出に使う方策を返す。
   *
   * @param policyProbability モデルが返した正規化済み合法手の確率
   * @return 入力を複製し、探索前後で共有する分布の組
   */
  ...
  /**
   * 呼び出し側が所有権を渡した配列をそのまま共通形式の方策として使う。
   *
   * <p>配列は正値化・正規化され、返す探索前後の分布は同一の不変配列を共有する。
   *
   * @param ownedPolicyProbability この呼び出しへ所有権を移す合法手の確率配列
   * @return 探索前後で同じ配列を参照する分布の組
   */
  ...
  /**
   * 合法行動と節点別探索率に従って最終行動を選ぶための探索後の方策を構築する。
   *
   * @param policyProbability モデルが返した正規化済み合法手の確率
   * @param legalActions 確率配列の位置と同順の合法行動
   * @param config 方策グラフ節点別の探索に割り当てる確率
   * @return 探索を加えた分布と元のモデル分布の組
   */
  ...
  /**
   * 合法行動と節点別探索率へ判断固有の倍率を掛けて最終行動を選ぶための探索後の方策を構築する。
   *
   * <p>倍率は節点探索に割り当てる確率だけへ適用し、合法手の確率下限は固定する。
   */
  public static Mixture distribution(
  ...
  /**
   * 呼び出し側が所有権を渡した共通形式の方策へFULL_SUPPORTを適用する。
   *
   * <p>入力配列を正値化・正規化して対局生成方策として再利用し、判断ごとの余分な配列複製を作らない。
   *
   * @param ownedPolicyProbability この呼び出しへ所有権を移す合法手の確率配列
   * @param legalActions 確率配列の位置と同順の合法行動
   * @param config 方策グラフ節点別の探索に割り当てる確率
   * @return 新しく作った探索用の分布と、所有権を受け取った元のモデル分布の組
   */
  ...
  /**
   * 呼び出し側が所有権を渡した共通形式の方策へ、判断固有倍率付きFULL_SUPPORTを適用する。
   *
   * @param explorationMultiplier 全方策グラフ節点の探索に割り当てる確率へ掛ける有限の非負倍率
   */
  public static Mixture distributionInPlace(
      float[] ownedPolicyProbability,
      List<Action> legalActions,
  ...
  /** 階層節点の条件付き確率が掛け合わさっても、各合法手を有限頻度で選択できる下限を付与する。 */
  ...
  /** 受理した側の条件付き確率。存在しない側へ確率質量を割り当てない。 */
