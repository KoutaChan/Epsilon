...
import com.epsilon.major.ai.model.fusion.EpsilonPlayerMemoryFusionExecution;
import com.epsilon.major.ai.model.fusion.EpsilonStrategicContextFusionExecution;
import com.epsilon.major.ai.model.fusion.EpsilonTileRelationFusionExecution;

/**
 * 固定長の麻雀構成要素集合を、共有された34牌種・相手別公開メモリへ変換するエンコーダー。
 *
 * <p>局、4人、34牌種、河イベント、面子をそれぞれトークンへ投影する。牌種・河・面子には同じ標準形式の牌識別情報を加え、 牌種トークンには {@link
 ...
 * が同じメモリを読む。価値側がこの共有メモリを勾配を遮断して使うことで、価値損失は共有エンコーダーを更新しない。
 */
public final class EpsilonMahjongStateEncoder extends AbstractBlock {

  /** 状態エンコーダーと特徴量の集約が共有する Attentionヘッド数。 */
   ...
   * 指定幅の構成要素エンコーダーを構築する。
   *
   * @param hiddenSize 全トークンと集約状態の埋め込み幅。Attentionヘッド数で割り切れる正数
   */
  public EpsilonMahjongStateEncoder(int hiddenSize) {
    ...
    playerMemoryKeyValueProjection =
        addChildBlock(
            ...
            Linear.builder()
                .setUnits(
                    2L * Math.min(hiddenSize, EpsilonDecisionArchitecture.POLICY_CONTEXT_WIDTH))
                .optBias(false)
                .build());
    publicHistoryEncoder =
        addChildBlock("publicHistoryEncoder", new EpsilonPublicHistoryEncoder(hiddenSize));
    strategicContextEncoder =
        addChildBlock("strategicContextEncoder", new EpsilonStrategicContextEncoder(hiddenSize));
  }

  /** 凍結済み戦略文脈の層の組を、このデバイスパイプライン専用の永続Fusion 実行計画へ結び付ける。 */
  ...
  /** 凍結済みプレイヤーの履歴表現エンコーダーを、このデバイスパイプライン専用の疎な永続Fusion 実行計画へ結び付ける。 */
  ...
  /** 凍結済み牌種間の関係層の組を、このデバイスパイプライン専用の永続Fusion 実行計画へ結び付ける。 */
   ...
   * @param training 子ブロックへ伝播する学習モード。学習固有の layer を追加した場合にもこの値を使う
  ...
  /**
   * 標準形式の状態連続バッファとコンパクトプレイヤーの履歴表現インデックスから共有局・プレイヤー・牌などの特徴表現を作る。
   *
   * @param parameterStore パラメーター取得先
   * @param stateCategories {@code [batch, STATE_INT_COUNT]} カテゴリ値状態テンソル
   * @param stateNumerics {@code [batch, STATE_FLOAT_COUNT]} 数値状態テンソル
   * @param playerMemoryPresentIndices 存在するプレイヤーの履歴表現行のコンパクトインデックス。未指定ならマスクから求める
  ...
  private EncodedMemory encodeMemoryInternal(
    ...
    NDArray categoricalFeatureValues =
        stateCategories
            .get(":,0:{}", DecisionInputSchema.PUBLIC_HISTORY_RELATION_OFFSET)
            .stopGradient();
    ...
    NDArray publicTokens = publicHistoryTokens(playerMemory, hiddenSize);
    NDArray publicKeyValues =
        applyLinear(
            ...
            runtimeParameters);
    NDArray publicMask =
        NDArrays.concat(new NDList(riverEventPresentMask, meldPresentMask), 1).stopGradient();
    NDArray relationCodes =
        stateCategories
            .get(":,{}:", DecisionInputSchema.PUBLIC_HISTORY_RELATION_OFFSET)
            .reshape(
                rowCount,
                DecisionPublicHistory.QUERY_COUNT,
                DecisionPublicHistory.TOKEN_COUNT,
                DecisionPublicHistory.PACKED_WORDS);
    NDArray publicSummaries =
        publicHistoryEncoder.encode(
            parameterStore,
            playerMemory.get(":,:,0,:"),
            publicKeyValues,
            publicMask,
            ...
            training,
            runtimeParameters);
    playerMemory = replacePlayerSummaries(playerMemory, publicSummaries, training);
    ...
    int contextWidth = Math.min(hiddenSize, EpsilonDecisionArchitecture.POLICY_CONTEXT_WIDTH);
    NDArray summaryKeyValues =
        applyLinear(
            playerMemoryKeyValueProjection,
            parameterStore,
            ...
            training,
            runtimeParameters);
    NDArray playerMemoryKeyValues =
        NDArrays.concat(
            new NDList(
                summaryKeyValues.expandDims(2),
                publicKeyValues
                    .get(":,0:{},:", RIVER_TOKEN_COUNT)
                    .reshape(
                        rowCount,
                        GameState.NUM_PLAYERS,
                        DecisionInputSchema.MAX_RIVER_EVENTS_PER_PLAYER,
    ...
    // 河・面子は家ごとのビューを直接連結し、4家を畳む中間テンソルを作らない。
    ...
    return new EncodedMemory(
        ...
        tileProjectionEmbeddings,
        playerMemoryMask,
        playerMemoryKeyValues);
  }

  /** 標準形式の入力と同じ河96・面子16の順序で、局所文脈化済み公開トークンを並べる。 */
  private static NDArray publicHistoryTokens(NDArray playerMemory, int width) {
    long rows = playerMemory.getShape().get(0);
    int meldStart = 1 + DecisionInputSchema.MAX_RIVER_EVENTS_PER_PLAYER;
    return NDArrays.concat(
        new NDList(
            playerMemory.get(":,:,1:{},:", meldStart).reshape(rows, RIVER_TOKEN_COUNT, width),
            playerMemory.get(":,:,{}:,:", meldStart).reshape(rows, MELD_TOKEN_COUNT, width)),
        1);
  }

  /**
   * 戦略文脈の層の組が更新した4家の要約だけをプレイヤーごとの履歴表現へ戻す。
   *
   * <p>推論時のプレイヤーごとの履歴表現は当該順伝播が所有する出力なので、先頭トークンだけを上書きし、残り28 トークンを
   * 再連結しない。学習時はautogradが参照する値を破壊しないよう、従来どおり新しいテンソルを構成する。
   */
  ...
  /** 各家の要約トークンを必ず有効にし、その後ろへ河・面子の存在マスクを並べる。 */
  ...
  /**
   * 未加工の状態カテゴリへフィールド名前空間オフセットを加え、共有テーブルを中間ID テンソルなしで参照する。
   *
  ...
  /** GPU凍結推論で埋め込みテーブルと数値入力のデータ型が一致するときだけ構成要素特徴量を直接連結する。 */
  ...
  protected void initializeChildBlocks(NDManager manager, DataType dataType, Shape... inputShapes) {
    stateCategoryOffsets =
        manager
            ...
            .reshape(1, DecisionInputSchema.PUBLIC_HISTORY_RELATION_OFFSET);
    stateFeatureEmbedding.initialize(
        manager, dataType, new Shape(-1, DecisionInputSchema.PUBLIC_HISTORY_RELATION_OFFSET));
    ...
    publicHistoryEncoder.initialize(
        manager, dataType, new Shape(-1, DecisionPublicHistory.QUERY_COUNT, hiddenSize));
  ...
  public Shape[] getOutputShapes(Shape[] inputShapes) {
    return new Shape[] {
      ...
      new Shape(
          -1,
          GameState.NUM_PLAYERS,
          PLAYER_MEMORY_TOKEN_COUNT,
          2 * Math.min(hiddenSize, EpsilonDecisionArchitecture.POLICY_CONTEXT_WIDTH))
    };
  }
  ...
  /**
   * 推論用に所有している射影出力へ標準形式の牌識別情報を直接加え、パディングトークンを同時に0化する。
   *
   * <p>参照、PAD除外、面子の有効牌平均、存在するマスクを一つのエンジン演算へ渡す。学習時はautograd境界を変えないため、 {@link
   * #embedCanonicalTiles(ParameterStore, NDArray, boolean, PairList)} のeager式を使う。
   */
  ...
  /** 固定34牌種トークンへ、パディング行を除く標準形式の埋め込みテーブルをビューのまま加える。 */
   ...
   * @param tileEmbeddings 34牌種それぞれの文脈を反映した表現。形状は {@code [batch, 34, hidden]}
   * @param tileProjectionEmbeddings 牌種表現をLinearへ渡す低精度ビュー。学習時とCPU推論では {@code tileEmbeddings} と同じ参照
   * @param playerMemoryMask 各家の要約・河・面子の有効トークンマスク。形状は {@code [batch, 4, playerMemory]}
   * @param playerMemoryKeyValues 方策が再利用するK/V。形状は {@code [batch, 4, playerMemory, 2C]}
   */
  public record EncodedMemory(
    ...
    EncodedMemory stopGradient() {
      ...
      return new EncodedMemory(
          ...
          playerMemoryMask,
          playerMemoryKeyValues.stopGradient());
    }

    /**
     * 目的固有の集約表現を付与して方策ヘッド入力を作る。
     *
   ...
   * @param tileProjectionEmbeddings 牌種表現をLinearへ渡す低精度ビュー。学習時とCPU推論では {@code tileEmbeddings} と同じ参照
   * @param playerMemoryMask 各家の要約・河・面子の有効トークンマスク。形状は {@code [batch, 4, playerMemory]}
   * @param playerMemoryKeyValues 共有エンコーダーで射影済みのK/V。形状は {@code [batch, 4, playerMemory, 2C]}
   */
