...
/**
 * 未正規化スコアを、麻雀の条件付き意思決定構造に沿った合法最終行動確率へ合成する。
 *
 * <p>応答局面はRON/DECLINE_RON、PASS/MELD、MELD 種類、具体候補の順に分解する。自摸局面は
 * TSUMO/DECLINE、九種九牌/CONTINUE、CONTINUE/KAN、KAN 種類、打牌識別情報、RIICHI/DAMAの順に
 * 分解する。CALLとKANの総確率は専用の二択分岐だけで決まり、合法なCHI 形数やKAN 種類数を増減しても上位分岐の確率は変化しない。
 *
 * <p>最終行動の対数確率は到達経路上の条件付き対数確率の和である。選択行動損失は
 * 選択最終行動が通過した節点だけへ勾配を流し、未選択分岐へ反実仮想ラベルを与えない。パディング位置の確率は0である。
 */
public final class EpsilonDecisionPolicyGraph {
  ...
  /**
   * 採点器出力と符号化済み行動メタデータから最終行動分布を作る。
   *
   * @param scores バイナリ分岐、種類、具体候補、RIICHI/DAMA 二択分岐の未正規化スコア
   * @param actionCategories {@code [batch, legalActionCapacity, ACTION_INT_STRIDE]} の行動 メタデータ
   * @param actionRoutes 同じ物理打牌の代表位置と RIICHI/DAMA 位置を指す経路選択メタデータ
   * @return 各行の合法位置上で総和1となる最終行動分布
   */
  ...
  /**
   * 推論で不要な確率テンソルを作らず、最終行動の対数確率だけを返す。
   *
   * @param scores バイナリ分岐、種類、候補、RIICHI・DAMA 二択分岐の未正規化スコア
   * @param actionCategories 行動種類・グループを含むカテゴリ値テンソル
   * @param actionRoutes 同一打牌と RIICHI・DAMA 対応を持つ経路選択テンソル
   * @return 合法最終行動上で正規化した対数確率
   */
  ...
  private static NDArray assignResponseBranch(
    ...
    NDArray[] meldTypeMasks =
        typeMasks(actionTypes, branch.unresolvedCandidateMask(), DecisionPolicyNode.meldTypes());
    ...
    return assignTypedCandidates(
        assigned, scores, meldTypeMasks, DecisionPolicyNode.meldTypes(), callMass, rowCount);
  }
  ...
  private static PolicyBranch splitKanBranch(
    ...
    NDArray[] kanTypeMasks =
        typeMasks(actionTypes, branch.unresolvedCandidateMask(), DecisionPolicyNode.kanTypes());
