...
import com.epsilon.major.ai.decision.policy.DecisionPolicyCandidatePrefixExecution;
...
/**
 * 候補アフィン変換の先頭構成部分をdjl-rocmの型付き OUTPUT_PACKで直接連結する。
 *
 * <p>四つの候補別構成部分は{@code [rows * actions, width]} ビューとして一つの再利用するバッファへ書き、同時に射影
 * 重みのデータ型へ変換する。後段は学習ブロックと同じ一回のrocBLAS GEMMを使うため、AFFINE_SUMのように 構成部分ごとのGEMMへ分割しない。候補全体の{@code
 * cat}とautocast 複製だけを一つのHIP 起動へ置き換える。
 *
 * <p>一実行枠の最大永続出力は {@code maxBatch * MAX_LEGAL_ACTIONS * prefixWidth * dtypeBytes}
