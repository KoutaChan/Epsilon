 ...
 * @param slotsPerDevice 各デバイスが持つ再利用可能な実行枠数。枠ごとにページ固定の入出力とデバイス作業領域を持つためVRAM使用量を決める
 ...
 * @param acceleratorGraph GPU の計算グラフ記録を指定する互換性用の設定。未対応のため {@code false} のみ受け付ける
 * @param acceleratorGraphRows 計算グラフ記録用の予約設定。現在は計算グラフ記録が未対応のため使用しない
 * @param profileFile 推論プロファイルを書き出すファイル。空文字列なら無効
 * @param profileMinimumRows プロファイル集計へ含める最小バッチ行数
 * @param profileAfterCalls プロファイル開始前にウォームアップとして除外する順伝播回数
 * @param profileTransitionCapacity プロファイル対象を絞る遷移容量。{@code 0} は制限なし
 */
