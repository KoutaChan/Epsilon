 ...
 *
 * <p>学習ブロックとパラメーターは所有せず、重みを固定したパラメーターを参照する。投入スレッドは順伝播を直列に開始でき、位置ごとの{@link Forward}
 * を対応する詰めたスコアのホスト転送が完了するまで保持する。
 */
public abstract class DecisionPolicyAffineExecution implements AutoCloseable {
   ...
   * 一つの方策順伝播を開始する。
   *
   * @param workingManager 最終詰めたスコアのデバイス完了まで中間テンソルを所有するバッチ管理元
  ...
  /** この実行単位を利用不能にした最初の失敗を返す。正常なら{@code null}を返す。 */
  public abstract Throwable failure();

  /**
   * 呼び出し側が同じストリーム末尾の完了を証明した後、失敗順伝播が保持した利用権を解放する。
   *
   * <p>実行単位の再利用不可に設定状態は解除せず、資源解放だけを可能にする。
   */
   ...
   *
   * <p>{@link #seal()}後は詰めたスコア側へ所有権を渡す。正常時の{@link #close()}は最終D2H完了後にだけ呼ばれる。
   */
  public abstract static class Forward implements AutoCloseable {
    ...
    /** アフィン変換出力の所有権を最終詰めたスコアへ渡せる状態に固定する。 */
    public abstract AutoCloseable seal();

    /**
     * 最終出力の完了証明へ到達できなかった失敗を記録する。
     *
     * <p>Fusion実装は既に投入した利用権を解放せず実行単位を再利用できない状態にする。EAGER実装は保持資源を持たない。
     */
  ...
  /** 状態と候補を射影し、分岐固有の学習可能なオフセットを加えてからSiLUを適用する。 */
  static NDArray alternativeContexts(
      Linear alternativeHidden,
      Parameter alternativeOffsets,
      ParameterStore parameterStore,
      boolean training,
    ...
    NDArray offsets =
        parameterStore.getValue(alternativeOffsets, stateContext.getDevice(), training);
    if (offsets.getDataType() != projected.getDataType()) {
      offsets = offsets.getNDArrayInternal().differentiableCast(projected.getDataType());
      offsets.attach(projected.getManager());
    }
    return EpsilonMahjongStateEncoder.silu(projected.add(offsets));
  }

  /** 重みを固定したモデルの推論の遷移別入力元を必要な要素数だけのバッファへ詰める処理し、二つのGEMMで射影する。 */
