...
/**
 * 方策グラフの条件付き子分布から上位分岐のコンテキストを構成する。
 *
 * <p>候補・型・二択の判定の重みは実際の方策分布と共有し、上位損失から子ロジットへの直接勾配だけを遮断する。 独立した注意機構や反実仮想教師値は導入しない。
 */
public final class DecisionPolicyContextPool {
  ...
  /** 二択の判定の条件付き分布で二分岐を集約し、片側だけ合法ならそのコンテキストを維持する。 */
   ...
   *
   * <p>学習時は候補表現への勾配だけを維持し、方策スコアは従来どおり勾配を切り離す。ROCm推論時は種類ごとの マスク付きソフトマックス・積・reductionを単一カーネルへまとめる。
   */
  ...
  /** 種類一括再利用領域をネットワーク列順へ並べ替え、PASSを独立した連続集合として返す。 */
  ...
  /** DAMA/RIICHIへ同じ物理牌の打牌の識別情報スコアを共有する。 */
  ...
  /** ネットワーク列順の選択肢集合と、独立したPASS 集合を保持する候補文脈。 */
