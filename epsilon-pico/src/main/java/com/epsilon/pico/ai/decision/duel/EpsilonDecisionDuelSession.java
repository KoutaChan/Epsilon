 ...
 *
 * <p>価値を評価しないことと、段階評価の間でチェックポイント複製モデルを再読込しないことをこの境界で保証する。対局の乱数シード範囲とスケジューラーは各{@link #evaluate(int,
...
public final class EpsilonDecisionDuelSession implements DuelEvaluationSource, AutoCloseable {
  ...
  /**
   * 候補と対戦相手を方策のみ複製モデルとして一度ずつ開く。
   *
   * @param candidateCheckpoint 候補チェックポイントまたはその起点
   * @param opponentCheckpoint 重みを固定した対戦相手チェックポイントまたはその起点
  ...
  /**
   * 全席順を入れ替えた対局を保持せず、完了した牌山を逐次渡しながら一つの連続対局実行環境で評価する。
   *
