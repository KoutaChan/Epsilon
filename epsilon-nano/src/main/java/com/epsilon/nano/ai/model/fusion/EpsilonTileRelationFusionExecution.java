 ...
 *
 * <p>モデルブロックを参照せず、構築時に受け取った名前付きのパラメータービューだけを定数へ関連付ける。呼び出し元が所有する入力は借用し、
 * 出力利用権は後段のデバイス処理が終わるまで枠単位で保持する。
 */
public final class EpsilonTileRelationFusionExecution implements AutoCloseable {
  ...
  /**
   * 凍結済みパラメーター参照を関連付け、牌関係Transformerの永続実行計画を作る。
   *
   ...
   * @return パイプライン専用の牌-関係 Fusion実行境界
   */
  ...
  /** 空き循環バッファ枠を予約する。入力NDArrayの管理元所有権は移譲しない。 */
  ...
  /** 一つのネットワーク順伝播が保持する実行枠のバッファに保持する牌関係出力。 */
  public static final class Forward {
    ...
    /** 借用した入力を一度だけ投入し、有効なバッチに切った利用権付きの出力を返す。 */
  ...
  /** 一つのTransformer ブロックを構成する名前付き NDArray参照。 */
  ...
  /** ブロック所有のNDArray参照を複製せず定数関連付ける順序へ記録する。 */
