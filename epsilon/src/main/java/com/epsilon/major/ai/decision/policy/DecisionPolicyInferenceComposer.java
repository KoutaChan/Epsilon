...
/**
 * 推論用の未正規化スコアを、方策グラフと同じ階層に従ってCPU上で最終行動対数確率へ合成する。
 *
 ...
 * 採点器の出力だけを一度同期し、RON/TSUMO、CALL/KAN、種類、具体候補、DAMA/RIICHIの順に同じ確率を合成する。 ネットワーク パラメーターや入力表現は変更しない。
 *
...
public final class DecisionPolicyInferenceComposer {
   ...
   * 一行あたりの同期スコア数を返す。
   *
   * @param legalActionCapacity まとめた行内の候補位置数
   * @param includesValue 末尾へ復号済み期待効用を含めるなら {@code true}
   * @return まとめた行のfloat要素数
   */
  ...
  /**
   * まとめた採点器出力を、入力部分領域と同じ行順の最終行動対数確率へ合成する。
   *
   * @param packedScores {@link #packedScoreWidth}の行を連結した採点器出力
   * @param hostRows 採点器へ渡したホスト行部分領域
   * @param includesValue まとめた行末尾に価値ロジットを含む場合true
  ...
  /** 複数の非所有行ビューを、まとめた行順のまま一つの対数確率配列へ合成します。 */
  public static float[] composeLogProbabilities(
  ...
  /**
   * まとめた採点器出力を、入力部分領域と同じ行順の最終行動確率へ合成する。
   *
   * <p>各二択分岐と条件付きsoftmaxを確率空間で一度だけ合成する。自己対局はこの配列をそのまま無作為抽出へ使い、 ログ 確率へ変換して直後に再度expする往復を行わない。
   *
   * @param packedScores {@link #packedScoreWidth}の行を連結した採点器出力
   * @param hostRows 採点器へ渡したホスト行部分領域
   * @param includesValue まとめた行末尾に価値ロジットを含む場合true
  ...
  /** 複数の非所有行ビューを中間スコア配列へ分割せず、一つの最終行動確率配列へ合成します。 */
  public static float[] composeProbabilities(
      float[] packedScores, DecisionHostBatch.RowBatch hostRows, boolean includesValue) {
    int actionCapacity = hostRows.bucket().legalActionCapacity();
    int packedWidth = packedScoreWidth(actionCapacity, includesValue);
  ...
  /** 複数の非所有行ビューについて、全候補配列を作らずまとめた行からargmax 位置を返します。 */
  public static int[] composeGreedyActionSlots(
      float[] packedScores, DecisionHostBatch.RowBatch hostRows, boolean includesValue) {
    int actionCapacity = hostRows.bucket().legalActionCapacity();
    int packedWidth = packedScoreWidth(actionCapacity, includesValue);
