 ...
 * DecisionTrainingDeviceTransferSchedule#ON_DEMAND}では確定済みホスト バッチを返し、演算担当の学習ワーカーが取得した時点で転送する。{@link
 * DecisionTrainingDeviceTransferSchedule#OVERLAPPED}では復号直後に学習ワーカー専用ストリームへ
 * H2D転送をキューへ追加する。呼び出し側のAPIは両方式で同一である。
 *
 * <p>ページ固定のホスト位置は順伝播終了ではなくH2D 完了受付記録で回収する。デバイス側バッチは独立した子管理元へ所有権を 移すため、ホスト位置を次バッチの復号へ先に再利用できる。
 */
final class DecisionTrainingInputPipeline implements AutoCloseable {
  ...
  /**
   * H2D完了済み受付記録を回収し、現在復号へ再利用できるホスト位置数を返す。
   *
   * <p>デバイス利用権の寿命とは独立したホスト回収をテストと診断用の集計から確認するための学習ワーカーごとの値である。
   */
   ...
   * @return 復号と任意の先行H2Dを経て準備済みバッチで完了する完了通知
   */
   ...
   * 準備済みバッチを現在の演算ストリームへ一度だけ取得する。
   *
   * <p>OVERLAPPEDでは転送イベントのGPU-側 waitだけを現在のストリームへ追加する。ホストスレッドは複製完了を待たない。
   *
   * @param ready このパイプラインが返した準備済みバッチ
  ...
  /**
   * ワーカーキューを処理完了まで回収した後、同じワーカー上でキャッシュしたデータ本体チャネルを閉じる。
   *
   * <p>収集・更新単位境界で呼ぶと、削除済みデータ本体ファイルのinodeを次の収集・更新単位まで保持しない。
   */
  ...
  /**
   * ワーカー、準備済みバッチ、H2D 受付記録、ホスト記憶領域の順で学習ワーカー資源を閉じる。
   *
  ...
  /** 二つのホスト位置の一つ。状態遷移は外側パイプラインが直列化する。 */
  ...
  /** 一種類の再利用可能なページ固定バッファまたはホスト側のダイレクトバッファ。 */
  ...
  /** H2D 受付記録、未加工のデバイス連続バッファ、ホスト位置回収をまとめる内部所有権。 */
