 ...
 * 指定位置の要素を集めるで広いインデックスが必要な箇所だけがモデル内で局所的に拡張するため、転送バイト数とデバイス側記憶領域の両方を半減できる。
 *
 * <p>{@link Workspace}を使う経路は実行枠ごとのページ固定のまたは直接参照のバッファを再利用し、{@link
 * DecisionHostBatch.RowSlice}を一時バッチへデータを復元せずに転送する。返却テンソルの寿命は転送先管理元に従う。
 * 推論時のプレイヤーごとの履歴表現、遷移、方策実行インデックスは一つのINT32 連続バッファへ詰め、デバイス上のビューとして関連付ける。
 */
public final class DecisionBatchTransfer {
  ...
  /**
   * 完全なホスト側バッチを直接参照の-バッファ経路で転送する簡易API。
   *
   ...
   *
   * @param manager 転送先デバイスを所有する管理元
   * @param hostRows 容量まで確定済みバッチの連続行部分ビュー
   * @param workspace 実行枠専用の再利用バッファ。{@code null} なら一時バッファを作る
   * @param transferMode ページ固定のまたは直接参照のバッファ転送方式
   * @return 論理ビューと任意の学習教師値を所有するデバイス側バッチ
   */
  public static DecisionDeviceBatch transferTrainingToDevice(
      NDManager manager,
  ...
  /** 完全なホスト側バッチをホストで有効な要素だけに集約した逐次実行用の推論入力としてデバイスへ転送する。 */
  ...
  /** 数値連続バッファを指定精度へ一度だけ変換するホストで有効な要素だけに集約した入力の逐次推論用の転送。 */
  public static DecisionInferenceDeviceBatch transferInferenceToDevice(
      NDManager manager,
      DecisionHostBatch.RowSlice hostRows,
      Workspace workspace,
  ...
  /**
   * 枠専用ページ固定のバッファへ複数の非所有行ビューを直接指定位置の要素を集めた状態です。
   *
   * <p>記述情報はバッファを所有しません。同じ{@link Workspace}で次の準備処理を始める前に、H2D イベントの完了を保証する必要があります。
   */
  public static final class StagedInference {
    ...
    /** 指定位置の要素を集める済みの合計行数を返します。 */
    public int rows() {
      return rows.size();
    }

    /** 指定位置の要素を集める済み入力の型付き容量区分を返します。 */
  ...
  /** 行動が一意な価値推論用に状態入力だけを指定位置の要素を集めた状態です。 */
  ...
  /**
   * 固定形状の GPU 計算グラフが再利用する入力記憶領域を作る。
   *
   * <p>ホスト側のページ固定のバッファとデバイス側の二本の元の連続バッファは計算グラフの寿命中同じアドレスを保つ。各再生前に{@link
   * GraphInput#refresh(DecisionHostBatch.RowSlice)}で内容だけを更新し、数値精度変換だけを記録対象にする。
   *
   * @param manager 計算グラフを記録するPyTorch デバイス管理元
   * @param hostRows 計算グラフの固定形状を決める、容量まで確定済み行部分ビュー
   * @param workspace 計算グラフの寿命中専有する再利用バッファ
   * @param inputNumericType 記録内でモデルへ渡す数値データ種類
   * @return アドレスを固定したの計算グラフ入力
   */
   ...
   *
   * <p>行動が一意に決まる局面の価値推論では行動/遷移連続バッファを転送せず、方策ネットワークを型として呼び出せない {@link DecisionStateInputs}を返す。
   *
   * @param manager 転送先デバイスを所有する管理元
   * @param hostRows 容量まで確定済みバッチの連続行部分ビュー
   * @param workspace 実行枠専用の再利用バッファ。{@code null} なら一時バッファを作る
  ...
  /**
   * 既にデバイスへ転送された共通形式の学習連続バッファを論理ビューへ関連付ける。
   *
   * <p>自己対局学習入力パイプラインはヒープ上の {@link DecisionHostBatch} を経由せず、この境界へ転送済み連続バッファを渡す。牌譜などの固定データによる
   * 事前学習の配列転送と論理的な配置を共有し、方策/価値側に別実装を作らない。
   *
   * @param rows バッチ行数
   * @param bucket 行動形状を固定する容量区分
   * @param inputCategories 共通形式の入力のカテゴリ連続バッファ
   * @param inputNumerics 共通形式の入力の数値連続バッファ。型変換時はこのメソッドが閉じる
   * @param playerMemoryPresentIndices プレイヤーごとの履歴表現の有効トークンインデックス
   ...
   * @param hasTrainingTargets 教師値連続バッファを論理的な教師値へ関連付けるか
   ...
   *
   * <p>行動、遷移、探索後の方策、対局生成方策、方策モデル教師値は転送しない。大きい合法候補容量区分でも転送量は 状態スキーマと行数だけで決まり、価値ネットワークの計算内容は完全な{@link
   * DecisionDeviceBatch}を渡す場合と同一になる。
   *
   * @param manager 転送先デバイスを所有する管理元
   * @param hostRows 学習教師値を持つ、容量まで確定済み行部分ビュー
   * @param workspace 実行枠専用の再利用バッファ。{@code null} なら一時バッファを作る
   * @param transferMode ページ固定のまたは直接参照のバッファ転送方式
   ...
   *
   * <p>同時利用を同期しない。ページ固定の複製は非同期なので、次の利用前に前回の順伝播を完了して前回の管理元を閉じる必要がある。 PyTorch GPU 以外では通常の管理元作成処理を使う。
   */
  public static final class Workspace implements AutoCloseable {
    ...
    /**
     * 複数の行ビューを枠専用ページ固定の入力へ直接指定位置の要素を集めます。
     *
     * @param rows 同じ容量区分に属する非所有行ビュー群
     * @return 後続のH2D キューへ追加に渡す軽量記述情報
     */
    public StagedInference stageInference(DecisionHostBatch.RowBatch rows) {
      ...
      try (var event =
          InputBatchProfile.beginStage(
              "nano", rows.bucket(), rows.size(), "inference.layout_count")) {
    ...
    /**
     * 現在のPyTorch ストリームへ指定位置の要素を集める済み入力のH2D 複製をキューへ追加します。
     *
    ...
    /** 価値のみ行を枠専用ページ固定の状態バッファへ直接指定位置の要素を集めます。 */
    public StagedState stageState(DecisionHostBatch.RowBatch rows) {
      ...
      try (var event =
          InputBatchProfile.beginStage("nano", rows.bucket(), rows.size(), "state.layout_count")) {
      ...
      try (var event =
          InputBatchProfile.beginStage(
              "nano", rows.bucket(), rows.size(), "state.buffer_capacity")) {
      ...
      try (var event =
          InputBatchProfile.beginStage("nano", rows.bucket(), rows.size(), "state.input_copy")) {
      ...
      try (var event =
          InputBatchProfile.beginStage("nano", rows.bucket(), rows.size(), "state.index_write")) {
    ...
    /** 現在のPyTorch ストリームへ指定位置の要素を集める済み状態入力のH2D 複製をキューへ追加します。 */
  ...
  /** アドレスを固定したのデバイス入力と、その内容を更新する専用ページ固定のバッファ。 */
  public static final class GraphInput {
     ...
     *
     * @param hostRows 記録時と同じ行数・容量区分を持つ確定済み部分ビュー
     */
    ...
    /**
     * 記録中に一度だけ実行し、モデルが読む共通形式のビューを構築する。
     *
  ...
  static final class PinnedInferenceInputSlabs implements AutoCloseable {
    ...
    private void stage(DecisionHostBatch.RowBatch hostRows, DecisionInferenceInputLayout layout) {
      try (var event =
          InputBatchProfile.beginStage(
              "nano", hostRows.bucket(), hostRows.size(), "inference.buffer_capacity")) {
        ensureCapacity(layout);
        if (event != null) {
          event.success = true;
        }
      ...
      try (var event =
          InputBatchProfile.beginStage(
              "nano", hostRows.bucket(), hostRows.size(), "inference.input_copy")) {
        hostRows.copyInferenceInputsTo(
            categoricalBuffer.getByteBuffer().order(ByteOrder.nativeOrder()).asShortBuffer(),
            numericBuffer.getByteBuffer().order(ByteOrder.nativeOrder()).asFloatBuffer(),
            layout);
  ...
  static final class PinnedInferenceIndexSlot implements AutoCloseable {
    ...
    private void stage(
      ...
      try (var event =
          InputBatchProfile.beginStage(
              "nano", hostRows.bucket(), hostRows.size(), "inference.index_write")) {
        hostRows.copyInferenceIndicesTo(
            buffer.getByteBuffer().order(ByteOrder.nativeOrder()).asIntBuffer(), layout);
        if (event != null) {
