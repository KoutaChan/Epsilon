...
/**
 * 入力と教師値を行順に書き込み、指定した容量を満たす Decision バッチを構築する。
 *
 * <p>推論の実遷移記憶領域と学習の固定長連続バッファを同じ入力エンコーダーへ接続し、各行を一度だけ確定する。ビルダーは宣言した容量を完全に埋めたときだけ {@link #build()}
 * でき、その後は再利用できない。学習行では探索後と探索前の方策、選択 格納位置、価値/アドバンテージ、方策学習/サンプル重みを入力と同じ容量区分順で格納する。
 */
public final class DecisionBatchBuilder {

  /** 並列エンコーダー実行処理が所有し、バッチをまたいで再利用する行符号化作業領域集約。 */
  ...
  /**
   * 合法手を伴わない行動適用後の状態の状態/価値表現用1行バッチを構築する。
   *
   * @param state 符号化する局状態
   * @param player 観測主体の席番号（0-3）
   * @return 行動軸を代替用 1 格納位置にした状態のみのバッチ
   */
  ...
  /** 対局実行処理の借用観測を一度だけ解析し、系列固有の入力行へ書き込みます。 */
  public void encodeObservedInferenceRow(
      int rowIndex, PublicObservation observation, List<Action> legalActions) {
    try (var session = openEncoding()) {
   ...
   * @param chosenSlot 実際に選択された候補格納位置
   * @param behaviorPolicy 候補格納位置順の探索適用後の確率
