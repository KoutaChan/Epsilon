 ...
 *
 * <p>同じバッチのプレイヤーごとの履歴表現、牌種間の関係、戦略文脈の、特徴量の集約、方策出力は同時に開始し、最終D2Hが完了した時だけ一緒に解放する。
 * これにより、後段カーネルがまだ参照している循環バッファ実行枠を次バッチが上書きしない。
 */
public final class DecisionInferenceExecution implements AutoCloseable {
  ...
  /** 同じストリーム末尾の完了確認後、失敗順伝播が保持した循環バッファ実行枠を解放する。 */
  ...
  public static final class Forward {
    ...
    /** 全循環バッファ実行枠を最終D2H完了まで保持する依存オブジェクトへ変換する。 */
