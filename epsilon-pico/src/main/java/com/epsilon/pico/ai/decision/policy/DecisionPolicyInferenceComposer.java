...
/**
 * 推論用の未正規化スコアから、学習時と同じ条件付き階層に従って最終行動の対数確率を CPU 上で計算する。
 *
 * <p>行動の情報はホスト側にあるため、GPU からはスコアだけを転送する。和了、鳴き・槓、種類、具体候補、ダマ・リーチの条件付き確率を合成する。
 *
 * <p>1行の転送形式は {@code [alternative(10), candidate(A), riichiGate(A), valueUtility(0 or
 * 1)]}。末尾の任意の1値はデコード済みの期待効用である。
 */
public final class DecisionPolicyInferenceComposer {
  ...
  private static final int[] MELD_TYPE_IDS = encodedTypes(DecisionAlternative.meldTypes());
  private static final int[] KAN_TYPE_IDS = encodedTypes(DecisionAlternative.kanTypes());
   ...
   * 一行あたりの同期スコア数を返す。
   *
   * @param legalActionCapacity 詰めた行内の候補枠数
   * @param includesValue 末尾へ復号済み期待効用を含めるなら {@code true}
   * @return 詰めた行のfloat要素数
   */
  ...
  /**
   * 詰めたスコア計算処理出力を、入力スライスと同じ行順の末端の行動対数確率へ合成する。
   *
   * @param packedScores {@link #packedScoreWidth}の行を連結したスコア計算処理出力
   * @param hostRows スコア計算処理へ渡したホスト行スライス
   * @param includesValue 詰めた行末尾に価値ロジットを含む場合true
  ...
  /** 複数の非所有行ビューを、詰めた行順のまま一つの対数確率配列へ合成します。 */
  public static float[] composeLogProbabilities(
  ...
  /**
   * 詰めたスコア計算処理出力を、入力スライスと同じ行順の末端の行動確率へ合成する。
   *
   * <p>各二択の判定と条件付きsoftmaxを確率空間で一度だけ合成する。自己対局はこの配列をそのまま無作為抽出へ使い、 ログ 確率へ変換して直後に再度expする往復を行わない。
   *
   * @param packedScores {@link #packedScoreWidth}の行を連結したスコア計算処理出力
   * @param hostRows スコア計算処理へ渡したホスト行スライス
   * @param includesValue 詰めた行末尾に価値ロジットを含む場合true
  ...
  /** 複数の非所有行ビューを中間スコア配列へ分割せず、一つの末端の行動確率配列へ合成します。 */
  public static float[] composeProbabilities(
      float[] packedScores, DecisionHostBatch.RowBatch hostRows, boolean includesValue) {
    int actionCapacity = hostRows.bucket().legalActionCapacity();
    int packedWidth = packedScoreWidth(actionCapacity, includesValue);
   ...
   * boolean)}と同じ演算順を使うため、同値確率では先頭位置を選ぶ規則も含めて完全出力のargmaxと一致する。
   *
   * @param packedScores {@link #packedScoreWidth}の行を連結したスコア計算処理出力
   * @param hostRows スコア計算処理へ渡したホスト行スライス
   * @param includesValue 詰めた行末尾に価値ロジットを含む場合true
  ...
  /** 複数の非所有行ビューについて、全候補配列を作らず詰めた行からargmax 位置を返します。 */
  public static int[] composeGreedyActionSlots(
      float[] packedScores, DecisionHostBatch.RowBatch hostRows, boolean includesValue) {
    int actionCapacity = hostRows.bucket().legalActionCapacity();
    int packedWidth = packedScoreWidth(actionCapacity, includesValue);
  ...
  private static void composeResponse(
    ...
    assignAlternativeTypes(
        ...
        outputOffset,
        chiSlots,
        DecisionAlternative.forActionType(DecisionAlternative.meldTypes()[0]),
        ponSlots,
        DecisionAlternative.forActionType(DecisionAlternative.meldTypes()[1]),
        daiminkanSlots,
        DecisionAlternative.forActionType(DecisionAlternative.meldTypes()[2]),
        callMass);
  }
  ...
  private static void composeTurn(
    ...
    if (kanCount != 0) {
      assignAlternativeTypes(
          ...
          outputOffset,
          ankanSlots,
          DecisionAlternative.forActionType(DecisionAlternative.kanTypes()[0]),
          kakanSlots,
          DecisionAlternative.forActionType(DecisionAlternative.kanTypes()[1]),
