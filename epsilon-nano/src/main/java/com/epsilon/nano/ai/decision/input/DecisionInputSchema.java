 ...
 *
 * <p>状態、合法行動、行動適用後に選べる遷移を固定間隔のカテゴリ値フィールドと数値フィールド
 * に分離する。各カテゴリ値フィールドは自身の正確なカテゴリ数だけを持ち、その先頭部分総和を名前空間オフセットとして単一埋め込み
 * 表へ渡す。値0は真偽値のfalseや数値カテゴリのゼロにも使い、可変長構成要素の未使用枠かどうかはIDまたはPRESENT
 * フィールドの0で判定する。合法候補数と一候補あたりの遷移数だけは宣言済み容量区分に丸め、未使用枠をゼロ初期値のまま保持する。
 *
...
public final class DecisionInputSchema {
  ...
  /** 可変長枠の未使用値。カテゴリ埋め込み上の実値とはフィールドごとの符号化・復号処理で分離する。 */
  public static final int PAD_ID = 0;

  /** エンジンが一判断で生成し得る最初の合法行動の最大数。 */
  ...
  /** 最初の行動適用後の、具体的かつ自己完結した遷移の種類。 */
  public enum ActionTransitionKind {
    /** PASSなど、手牌を変えずに最初の行動だけで完結する遷移。 */
    IDENTITY,
  ...
  /** 共有の打牌スコア計算処理へ、同じ物理打牌がどの判断から生じたかを伝える文脈。 */
  ...
  public enum RiverFloat {
    /** この席の河の中における捨て牌位置の進行度。 */
    PLAYER_PROGRESS,
    /** 全プレイヤーを通した捨て牌時系列の進行度。 */
    GLOBAL_PROGRESS
  }
  ...
  /** 最初の行動だけで確定し、後続打牌によって変わらないカテゴリ値フィールド。 */
  ...
  /** 最初の行動だけで確定する費用・和了・順位射影の数値フィールド。 */
  ...
  /**
   * 方策グラフ専用の行動候補の位置参照先の対応メタデータ。
   *
   * <p>値は埋め込みへ入れない。0は該当枠なし、それ以外は行動候補の位置インデックス + 1である。同じ物理打牌に属する
   * DAMA/RIICHIをネットワーク内で全候補同士比較せず、入力生成時に一度だけ解決して直接指定位置の要素を集める。
   */
  ...
  /** 最初の行動適用後の一つの具体遷移に付与するカテゴリ値フィールド。 */
  public enum ActionTransitionInt {
    /** 識別情報・打牌・処理待ちの・終局の遷移種別。 */
    KIND,
    /** 共有の打牌スコア計算処理へ渡す通常手番・CHI後・PON後の文脈。 */
    DISCARD_CONTEXT,
    /** 遷移内で実行する打牌の安定行動 ID。 */
    DISCARD_ACTION_ID,
    /** 遷移内で捨てる牌種。 */
    ...
    /** 行動後の手牌で七対子・国士などの特殊手経路が利用可能なら1。 */
    SPECIAL_HANDS_AVAILABLE,
    /** パディングでない実遷移なら1。 */
    PRESENT
  }

  /** 行動後の手牌全体のシャンテン・受入れ・ドラ・面子状態を保持する数値フィールド。 */
  ...
  /** RONまたはTSUMO一経路の全役を保持するために必要なチャンク数。 */
   ...
   *
   * <p>カテゴリ 配置はこのクラスの定数を参照するため、クラス初期化中には計算しない。遅延初期化用の内部クラスにより、どちらのクラスが先に参照されても完全に初期化された配置だけを使用する。
   */
