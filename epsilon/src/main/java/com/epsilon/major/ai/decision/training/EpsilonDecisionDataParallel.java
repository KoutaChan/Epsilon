...
final class EpsilonDecisionDataParallel
  ...
  /** 本番自己対局学習器用の学習ワーカー集合を、有効デバイス設定から構築する。 */
  ...
  /** FLOAT32・直接参照の転送の保存データによる事前学習用ワーカー集合を構築する。 */
  ...
  /** 保存データによる事前学習用ワーカー集合を、指定したホスト転送方式と実行パラメーター型で構築する。 */
  static EpsilonDecisionDataParallel openPretraining(
      Model canonicalModel,
  ...
  static final class TrainingInputSequence implements AutoCloseable {
    ...
    /** 次バッチを返し、直前バッチが解放した位置へ次の先読みバッチを補充する。 */
    ...
    /** 未使用の先読みバッチを完了まで待ち、ホスト位置と転送受付記録をパイプラインへ返す。 */
    @Override
