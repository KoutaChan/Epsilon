 ...
 *
 * <p>入力特徴量とは独立して、選択枠、探索後の/対局生成方策、価値効用、選択行動 アドバンテージ、方策モデル/サンプル重みを所有する。カテゴリ値と数値の2 連続バッファは変換済み
...
public final class DecisionTrainingTargets {
  ...
  /** Decision 価値、アドバンテージ、方策モデル/サンプル重みを合わせた行幅。 */
  ...
  /**
   * 復号済み共通形式の教師値連続バッファの所有権を受け取り、複製せずに記憶領域へ復元する。
   *
   * <p>呼出元は返却後に配列を読み書きしてはならない。
   *
   * @param capacity データ本体に含まれる行数
   * @param bucket データ本体のDecision 容量区分
   * @param categoricalSlab 所有権を渡す行ごとの選択枠
   * @param numericSlab 所有権を渡す共通形式の順の方策・価値・重み連続バッファ
   ...
   *
   * <p>返り値は内部配列である。通常の更新には{@link #writeRow}を使う。
   *
   * @return 行ごとの選択行動の位置配列
   */
   ...
   *
   * <p>返り値は内部配列である。通常の更新には型付きAPIを使う。
   *
   * @return 共通形式の順の内部float配列
   */
   ...
   * @param rolloutPolicy 対局生成または基準値として記録する最終的な行動の確率分布
  ...
  /** Decision 効用教師値と重みを行ごとの有効要素のみの表現へ書く。 */
