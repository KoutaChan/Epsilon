...
final class EpsilonPlayerMemoryEncoder extends AbstractBlock {
   ...
   * プレイヤーごとの公開メモリを文脈化する。
   *
   * @param playerTokens {@code [batch,4,29,hidden]} のプレイヤーごとのトークン列
  ...
  /**
   * 固定形状計算グラフ記録用の入力内容に依存しない処理。
   *
   * <p>パディングも同じ密な演算へ通すが、キーマスクと各残差後のマスクにより存在トークンの意味は通常経路と同じになる。 {@code
  ...
  /** 凍結済みプレイヤーごとの履歴表現重みを、Fusion実装へ渡す名前付きビューとして解決する。 */
