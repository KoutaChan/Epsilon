...
/** 計算済みの1つの状態遷移を、遷移入力用の連続バッファへ書き込む。 */
final class DecisionTransitionEncoder {
  ...
  static void encode(
    ...
    for (int waitSlot = 0; waitSlot < hand.waitCount(); waitSlot++) {
      ...
      encodeWaitScore(
          ...
          DecisionInputSchema.ActionTransitionWaitFloat.RON_NORMALIZED_FU,
          DecisionInputSchema.ActionTransitionWaitFloat.RON_NORMALIZED_BASE_POINTS,
          writer,
          actionSlot,
          transitionSlot,
          waitSlot);
      encodeWaitScore(
          ...
          DecisionInputSchema.ActionTransitionWaitFloat.TSUMO_NORMALIZED_FU,
          DecisionInputSchema.ActionTransitionWaitFloat.TSUMO_NORMALIZED_BASE_POINTS,
          writer,
          actionSlot,
          transitionSlot,
          waitSlot);
  ...
  private static void encodeWaitYaku(
      long yakuBits,
    ...
    for (int chunk = 0; chunk < DecisionInputSchema.WAIT_YAKU_CHUNKS_PER_WIN_TYPE; chunk++) {
      writer.waitYaku(
          ...
          transitionSlot,
          waitSlot,
          DecisionFeatureCodec.waitYakuFeature(winType, chunk),
          DecisionFeatureCodec.waitYakuChunk(yakuBits, chunk));
    }
  }

  private static void encodeWaitScore(
      int visibleHan,
      int fu,
      int basePoints,
      DecisionInputSchema.ActionTransitionWaitFloat hanFeature,
      DecisionInputSchema.ActionTransitionWaitFloat fuFeature,
      DecisionInputSchema.ActionTransitionWaitFloat basePointsFeature,
      DecisionInputWriter writer,
      int actionSlot,
      int transitionSlot,
      int waitSlot) {
    writer.waitScore(actionSlot, transitionSlot, waitSlot, hanFeature, visibleHan / HAN_NORMALIZER);
    writer.waitScore(actionSlot, transitionSlot, waitSlot, fuFeature, fu / FU_NORMALIZER);
    writer.waitScore(
        actionSlot,
        transitionSlot,
        waitSlot,
        basePointsFeature,
        basePoints / BASE_POINTS_NORMALIZER);
  }
