 ...
 *
 * <p>収集中のヒストグラムは次の学習サイクルでだけ利用する。したがってGPU完了順や収集処理スレッドの実行順は、現在の学習サイクルの行動選択へ影響しない。
 */
public final class DecisionAdaptiveExploration {
  ...
  /** 次の学習サイクルが参照するCDFをチェックポイント境界で固定する。 */
  ...
  /** 前学習サイクルの変更不可 CDFだけを参照する収集セッションを開始する。 */
  ...
  /** 当該学習サイクルの集計をレポートへ固定し、そのヒストグラムを次の学習サイクルのCDFへ切り替える。 */
  ...
  /** 収集失敗時は途中ヒストグラムを次の学習サイクルへ持ち越さない。 */
  ...
  /** 選択時と対局エンジン結果確定時を結ぶ、対局中の行動履歴一時利用の軽量観測値。 */
  record Observation(
  ...
  /** 一学習サイクル中に全行動選択プレイヤープレイヤーが共有する収集セッション。CDF自体は変更不可。 */
  public static final class MacroSession {
    ...
    /** PendingDecisionが保持する詰めたコードから種別/十分位別学習役割を記録する。 */
