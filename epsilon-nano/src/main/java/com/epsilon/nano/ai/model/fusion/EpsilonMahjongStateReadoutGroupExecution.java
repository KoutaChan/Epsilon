...
/**
 * 一つのデバイスパイプラインが所有する方策/価値特徴量の集約グループと、その出力循環バッファを管理する。
 *
 * <p>局・プレイヤー・牌などの特徴表現の先頭トークンをクエリの初期値としてネイティブ準備処理内で選ぶため、非連続ビューや中間複製を作らない。方策のみでは方策
 * 特徴量の集約一つ、方策モデルでは方策と価値を同じ投入で計算する。出力利用権は後段方策、価値、最終D2Hがすべて完了するまで保持する。
 */
public final class EpsilonMahjongStateReadoutGroupExecution implements AutoCloseable {
  ...
  // DJLの通常自動混合精度 LayerNormが返す型。後段射影の低精度データ型とは独立である。
  ...
  /**
   * 凍結済み特徴量の集約を関連付け、一つまたは二つの特徴量の集約を実行する永続実行計画を作る。
   *
   ...
   * @return パイプライン専用の状態-特徴量の集約 Fusion実行境界
   */
  ...
  /** 空き循環バッファ枠を予約し、一つのネットワーク順伝播を開始する。 */
