 ...
 *
 * <p>上位分岐と下位の条件付き選択を別節点として表す。とくにCALLとKANは、合法な型の数を直接softmaxへ
 * 入れず、基準分岐と集約済み分岐を比較する二値分岐である。ネットワークインデックスは節点埋め込みと チェックポイントで使う明示的な座標で、enum宣言順には依存しない。
 */
public enum DecisionPolicyNode {
  ...
  MELD_TYPE(4),
  /** 同じMELD 種類内の組み合わせ・赤牌種類を比較する節点。 */
  MELD_CANDIDATE(5),
  /** CONTINUEとKANを比較する手番分岐。 */
  KAN_GATE(6),
