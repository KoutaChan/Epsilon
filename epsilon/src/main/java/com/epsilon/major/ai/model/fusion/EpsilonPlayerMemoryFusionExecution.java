...
public final class EpsilonPlayerMemoryFusionExecution implements AutoCloseable {
  ...
  /**
   * 凍結済みパラメーターを同じデバイスへ結び付け、疎なプレイヤーごとの履歴表現エンコーダーの永続実行計画を作る。
   *
   * <p>存在するインデックスは既存のまとめたデバイス連続バッファ上のビューをそのまま結び付ける。バッチと存在する数を別の可変次元の長さとして扱うため、 インデックスの複製や再転送は発生しない。
   *
   ...
   * @return パイプライン-局所的なプレイヤーの履歴表現 Fusion実行境界
   */
  ...
  /** 空き実行枠を予約し、一つのプレイヤーの履歴表現順伝播を開始する。 */
  ...
  /** 一つのネットワーク順伝播が保持するプレイヤーの履歴表現出力利用権。 */
  public static final class Forward {
    ...
    /** 最終出力まで到達しなかった失敗を記録し、実行系を再利用不可に設定する。 */
