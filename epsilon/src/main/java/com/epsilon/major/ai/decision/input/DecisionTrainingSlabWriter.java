...
/**
 * 確定した学習用小バッチの転送先バッファへ、保存済みの入力と教師値を直接復号する。
 *
 * <p>保存形式、ヒープ/ページ固定の記憶領域、デバイス転送を分離するため、この型はプリミティブ型のバッファだけを扱う。入力は
 * テンソルごとにまとめた一行データ本体から、同じテンソルごとにまとめた複数行バッチへ領域単位で配置する。方策パディングは 行ごとに明示的に0で上書きし、再利用バッファに前バッチの末尾を残さない。
 */
public final class DecisionTrainingSlabWriter {
  ...
  /** 一行データ本体の二本の入力連続バッファを最終バッチの指定行へ配置する。 */
  ...
  /** 一行データ本体のカテゴリ値連続バッファを配置する。直後に同じ行の数値連続バッファを書く。 */
  ...
  /** 処理待ちのカテゴリ値行と同じ行の数値連続バッファを指定位置へ配置し、派生インデックスを確定する。 */
  ...
  /** メタデータ教師値とデータ本体内の二本の方策を、最終教師値連続バッファへ直接書く。 */
  ...
  /** 探索適用後の方策を先に最終教師値連続バッファへ書き、同じ符号化・復号処理一時バッファの再利用を可能にする。 */
  ...
  /** 対局生成方策とメタデータ教師値を書き、処理待ちの教師値行を確定する。 */
