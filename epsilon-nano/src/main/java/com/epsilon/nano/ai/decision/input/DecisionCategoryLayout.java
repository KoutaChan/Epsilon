...
/**
 * カテゴリ特徴量の値域と、埋め込み表で参照する範囲を定義する。
 *
 * <p>項目ごとに必要なカテゴリ数を確保し、その累積和を埋め込み表の開始位置とする。同じ意味の項目は、プレイヤーや牌などの位置によらず同じ埋め込みパラメータを参照する。
 *
 * <p>{@link DecisionInputWriter} を使う入力生成時に値域を検査し、隣の項目の範囲を誤って参照することを防ぐ。
 */
public final class DecisionCategoryLayout {
  ...
  /** 最初の行動・遷移フィールドが共有する候補埋め込み辞書の総要素数。 */
  ...
  /**
   * 1次元化した状態テンソルの各位置に加えるフィールド名前空間オフセットを返す。
   *
  ...
  /**
   * 最初の行動テンソルへ加えるフィールド名前空間オフセットを返す。
   *
  ...
  /**
   * 互換性識別子へ含める、フィールド順とカテゴリ数の共通形式の記述を返す。
   *
