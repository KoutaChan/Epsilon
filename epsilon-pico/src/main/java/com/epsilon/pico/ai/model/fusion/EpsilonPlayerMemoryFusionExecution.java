...
public final class EpsilonPlayerMemoryFusionExecution implements AutoCloseable {
  ...
  /**
   * 重みを固定したパラメーターを同じデバイスへ結び付け、疎なプレイヤーごとの履歴表現エンコーダーの永続実行計画を作る。
   *
   * <p>存在するインデックスは既存の詰めたデバイス連続バッファ上のビューをそのまま結び付ける。バッチと存在する数を別の可変次元の長さとして扱うため、 インデックスの複製や再転送は発生しない。
   *
   * @param manager パイプラインと同じGPUを持つ長寿命管理元
   * @param parameters 重みを固定したエンコーダーパラメーターの名前付きビュー
   ...
   * @return パイプライン-局所的なプレイヤー-メモリ Fusion実行境界
   */
  ...
  /** 一つのネットワーク順伝播が保持するプレイヤー-メモリ出力利用権。 */
  public static final class Forward {
    ...
    /** 論理的に連結したプレイヤーごとの履歴表現区間を、物理concatなしでエンコーダーへ渡す。 */
    ...
    /** 最終出力まで到達しなかった失敗を記録し、実行単位を再利用できない状態にする。 */
  ...
  /**
   * 重みを固定したプレイヤー-メモリパラメーターの名前付きビュー。
   *
   * <p>NDArrayの所有権は移さず、実行の寿命中に値を更新しない呼び出し契約とする。
   */
