...
/**
 * Decision の特徴量を、未使用値を区別できるカテゴリ ID へ変換する。
 *
 * <p>行動 ID、牌の関係、役を表すビット列など、1項目に格納する値の符号化と復号を行う。バッファ内の位置やテンソルの形状は扱わない。
 */
public final class DecisionFeatureCodec {
   ...
   *
   * <p>0はパディング専用。実値は行動後の副露に含まれない手牌の枚数、改善牌残り枚数、役の有無を問わず和了形になる待ち、RON役あり待ち、TSUMO役あり待ち、打牌、
   * 新設面子への使用、赤5和了牌の残存可能性の直積を使う。
   *
   * @param concealedCount 遷移後の副露に含まれない手牌にある同牌種枚数
   * @param ukeireRemainingCount 公開情報から算出した同牌種の残り受入れ枚数
   * @param shapeWait 同牌種で役の有無を問わず和了形になるなら {@code true}
   ...
   * @param calledIntoMeld 最初の行動で新しい面子へこの牌種を使うなら {@code true}
  ...
  /**
   * 遷移牌カテゴリから行動後の副露に含まれない手牌の枚数を復元する。
   *
  ...
  /**
   * 遷移牌カテゴリが役の有無を問わず和了形になる待ちを表すかを返す。
   *
   * @param storedCode {@link #transitionTile} が返したカテゴリ
   * @return 役の有無を問わず和了形になる待ちなら {@code true}
   */
   ...
   *
   * @param storedCode {@link #transitionTile} が返したカテゴリ
   * @return 最初の行動で面子へ使う牌種なら {@code true}
   */
  ...
  /**
   * 役ビット集合の指定チャンクを、パディング 0と衝突しないカテゴリへ変換する。
   *
   * @param yakuBits 成立可能役を表すビット集合
   * @param chunk 取り出すチャンクインデックス
   * @return チャンク値へ1を加えたカテゴリ
   */
  ...
  /**
   * 保存済みカテゴリから指定チャンクの役ビットを元の位置へ復元する。
   *
   * @param storedCode 0をパディングに予約したチャンクカテゴリ
   * @param chunk 復元先チャンクインデックス
   * @return 元のビット位置へずらす量した役ビット集合
   */
  ...
  /**
   * RON/TSUMOとチャンク番号を待ち役テンソル末尾次元へ写す。
   *
   * @param winType RONまたはTSUMO
   * @param chunk 役ビットチャンクインデックス
