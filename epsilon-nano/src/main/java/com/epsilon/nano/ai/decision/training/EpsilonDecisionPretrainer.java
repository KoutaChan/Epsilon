...
import com.epsilon.nano.ai.decision.input.DecisionValueDeviceBatch;
...
import com.epsilon.nano.config.settings.DecisionPretrainSettings;
 ...
 *
 * <p>方策/価値を各デバイスへ複製するモデル全体のデータ並列だけを使う。デバイス数は2の冪に限定せず、サンプル重みの総和で
 * 正規化した小バッチ勾配を共通形式の実行単位へ集約する。共有状態メモリは一度だけ順伝播し、価値損失はネットワーク内部で
 * 勾配を切り離された価値特徴量の集約以降にだけ流れる。オプティマイザーと保存対象チェックポイントは更新の基準となるモデルだけが所有し、対象パラメーターを
 * 連続バッファ上で一括更新してからモデルの複製へ同期する。
 *
 * <p>入力は{@link DecisionHostBatch}の型付き連続バッファであり、各実行単位は転送バッファを再利用する。利用GPU数はオプティマイザーバッチから
 * 一実行単位当たり十分な行数を保てる範囲で自動決定するため、2の冪にも方策/価値比にも依存しない。
 *
 * <p>合法行動が一つだけのバッチは方策教師情報を持たないものとして扱う。方策の順伝播/逆伝播を実行せず、独立した方策 オプティマイザーの更新回数、AdamW
 * のモーメント、重み減衰も進めない。価値は状態のみの経路で通常どおり学習する。
 *
 * <p>このクラスは一つの学習ループが逐次所有する。{@link #trainEpoch}、{@link #setLearningRateScale(float)}、{@link
 * #close()} を複数スレッドから同時に呼び出してはならない。内部実行処理は、同じパラメーター更新のデータ並列の学習ワーカーだけを並行実行する。
 */
public final class EpsilonDecisionPretrainer implements AutoCloseable {
  ...
  /**
   * プロセス設定と検出済み学習器デバイスから事前学習処理を開く。
   *
   * @param checkpointModel 更新対象の共通形式の Decision モデル。先頭学習器デバイス上に存在すること
   * @return 設定に応じてデバイス構成を構築した事前学習処理
   */
  ...
  private EpsilonDecisionPretrainer(
    ...
    this(
        ...
        devices,
        executionContext,
        com.epsilon.nano.config.settings.DecisionSettings.defaults());
  }
  ...
  public static EpsilonDecisionPretrainer open(
    ...
    return new EpsilonDecisionPretrainer(
        ...
        context,
        config.bind(com.epsilon.nano.config.settings.DecisionSettings.class));
  }
  ...
  public static EpsilonDecisionPretrainer open(
      ...
      com.epsilon.nano.config.settings.DecisionSettings optimizer) {
  ...
  private EpsilonDecisionPretrainer(
      ...
      NetworkDevices devices,
      DecisionExecutionContext executionContext,
      com.epsilon.nano.config.settings.DecisionSettings optimizerSettings) {
   ...
   *
   * <p>各バッチは学習教師値を持ち、設定されたオプティマイザーバッチ上限以下でなければならない。複数デバイスへの分割や末尾の不均等 小バッチは内部でサンプル重みの総和に応じて補正される。
   *
  ...
  /**
   * 学習済み共通形式のチェックポイントモデルを自己対局学習状態へ戻し、全実行単位資源を解放する。
   *
   * <p>事前学習中のパラメーター有効範囲は内部実装であり、呼び出し側へ持ち越さない。同期後のチェックポイントモデルは方策/価値の全パラメーターを
   * 学習可能へ戻すため、そのまま方策と価値を同時に更新する自己対局学習へ渡せる。
   */
  @Override
  public void close() {
    if (closed) {
  ...
  /** 一つの事前学習ワーカーで同じパラメーター更新中に連続実行するホスト部分ビュー。 */
   ...
   * @param behaviorCloningLoss 選択した行動負の対数尤度
   * @param valueLoss Decision の価値予測に対する HL-Gauss 損失
   * @param entropy 最終的な合法手の確率分布エントロピー
