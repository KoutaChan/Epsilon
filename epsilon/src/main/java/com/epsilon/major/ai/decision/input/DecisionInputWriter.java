...
/**
 * 一行の Decision 入力へ、意味ごとの特徴量を書き込むためのインターフェース。
 *
 * <p>元の連続バッファ、フィールド通し番号、物理オフセットを書き込み処理から隠す。値は標準形式のエンコーダーが生成済みであることを前提とし、頻繁に実行する処理では再検証しない。
 */
public final class DecisionInputWriter {
  ...
  /** 7個の4bit公開関係を2shortへ保存する。符号ビットもデータでありカテゴリ IDではない。 */
  void publicHistory(int query, int token, int packedRelations) {
    int offset =
        stateCategoryBase
    ...
    categoricalSlab[offset] = (short) packedRelations;
    categoricalSlab[offset + 1] = (short) (packedRelations >>> Short.SIZE);
  }
  ...
  /**
   * 遷移行動適用後の状態の一牌種分を連結済みカテゴリとして書き込む。
   *
   * @param actionSlot 親行動候補の位置
   * @param transitionSlot 行動内の遷移候補の位置
   * @param tileType 34牌種インデックス
