...
/** 同じ牌山で席順を入れ替え、候補モデルと重みを固定した対戦相手を比較する。 */
public final class EpsilonDecisionDuelEvaluator {
   ...
   *
   * @param candidateCheckpointDir 評価対象チェックポイントまたはその起点ディレクトリ
   * @param opponentCheckpointDir 固定対戦相手チェックポイントまたはその起点ディレクトリ
   * @param games 実行する総対局数。4席を入れ替えた対局を構成できる値
   * @param seedBase 牌山乱数シード列の先頭値
   * @param context ワークフローが所有するGPU実行資源
   * @return 対応をそろえた順位差と順位・得点指標を集約した対戦評価結果
   * @throws IOException チェックポイントの解決、読込、または対局実行環境実行に失敗した場合
   */
