...
/**
 * Decision ログ事前学習のデータ読み込みと実行計画設定です。
 *
 * <p>設定の優先順位は {@link SettingsLoader} に従う。起動時に確定した設定からレコードを生成し、繰り返し実行する処理にはその参照を渡す。
 *
 * @param compilerSampleBufferRows データセット変換処理が並べ替え用に保持するサンプル行数
 ...
 * @param computePrecision 順伝播/入力の計算精度。BFLOAT16では実行パラメーター/勾配にも適用し、更新はFLOAT32 更新の基準となるパラメーターで行う
 * @param datasetRowsPerShard 変換済みデータセットの1 分割ファイル当たり行数
 * @param datasetPrefetchShards 学習中に先読みする変換済み分割ファイル数
 ...
 * @param behaviorCloningCoef 選択された方策 NLLの損失係数
...
public record DecisionPretrainSettings(
    ...
    @Setting("optimizerBatchRows") @Default("1024") @Positive int optimizerBatchRows,
    @Setting("maximumDeviceBatchRows") @Default("512") @Positive int maximumDeviceBatchRows,
    @Setting("tensorTransfer") @Default("DIRECT_BUFFER") DecisionTensorTransfer tensorTransfer,
