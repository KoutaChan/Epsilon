...
/**
 * Decision の一時保存で使うプリミティブ配列のバイナリ変換処理。
 *
 * <p>呼び出し元ごとの {@link Scratch} でバイトバッファを再利用し、必要な容量が増えたときだけ拡張する。呼び出しごとの配列確保と初期化の負荷を抑える。
 *
 * <p>配列長を int で記録し、要素をリトルエンディアンで格納する。{@link DataOutputStream#writeFloat(float)}
 * はビッグエンディアンで書き込むため使用しない。
 */
