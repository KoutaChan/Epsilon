...
/**
 * 局、4家、34牌種、河、面子の特徴量から、牌種間の関係と各家の履歴を反映した表現を作る。
 *
 * <p>牌種には関係付きの自己注意を、各家の要約・河・面子にはプレイヤー内の自己注意を適用する。河の順序、リーチ宣言前後、手出し、副露との対応を保持し、パディングは注意機構と出力から除外する。
 *
 * <p>方策と価値関数はこの表現を共有するが、局面全体への集約は個別に行う。価値関数は共有表現への勾配を切り離し、このエンコーダーを更新しない。
 */
public final class EpsilonMahjongStateEncoder extends AbstractBlock {
  ...
  /** 重みを固定した戦略文脈の層の組を、このデバイスパイプライン専用の永続Fusion 実行計画へ結び付ける。 */
  ...
  /** 重みを固定したプレイヤー-メモリエンコーダーを、このデバイスパイプライン専用の疎な永続Fusion 実行計画へ結び付ける。 */
  ...
  /** 重みを固定した牌種間の関係層の組を、このデバイスパイプライン専用の永続Fusion 実行計画へ結び付ける。 */
  ...
  /**
   * 標準形式の状態 slabsを構成要素トークンへ展開し、共有メモリを返す。
   *
  ...
  /**
   * 標準形式の状態 slabs と詰めたプレイヤー-メモリインデックスから共有局・プレイヤー・牌などの特徴表現を作る。
   *
   * @param parameterStore パラメーター取得先
   * @param stateCategories {@code [batch, STATE_INT_COUNT]} カテゴリ値状態テンソル
   * @param stateNumerics {@code [batch, STATE_FLOAT_COUNT]} 数値状態テンソル
   * @param playerMemoryPresentIndices 存在するプレイヤー-メモリ行の詰めたインデックス。未指定ならマスクから求める
  ...
  private EncodedMemory encodeMemoryInternal(
    ...
    try (var ignored = InferenceProfile.section("state.project.round")) {
      roundEmbedding =
    ...
    try (var ignored = InferenceProfile.section("state.project.player")) {
      playerEmbeddings =
          projectEntityFeatures(
    ...
    try (var ignored = InferenceProfile.section("state.project.tile")) {
      tileEmbeddings =
          projectEntityFeatures(
    ...
    try (var ignored = InferenceProfile.section("state.project.river")) {
    ...
    try (var ignored = InferenceProfile.section("state.project.meld")) {
      meldEmbeddings =
          projectEntityFeatures(
    ...
    try (var ignored = InferenceProfile.section("state.entity_concat")) {
      ...
      entityTokenEmbeddings = NDArrays.concat(entitySections, 1);
    }
   ...
   *
   * <p>未対応対局エンジンではDJLの可搬な加算後埋め込みへ代替処理する。返却データ型は{@code table}と同じで、学習時は通常の密な埋め込み 勾配を保持する。
   *
  ...
  /** GPU重みを固定したモデルの推論で埋め込みテーブルと数値入力のデータ型が一致するときだけ構成要素特徴量を直接詰める。 */
   ...
   *
   * <p>lookup、PAD除外、面子の有効牌平均、存在するマスクを一つの対局エンジン演算へ渡す。学習時はautograd境界を変えないため、 {@link
   * #embedCanonicalTiles(ParameterStore, NDArray, boolean, PairList)} の逐次実行式を使う。
   */
   ...
   * @param tileEmbeddings 34牌種それぞれのcontextual表現。形状は {@code [batch, 34, hidden]}
   * @param tileProjectionEmbeddings 牌種表現をLinearへ渡す低精度ビュー。学習時とCPU推論では {@code tileEmbeddings} と同じ参照
   * @param playerMemory プレイヤートークンと各家の河・面子を席別に保持したメモリ。形状は {@code [batch, 4, playerMemory, hidden]}
  ...
  public record EncodedMemory(
    ...
    /** 低精度ビューを分けない既存call 適用箇所用の同値コンストラクター。 */
