...
/**
 * バッチの構成が確定した後、対局から得た各行を学習用の最終 CPU バッファへ直接書き込む。
 *
 * <p>保存形式とメモリの確保方法、デバイスへの転送を分離するため、プリミティブ型のバッファだけを扱う。テンソルごとに並んだ1行のデータを、複数行のバッチ内の対応位置へ書き込む。方策の未使用位置は明示的にゼロで上書きし、以前の内容が残ることを防ぐ。
 */
public final class DecisionTrainingSlabWriter {
  ...
  /** 一行データ本体の二本の入力連続バッファを最終バッチの指定行へ対応位置へ書き込む。 */
  ...
  /** 一行データ本体のカテゴリ値連続バッファを対応位置へ書き込む。直後に同じ行の数値連続バッファを書く。 */
  ...
  /** 処理待ちのカテゴリ値行と同じ行の数値連続バッファを対応位置へ書き込み、派生インデックスを確定する。 */
  ...
  /** 探索後の方策を先に最終教師値連続バッファへ書き、同じ符号化・復号処理一時バッファの再利用を可能にする。 */
