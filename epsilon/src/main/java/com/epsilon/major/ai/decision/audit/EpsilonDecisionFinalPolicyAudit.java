...
import com.epsilon.major.ai.decision.training.EpsilonDecisionTrainer;
import com.epsilon.major.ai.decision.training.EpsilonDecisionTrainingBatch;
 ...
 *
 * <p>メモリ内の入力元は一括、逐次処理入力元は上限を設けたまとまりに分けるが、監査対象を間引かない。方策対象外のサンプル も入力と出力の有限性を検査し、方策/KL 診断の集計だけから除外する。
 */
public final class EpsilonDecisionFinalPolicyAudit {
   ...
   * @param samples 省略せず評価する選択された入力元
   * @param batchSize 一回の論理バッチ行数
   * @param config 本番損失と同じ更新設定
   * @return 入力元全体のKL・探索分の学習への寄与・方策更新比率監査結果
   */
  ...
  /**
   * 行動選択プレイヤー重みとデバイス遷移の格納領域上限を本番更新と揃える逐次処理全件監査。
   *
   * @param model 監査対象 Decision モデル
   * @param fragmentPaths 読み込む学習データ片の順序付きパス
   * @param reader 学習データ片から選択されたサンプルを読むコールバック
