...
/** 順位や得点を、設定された評価基準に応じて Decision の価値学習に使う教師値へ変換する。 */
public final class EpsilonUtilityTargets {
  ...
  /** モデル学習入口で保存サンプルとモデルのプロファイル一致を検査します。 */
   ...
   *
   * @param sampleProfile サンプルに保存されたプロファイル通し番号
  ...
  /**
   * 0始まりの最終順位のワンホット分布を返す。
   *
   * @param finalRank 0始まりの最終順位
   * @return 長さ4のワンホット順位教師値
   */
  ...
  /**
   * 順位に基づく効用定義で四つの最終順位の分布をスカラー価値へ復号する。
   *
   * @param profile 順位効用を持つプロファイル
