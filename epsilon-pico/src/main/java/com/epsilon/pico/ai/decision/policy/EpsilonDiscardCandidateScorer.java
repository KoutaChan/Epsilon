 ...
 *
 * <p>入力は判断対象の行動の種類ではなく、完全に投影済みの打牌遷移表現である。したがって同じ物理牌の打牌候補を通常手番で比較するときも、
 * チー・ポン後の合法打牌集合を比較するときも、同じ関数と同じ尺度を使う。判断対象の行動の効用は別スコア計算処理が求め、このブロックのロジットは条件付き
 * マスク付きソフトマックスの重みにだけ使われる。
 */
