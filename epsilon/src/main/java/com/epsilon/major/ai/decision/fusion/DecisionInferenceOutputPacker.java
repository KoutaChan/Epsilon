package com.epsilon.major.ai.decision.fusion;
 ...
 *
 * <p>FUSION経路は入力元数・各データ型・各幅・最大行数を含む構成識別情報ごとに実行計画を遅延生成する。通常は直前に使った構成識別情報を比較だけで再利用し、バッチごとのキー生成や
 * 実行計画構築を行わない。各プログラムはパイプラインの実行枠数と同じ再利用する出力バッファを持ち、投入 スレッドから直列に作成された複数利用権を独立して保持できる。
 */
public final class DecisionInferenceOutputPacker implements AutoCloseable {
  ...
  /**
   * 指定した出力連結処理方式とパイプラインの実行枠数を持つ連結処理を作る。
   *
  ...
  private static final class FusionProgram implements AutoCloseable {
    ...
    /**
     * 投入後の構築失敗ではデバイス完了を確認してから実行枠を返す。
     *
     * <p>通常経路には同期を加えない。例外時だけ利用権が記憶する投入済み処理
     * ストリームの完了を待ち、その後に実行枠を返す。完了を確認できない場合だけプログラムを再利用不可に設定し、利用権とネイティブ資源を保持したまま再利用を禁止する。
     */
