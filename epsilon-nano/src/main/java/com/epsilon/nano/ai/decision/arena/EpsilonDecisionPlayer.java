...
import com.epsilon.nano.ai.decision.data.EpsilonDecisionLegalActions;
...
import com.epsilon.nano.ai.decision.data.EpsilonDecisionTrajectoryPayloadStore.PayloadRef;
...
/** 合法手の一覧から、ニューラルネットワークの出力に基づいて行動を選ぶプレイヤー。 */
public class EpsilonDecisionPlayer implements Player {
   ...
   * @param causalTraceLambda 価値学習と因果関係のある行動の方策学習で遡る際の減衰係数
  ...
  public static final class Builder {
    ...
    /**
     * 対局中の行動履歴に記録する学習の反復開始時の 学習側モデルのスナップショット ID を設定する。
     *
     ...
     *
     * @param fullSupport 探索後の方策に混ぜる探索設定
     * @return このビルダー
     */
    ...
    /**
     * 価値計算用と方策更新用の二つの遷移列に使うトレース係数を設定する。
     *
     ...
     *
     * @param randomSeed 方策モデル-局所的な乱数シード
     * @return このビルダー
     */
    ...
    /**
     * 選択方式、探索に割り当てる確率、スカラー値のトレース係数、探索学習への寄与混合率をまとめて設定する。
     *
     * @param rolloutConfig 方策モデルが従う対局生成規則
     * @return このビルダー
     */
    ...
    /**
     * 学習反復の区切りで固定された不確実性CDFと収集診断用の集計を設定する。
     *
  ...
  /**
   * 半荘終了時の対局履歴を、入力本体を読み戻さずに学習サンプルへ変換する。
   *
   * <p>型付き入力 / behaviorPolicy / rolloutPolicy は処理中の一時保存領域から読まず、選択時に保存した合法手メタデータ
   * と軽量統計を使って遅延サンプルを作る。学習データファイル書き込み処理はデータ本体ファイル表とオフセット/長さ参照だけを保存する。
   */
