...
/**
 * ホスト側の Decision 入力をデバイスへ転送し、モデルが参照するテンソルを構築する。
 *
 * <p>カテゴリ値と数値をそれぞれ連続バッファとして転送し、共通の配置定義から用途別のビューを作る。カテゴリ値は INT16 のまま保持し、埋め込み参照などで広い整数型が必要な箇所だけで変換する。
 *
 * <p>作業領域を使う場合は実行枠ごとのページロック済みバッファまたは直接バッファを再利用し、複数の入力行を中間バッチなしで集約する。推論用のインデックスは INT32
 * の連続バッファにまとめる。返却したテンソルの寿命は転送先のリソース管理オブジェクトに従う。
 */
public final class DecisionBatchTransfer {
   ...
   * @param hostBatch 収容上限まで確定済みのホスト側バッチ
   ...
   *
   * @param manager 転送先デバイスを所有する管理元
   * @param hostRows 収容上限まで確定済みバッチの連続行スライス
   * @param workspace 実行枠専用の再利用バッファ。{@code null} なら一時バッファを作る
   * @param transferMode ページロックしたまたは直接参照のバッファ転送方式
   * @return 論理ビューと任意の学習教師値を所有するデバイス側バッチ
   */
  public static DecisionDeviceBatch transferTrainingToDevice(
      NDManager manager,
  ...
  /** 完全なホスト側バッチをホスト-詰めた逐次実行推論入力としてデバイスへ転送する。 */
  ...
  /** 数値連続バッファを指定精度へ一度だけ変換するホスト-詰めた逐次実行推論転送。 */
  public static DecisionInferenceDeviceBatch transferInferenceToDevice(
      NDManager manager,
      DecisionHostBatch.RowSlice hostRows,
      Workspace workspace,
  ...
  /**
   * 実行枠専用ページロックしたバッファへ複数の非所有行ビューを直接指定位置から抽出した状態です。
   *
   * <p>サンプルの参照情報はバッファを所有しません。同じ{@link Workspace}で次の処理段階を始める前に、H2D イベントの完了を保証する必要があります。
   */
  ...
  /** 行動が一意な価値推論用に状態入力だけを指定位置から抽出した状態です。 */
   ...
   *
   * <p>行動が一意に決まる局面の価値推論では行動/遷移連続バッファを転送せず、方策出力用ネットワークを型として呼び出せない {@link DecisionStateInputs}を返す。
   *
   * @param manager 転送先デバイスを所有する管理元
   * @param hostRows 収容上限まで確定済みバッチの連続行スライス
   * @param workspace 実行枠専用の再利用バッファ。{@code null} なら一時バッファを作る
   * @param transferMode ページロックしたまたは直接参照のバッファ転送方式
   * @return FLOAT32の状態だけを使うデバイス入力
   */
  ...
  private static NDArray convertInputNumerics(NDArray numerics, DataType dataType) {
    ...
    try (var section = InferenceProfile.section("transfer.numeric_cast")) {
   ...
   * @param playerMemoryPresentIndices プレイヤー-メモリの有効トークンインデックス
   ...
   *
   * <p>行動、遷移、実際の行動選択に使う方策、探索前の方策、方策教師値は転送しない。大きい合法候補容量区分でも転送量は
   * 状態スキーマと行数だけで決まり、価値出力用ネットワークの計算内容は完全な{@link DecisionDeviceBatch}を渡す場合と同一になる。
   *
   * @param manager 転送先デバイスを所有する管理元
   * @param hostRows 学習教師値を持つ、収容上限まで確定済み行スライス
   * @param workspace 実行枠専用の再利用バッファ。{@code null} なら一時バッファを作る
   * @param transferMode ページロックしたまたは直接参照のバッファ転送方式
   ...
   *
   * <p>同時利用を同期しない。ページロックした複製は非同期なので、次の利用前に前回の順伝播を完了して前回の管理元を閉じる必要がある。 PyTorch
  ...
  public static final class Workspace implements AutoCloseable {
    ...
    /**
     * 複数の行ビューを実行枠専用ページロックした入力へ直接指定位置から抽出します。
     *
     * @param rows 同じ容量区分に属する非所有行ビュー群
     * @return 後続のH2D キューへの追加に渡す軽量サンプルの参照情報
     */
    public StagedInference stageInference(DecisionHostBatch.RowBatch rows) {
      ...
      try (var event =
          InputBatchProfile.beginStage(
              "pico", rows.bucket(), rows.size(), "inference.layout_count")) {
    ...
    /**
     * 現在のPyTorch ストリームへ指定位置の抽出済み入力のH2D 複製をキューへの追加します。
     *
     * @param destination バッチ-局所的なデバイス管理元
     * @param staged この作業領域が直前に作ったサンプルの参照情報
    ...
    public DecisionInferenceDeviceBatch enqueueStagedInference(
      ...
      InferenceInputSlabs input;
      InferenceIndexSlab indices;
      try (var section = InferenceProfile.section("transfer.h2d")) {
        input = pinnedInferenceInputs.enqueue(destination, staged.inputLayout);
        indices = pinnedInferenceIndices.enqueue(destination, staged.indexLayout);
      }
    ...
    /** 価値のみ行を実行枠専用ページロックした状態バッファへ直接指定位置から抽出します。 */
    public StagedState stageState(DecisionHostBatch.RowBatch rows) {
      ...
      try (var event =
          InputBatchProfile.beginStage("pico", rows.bucket(), rows.size(), "state.layout_count")) {
      ...
      try (var event =
          InputBatchProfile.beginStage(
              "pico", rows.bucket(), rows.size(), "state.buffer_capacity")) {
      ...
      try (var event =
          InputBatchProfile.beginStage("pico", rows.bucket(), rows.size(), "state.input_copy")) {
      ...
      try (var event =
          InputBatchProfile.beginStage("pico", rows.bucket(), rows.size(), "state.index_write")) {
    ...
    /** 現在のPyTorch ストリームへ指定位置の抽出済み状態入力のH2D 複製をキューへの追加します。 */
  ...
  /** 固定アドレスのデバイス入力と、その内容を更新する専用ページロックしたバッファ。 */
  public static final class GraphInput {
     ...
     *
     * @param hostRows 記録時と同じ行数・容量区分を持つ確定済みスライス
     */
  ...
  static final class PinnedInferenceInputSlabs implements AutoCloseable {
    ...
    private void stage(DecisionHostBatch.RowBatch hostRows, DecisionInferenceInputLayout layout) {
      try (var event =
          InputBatchProfile.beginStage(
              "pico", hostRows.bucket(), hostRows.size(), "inference.buffer_capacity")) {
        ensureCapacity(layout);
        if (event != null) {
          event.success = true;
        }
      ...
      try (var event =
          InputBatchProfile.beginStage(
              "pico", hostRows.bucket(), hostRows.size(), "inference.input_copy")) {
        hostRows.copyInferenceInputsTo(
            categoricalBuffer.getByteBuffer().order(ByteOrder.nativeOrder()).asShortBuffer(),
            numericBuffer.getByteBuffer().order(ByteOrder.nativeOrder()).asFloatBuffer(),
            layout);
  ...
  static final class PinnedInferenceIndexSlot implements AutoCloseable {
    ...
    private void stage(
      ...
      try (var event =
          InputBatchProfile.beginStage(
              "pico", hostRows.bucket(), hostRows.size(), "inference.index_write")) {
        hostRows.copyInferenceIndicesTo(
            buffer.getByteBuffer().order(ByteOrder.nativeOrder()).asIntBuffer(), layout);
        if (event != null) {
