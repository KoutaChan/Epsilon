...
public final class EpsilonDecisionLegalActions {
  ...
  /**
   * 先頭の合法位置だけを残した行動 ID 配列を返す。
   *
   * @param legalActionIdBySlot 位置順の行動 ID 配列
   * @param legalActionCount 有効な先頭位置数
   * @return 有効位置だけを含む配列。元配列が既に同じ長さならその参照
   */
  ...
  /**
   * 詰めた合法手配列から指定位置の安定行動 ID を取得する。
   *
   * @param legalActionIdBySlot 詰めた位置順の行動 ID
   * @param slot 取得するゼロ-based 位置
   * @return 指定位置の行動 ID
   */
