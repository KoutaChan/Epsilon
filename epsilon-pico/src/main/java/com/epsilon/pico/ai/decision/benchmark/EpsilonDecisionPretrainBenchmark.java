...
import com.epsilon.pico.ai.decision.input.DecisionTrainingTargets;
...
public final class EpsilonDecisionPretrainBenchmark {
   ...
   * @param warmupSteps JIT・メモリ割り当て処理を安定させる非計測更新回数数
   * @param measuredSteps 指標へ含める更新回数数
   * @param legalActionCapacity 人工生成のバッチの行動容量区分幅
  ...
  public static DecisionHostBatch syntheticBatch(int rows, DecisionBucket bucket) {
    return syntheticBatch(
        rows, bucket, com.epsilon.pico.config.settings.EpsilonSettings.defaults());
  }
   ...
   * @param optimizerBatchRows 一更新回数の総行数
   * @param maximumDeviceBatchRows 一デバイス順伝播の最大行数
   * @param warmupSteps 計測前に実行した更新回数数
   ...
   * @param loss 総目的関数の更新回数平均
   * @param behaviorCloningLoss 方策教師損失の更新回数平均
   * @param valueLoss 価値損失の更新回数平均
   */
