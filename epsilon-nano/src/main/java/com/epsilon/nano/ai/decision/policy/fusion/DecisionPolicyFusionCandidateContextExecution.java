...
import com.epsilon.nano.ai.decision.policy.DecisionPolicyCandidateContextExecution;
import com.epsilon.nano.ai.decision.policy.DecisionPolicyCandidates;
import com.epsilon.nano.ai.decision.policy.DecisionPolicyContextPool;
...
/**
 * djl-rocm のマスク付きソフトマックス集約を使い、候補の文脈表現を集約してネットワークの出力列へ対応付ける。
 *
 * <p>分岐候補の10列、PASS の1列、および後段の Fusion が使う RON・KYUSHU・TSUMO
 * の各1列を、一回のネイティブ呼び出しで作る。各出力はセッションが確保した二つのバッファの部分ビューであり、有効な行の参照だけを呼び出し側へ貸し出す。入力テンソルの所有権は移さず、出力の利用権は方策の順伝播が完了するまで保持する。
 *
 * <p>候補数の上限は Decision
 * の容量区分ごとに固定する。スコア、マスク、候補表現のデータ型は初回の入力から確定し、同じ区分では実行計画を再利用する。実際に使用した区分だけセッションを生成するため、未使用区分の VRAM
 * は確保しない。
 */
