...
import com.epsilon.major.ai.decision.policy.DecisionPolicyExecution;
import com.epsilon.major.ai.decision.policy.DecisionPolicyScores;
import com.epsilon.major.ai.decision.policy.EpsilonDecisionPolicyHead;
import com.epsilon.major.ai.model.fusion.EpsilonMahjongStateReadoutGroupExecution;
...
/**
 * 麻雀の合法手ごとの方策と、HL-Gauss によるスカラー価値の分布を予測するネットワーク。
 *
 * <p>方策と価値は牌・河・副露を文脈化する{@link EpsilonMahjongStateEncoder}を共有し、目的固有の{@link
 * EpsilonMahjongStateReadout}だけを分離する。共有エンコーダーは方策パラメーターであり、価値 特徴量の集約へ渡すメモリは常に勾配を遮断する。
 * したがって価値損失は共有表現を一切更新せず、方策分布を暗黙に移動させない。
 *
 * <p>方策または価値だけが必要な処理では専用順伝播により不要な特徴量の集約/ヘッドを省略する。両方が必要な通常順伝播では、高価な 構成要素文脈化を一度だけ実行する。
 */
public final class EpsilonDecisionNetwork extends AbstractBlock {
  ...
  private final Linear valueResidualHead;
  private final Linear valueHead;

  /** 設定で指定された隠れ層幅を使ってネットワークを構築する。 */
  ...
  /** モデルの複製が入力元モデルと同じ効用定義を明示的に継承するコンストラクター。 */
  public EpsilonDecisionNetwork(int hiddenSize, EpsilonUtilityProfile utilityProfile) {
    ...
    valueResidual.getDirectParameters().get("weight").setInitializer(new ConstantInitializer(0.0f));
    valueResidual.getDirectParameters().get("bias").setInitializer(new ConstantInitializer(0.0f));
    valueResidualHead = addChildBlock("valueResidualHead", valueResidual);
  ...
  /** ホストでコンパクト化したeager入力から方策と価値を推論する。 */
  ...
  /**
   * ホストでコンパクト化した入力を、実行枠を管理するな方策推論実行境界とともに推論する。
   *
  ...
  /**
   * 価値特徴量の集約を起動せず、共有状態エンコーダーと方策採点器だけを実行する。
   *
  ...
  /** 価値特徴量の集約を起動せず、ホストでコンパクト化したeager 方策入力だけを実行する。 */
  ...
  /**
   * 価値を起動せず、実行枠を管理するな方策推論実行境界とともに方策を推論する。
   *
  ...
  /**
   * 方策特徴量の集約を起動せず、共有状態エンコーダーと価値効用-区間ヘッドだけを実行する。
   *
   * <p>共有メモリは価値特徴量の集約の直前で勾配を遮断する。価値出力層と価値特徴量の集約だけが勾配を受け取る。
   *
   * @param parameterStore パラメーター取得先
   ...
   *
   * <p>価値ネットワーク部分は行動/遷移を参照しないため、この引数の異なるメソッドは通常の{@link DecisionDeviceBatch}経路と同じ状態
   * エンコーダーおよびヘッドを実行しながら、価値のみ事前学習のホストからデバイスへの転送を最小化する。
   *
   * @param parameterStore 順伝播に使うパラメーターの取得先
  ...
  /** ホストで確定したプレイヤーの履歴表現インデックスを使い、GPUで存在行を再集計せず価値を推論する。 */
  public NDArray forwardInferenceValue(
      ParameterStore parameterStore,
      DecisionStateInputs inputs,
      NDArray playerMemoryPresentIndices,
   ...
   *
   * <p>方策共有部分と候補採点器は実行しない。保存データによる検証で1候補行の方策統計を作らず、価値表現だけを監査するために使う。
   *
   * @param parameterStore 順伝播に使うパラメーターの取得先
   * @param inputs 状態テンソルだけを持つ価値専用入力
   * @param training 学習固有のブロック挙動を有効にするか
  ...
  /** コンパクトプレイヤーの履歴表現インデックスを再利用する価値専用診断経路。 */
  public ValueDiagnosticOutput forwardValueWithDiagnostics(
      ParameterStore parameterStore,
      DecisionStateInputs inputs,
      NDArray playerMemoryPresentIndices,
  ...
  /**
   * 両ネットワーク部分の出力に加え、それぞれの状態埋め込みを監査用に返す。
   *
  ...
  /** ホストでコンパクト化したeager入力から方策/価値と診断埋め込みを推論する。 */
  ...
  /**
   * 診断特徴量の集約を含め、実行枠を管理するな方策推論実行境界とともに推論する。
   *
  ...
  /** コンパクト推論バッチの論理ビューと疎インデックスを順伝播有効範囲へ一時的に借用する。 */
   ...
   * @param fusionSettings 構成部分ごとのFusion実行方式
  ...
  private InferenceReadouts encodeInferenceReadouts(
    ...
    try (NDManager scope = outputManager.newSubManager()) {
      ...
      outputManager.attachAll(
          ...
          policyState.playerMemoryKeyValues(),
          valueStateEmbedding);
  ...
  private NDArray forwardValueHeads(
    ...
    valueHiddenEmbedding =
        valueHiddenEmbedding.add(
            valueResidualHead
                .forward(
                    parameterStore,
                    new NDList(EpsilonMahjongStateEncoder.silu(valueHiddenEmbedding)),
                    training,
                    runtimeParameters)
                .singletonOrThrow());
  ...
  /**
   * パラメーター名が方策採点器群に属するか判定する。
   *
   * @param name DJL パラメーターの階層名
   * @return 方策ヘッド配下なら {@code true}
   */
  ...
  /**
   * パラメーター名が方策所有の共有エンコーダー、特徴量の集約、採点器に属するか判定する。
   *
   * @param name DJL パラメーターの階層名
  ...
  /**
   * パラメーター名が価値専用特徴量の集約または効用-区間ヘッドに属するか判定する。
   *
   * @param name DJL パラメーターの階層名
  ...
  public static boolean isValueParameterName(String name) {
    return name.contains("valueStateReadout")
        ...
        || name.contains("valueResidualHead")
   ...
   *
   * <p>共有状態エンコーダーは方策所有であり、価値だけを有効にしても学習可能にしない。
   *
   * @param policyEnabled 共有状態エンコーダー、方策特徴量の集約、採点器を更新するか
   * @param valueEnabled 価値特徴量の集約と効用-区間ヘッドを更新するか
   */
  public synchronized void prepareForOfflinePretraining(
    ...
    valueResidualHead.freezeParameters(!valueEnabled);
    valueHead.freezeParameters(!valueEnabled);
  }

  /** 自己対局による一つに学習で更新する方策/価値ブロックをすべて学習可能に戻す。 */
  public synchronized void prepareForOnlineTraining() {
    ...
    valueResidualHead.freezeParameters(false);
    valueHead.freezeParameters(false);
  }

  /**
   * 採点器/価値出力と、共有メモリから独立に読んだ集約表現をまとめた監査用出力。
   *
