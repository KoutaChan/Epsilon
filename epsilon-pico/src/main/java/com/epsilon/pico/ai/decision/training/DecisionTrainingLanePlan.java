...
/**
 * 一回のパラメータ更新に使う小バッチを、推定計算量に応じて学習ワーカーへ配分する。
 *
 * <p>管理するのはワーカーごとの一覧と配分順だけであり、小バッチやサンプルの参照情報は複製しない。計画の実行が終わるまで、返された一覧と配列を変更してはならない。
 */
final class DecisionTrainingLanePlan {
  ...
  /**
   * 処理量降順のLPTで、累積処理量が最小の学習ワーカーへ小バッチを一度だけ割り当てる。
   *
   * <p>{@code batches}とサンプルの参照情報は並べ替えもコピーもしない。同処理量時の探索開始学習ワーカーは更新回数ごとに回転し、短い末尾を学習ワーカー 0へ固定しない。
   */
  ...
  /** サンプルの参照情報を必要としない純粋LPT。戻り値は元処理量インデックスごとの学習ワーカー番号である。 */
