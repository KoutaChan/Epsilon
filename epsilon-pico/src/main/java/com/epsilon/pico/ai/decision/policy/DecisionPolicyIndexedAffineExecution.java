...
/**
 * 一つのDecision デバイスパイプラインで疎な二択の判定を実行する境界。
 *
 * <p>学習ブロックとパラメーターは所有せず、重みを固定したパラメーターを参照する。各順伝播が返す密なスコアは、最終D2H完了まで{@link Forward}の利用権で再利用を止める。
 */
