...
/**
 * 学習サイクルごとに固定したサンプルで、方策更新の前後の KL ダイバージェンスを測定する。
 *
 * <p>対象となる全サンプルから一様に無作為抽出し、入力の参照と更新前の合法手確率だけを保持する。モデルの複製や勾配計算は行わず、更新前後で同じ学習ワーカー、バッチ形状、行順、学習モード、精度設定を使う。
 */
    ...
    try (EpsilonDecisionDataParallel.TrainingInputSequence inputs =
      ...
      while ((prepared = inputs.next()) != null) {
        ...
        try (DecisionTrainingReadyBatch ready = prepared.ready();
            ...
            // Autograd 方式 also selects kernels. Match the 学習器, without 逆伝播 or オプティマイザー
            // 処理.
