...
public final class EpsilonDecisionEvaluatorFactory {
  ...
  /** 固定対戦評価用に方策のみ複製モデルと詰めた最大確率の行動を選ぶ方式出力境界を開く。 */
  public static GreedyHandle openGreedyPolicyCheckpointEvaluator(
      Path checkpoint,
  ...
  /** loader所有権を受け取り、固定/cast完了後だけ呼び出し側へ複製モデルを移す。 */
  ...
  public static final class Handle implements AutoCloseable {
    ...
    /** 単一の-デバイス診断を行うサーバーを返し、非同期循環バッファなら物理GPU全体を先に処理の回収します。 */
  ...
  /** 方策のみ評価器の資源所有権を保った対戦評価用詰めた最大確率の行動を選ぶ方式参照。 */
