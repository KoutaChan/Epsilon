...
/** 保存した AdamW 状態が、現在の方策損失と対局収集条件に対応するか識別する。 */
final class DecisionSelectedPgLearnerContract {
  ...
  /** 学習と対戦評価の一連の実行・比較試行・計画変更が共有する対局生成と保存契約を一度に解決する。 */
  ...
  static String fingerprint(
    ...
    // 目標計画はチェックポイントの制御器の状態が所有し、明示計画変更で変更できる。
    // 計画予算・終端倍率を互換性識別子へ含めると同じAdamWを保った延長を妨げるため、
    // ここでは更新回数と較正を含む不変の学習・制御意味だけを固定する。
    String canonical =
        String.join(
            "\n",
            "schema=" + SCHEMA,
