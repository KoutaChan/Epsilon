...
import com.epsilon.nano.ai.decision.policy.DecisionPolicyAffineExecution;
...
public final class DecisionPolicyFusionAffineExecution extends DecisionPolicyAffineExecution {
  ...
  private final NDArray policyNodeContexts;
  private final NDArray alternativeContexts;
  ...
  public DecisionPolicyFusionAffineExecution(
      ...
      IdEmbedding policyNodeEmbedding,
      IdEmbedding alternativeEmbedding,
      NDArray policyNodeIds,
    ...
    try {
      ...
      policyNodeContexts =
          embeddingContexts(
              policyNodeEmbedding, policyNodeIds, runtimeParameters, "policy-node contexts");
      alternativeContexts =
          embeddingContexts(
              alternativeEmbedding, alternativeIds, runtimeParameters, "alternative contexts");
      alternativeWeights = splitAlternativeWeights();
  ...
  private AffineProgram createAlternativeProgram(int capacity, DataType[] inputDataTypes) {
    ...
    FusionRecipe.Constant policyNodes =
        builder.addConstant(
            "policyNodeContexts",
            FusionRecipe.TensorSpec.fixed(dataType, 1, alternatives, hiddenSize));
    FusionRecipe.Constant alternativeNodes =
        builder.addConstant(
            "alternativeContexts",
            FusionRecipe.TensorSpec.fixed(dataType, 1, alternatives, hiddenSize));
    FusionRecipe.Value[] sources = {state, candidates, policyNodes, alternativeNodes};
    ...
    for (int component = 0; component < sources.length; component++) {
      weights[component] =
          builder.addConstant(
              "weight" + component,
              FusionRecipe.TensorSpec.fixed(dataType, hiddenSize, hiddenSize));
    ...
    bindings.bind(policyNodes, policyNodeContexts).bind(alternativeNodes, alternativeContexts);
  ...
  /** 列部分ビューは非連続ビューになるため、ネイティブ定数関連付け用に構築時の一度だけ連続化する。 */
  private NDArray[] splitAlternativeWeights() {
    NDArray weight = weight(alternativeHidden);
    NDArray[] slices = new NDArray[4];
    for (int component = 0; component < slices.length; component++) {
  ...
  private NDArray embeddingContexts(
      IdEmbedding embedding,
      NDArray ids,
      PairList<String, Object> runtimeParameters,
      String label) {
    NDArray contexts;
    try (NDManager scope = constantsManager.newSubManager()) {
      scope.tempAttachAll(ids);
      contexts =
          embedding
              .forward(parameterStore, new NDList(ids), false, runtimeParameters)
              .singletonOrThrow()
              .reshape(1, DecisionAlternative.NETWORK_SIZE, hiddenSize);
      contexts.attach(constantsManager);
    }
    if (contexts.getDataType() != dataType) {
      throw new IllegalStateException(label + " dtype differs from affine weights");
    }
    return contexts;
  }
