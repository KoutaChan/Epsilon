...
/**
 * カテゴリ値と数値を別々の連続バッファへ配置するための、入力形状とオフセットの定義。
 *
 * <p>ホスト側とデバイス側で同じ定義を使う。カテゴリ値は INT16 で転送・保持し、広い整数型が必要な演算だけで拡張する。
 */
public final class DecisionInputLayout {
  ...
  public enum Tensor {
    ...
    /** 方策グラフが条件付き候補を直接指定位置から抽出するための参照先の対応メタデータ。 */
    ACTION_ROUTES(Slab.CATEGORICAL),
    /** 判断対象の行動適用後の各遷移に属するカテゴリ値特徴量。 */
     ...
     *
     * @param rows バッチ行数
     * @param actions 合法手収容上限
     * @param transitions 行動あたりの遷移収容上限
  ...
  /**
   * 指定収容上限の二本のホスト側の連続バッファを記述する配置を作る。
   *
   * @param capacity 最大行数
   ...
   *
   * @return テンソル順序と牌種数を表す安定サンプルの参照情報
   */
