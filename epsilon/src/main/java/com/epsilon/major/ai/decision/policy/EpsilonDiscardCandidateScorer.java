 ...
 *
 * <p>入力は行動候補の種類ではなく、完全に投影済みの打牌遷移表現である。したがって同じ物理打牌候補を通常手番で比較するときも、
 * チー・ポン後の合法打牌集合を比較するときも、同じ関数と同じ尺度を使う。行動候補の効用は別採点器が求め、このブロックのロジットは条件付き マスク付きソフトマックスの重みにだけ使われる。
 */
