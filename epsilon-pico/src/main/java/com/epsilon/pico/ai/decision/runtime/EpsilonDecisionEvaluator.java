...
public interface EpsilonDecisionEvaluator extends AutoCloseable {
  ...
  /**
   * 一行の合法手集合を実際に評価する末端の行動評価器を返します。
   *
   * <p>通常の評価器は自身を返す。複数方策を振り分けるfacadeは、admission前に安定した末端の行動識別情報へ解決することで、 一つの論理的な
  ...
  /** ホスト符号化用格納枠を非blockingで取得し、失敗時は正確な解放通知を返します。 */
  ...
  /** この末端の行動が利用できるデバイスへ、次のバッチを供給する必要があるかを返す。 */
  default boolean needsInferenceWork() {
    return true;
  }

  /**
   * 取得済みホスト ingressと型付きバッチを同時に移譲します。
   *
   * <p>このメソッドを呼んだ後、呼び出し側はingressを解放しません。同期失敗を含む全終了処理は受け取った側が行います。
   */
  default CompletableFuture<List<EpsilonDecisionInferenceServer.Prediction>> submitBatch(
  ...
  /**
   * Streaming 対局実行環境が一度に構築する標準論理バッチ行数を返す。
   *
  ...
  /**
   * Streaming 対局実行環境が指定型付き容量区分について一度に構築する論理バッチ行数を返す。
   *
   * @param bucket 行動・遷移容量を表す型付き容量区分
   * @return ホスト側で一度に構築する行数
   */
