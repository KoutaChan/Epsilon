...
/**
 * CPU 上で Decision の入力と学習用の教師値を保持するバッチ。
 *
 * <p>入力と教師値の記憶領域を分離し、この型は構築済みの行順、有効範囲、行動・遷移の連続性を管理する。行の複製では独立した記憶領域を作り、行範囲の参照では元のバッチを保持して非同期処理が終わるまでその寿命を維持する。
 */
public final class DecisionHostBatch {
  ...
  /**
   * {@link DecisionInputSchema#fingerprint()}に一致する1行入力と確率分布の本体から復元する。
   *
   * @param bucket 入力と確率分布の本体の行動・遷移容量区分
   * @param categoricalValues 標準形式のカテゴリ値連続バッファ
   * @param numericValues 標準形式の数値連続バッファ
  ...
  /**
   * 復号済み標準形式の入力連続バッファの所有権を受け取り、複製せずに推論バッチを復元する。
   *
   * <p>呼出元は返却後に配列を読み書きしてはならない。永続化符号化・復号処理など、配列を新規確保して全要素を書き終えた境界だけで使用する。
   *
   * @param rowCount 入力と確率分布の本体に含まれる行数
   * @param bucket 入力と確率分布の本体の行動・遷移容量区分
  ...
  /**
   * 復号済み標準形式の入力/教師値連続バッファの所有権を受け取り、複製せずに学習バッチを復元する。
   *
   * <p>呼出元は返却後に配列を読み書きしてはならない。符号化・復号処理やバッチ生成処理が新規確保した配列だけを渡す。
   *
   * @param rowCount 入力と確率分布の本体に含まれる行数
   * @param bucket 入力と確率分布の本体の行動・遷移容量区分
   * @param encodedInputCategories 標準形式の入力カテゴリ値連続バッファ
   * @param encodedInputNumerics 標準形式の入力数値連続バッファ
   * @param encodedTargetCategories 標準形式の教師値カテゴリ値連続バッファ
   * @param encodedTargetNumerics 標準形式の教師値数値連続バッファ
   ...
   * @param transitionSlot 行動内の遷移候補の位置
   * @param waitSlot 待ち集合内の位置
   * @param feature 待ち-yaku 特徴量インデックス
   * @return 符号化済み役特徴量値
   */
  ...
  /**
   * 同一容量区分・教師値種別のバッチ群を一つのcontiguous 記憶領域へ連結する。
   *
  ...
  /**
   * 同一容量区分・教師値種別の連続行ビュー群を一つのcontiguous 記憶領域へ連結する。
   *
  ...
  public record RowSlice(DecisionHostBatch source, int fromInclusive, int size) {
    ...
    /**
     * 方策候補を伴わない価値推論で送る状態二連続バッファのバイト数を返す。
     *
    ...
    /**
     * 価値のみ転送で送る状態二連続バッファと着順教師値・サンプル重みの合計バイト数を返す。
     *
   ...
   *
   * <p>非同期推論パイプラインはこの型をCPU側の軽量サンプルの参照情報として保持し、GPU の実行枠が空いた時点で各入力元から実行枠専用のページロックした
   * バッファへ直接指定位置から抽出します。元バッチの所有権は移さないため、呼び出し側は推論完了まで入力元を保持します。
   */
  public static final class RowBatch {
    ...
    /** 学習教師値を含むサンプルの参照情報なら{@code true}を返します。 */
    public boolean hasTrainingTargets() {
    ...
    /** プレイヤー-メモリインデックスだけを集約バッチ上の位置へ直して書き込みます。 */
    ...
    /** 全入力元から詰めた推論入力を、集約配置上の最終位置へ直接書き込みます。 */
    void copyInferenceInputsTo(
        ShortBuffer categoricalDestination,
        FloatBuffer numericDestination,
        DecisionInferenceInputLayout layout) {
    ...
    /** 全入力元のプレイヤー-メモリ、遷移、方策実行インデックスを集約順に書き込みます。 */
    void copyInferenceIndicesTo(IntBuffer destination, InferenceIndexLayout layout) {
      int start = destination.position();
      copyPlayerMemoryPresentIndicesTo(
