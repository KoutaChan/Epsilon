...
final class EpsilonDecisionDataParallel
  ...
  /** 本番自己対局学習器用の実行単位集合を、有効デバイス設定から構築する。 */
  ...
  /** CPU 一致性テストなど、明示デバイス上へ単純な自己対局学習実行単位集合を構築する。 */
  static EpsilonDecisionDataParallel openOnline(
  ...
  /** FLOAT32・直接参照の転送の牌譜などの固定データによる事前学習ワーカー集合を構築する。 */
  ...
  /** 牌譜などの固定データによる事前学習ワーカー集合を、指定したホスト転送方式と実行パラメーター型で構築する。 */
  static EpsilonDecisionDataParallel openPretraining(
      Model canonicalModel,
  ...
  static final class TrainingInputSequence implements AutoCloseable {
    ...
    /** 次バッチを返し、直前バッチが解放した枠へ次の先読みを補充する。 */
    PreparedTrainingInput next() throws IOException {
      ...
      try {
        ...
        // 先行準備の失敗はキュー順に受け取り、元の例外型をそのまま返す。
      }
      if (futures.isEmpty()) {
        return null;
      }
    ...
    /** 未使用先読みを完了まで待ち、ホスト枠と転送完了通知をパイプラインへ返す。 */
    @Override
