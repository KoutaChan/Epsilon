 ...
 * 先頭構成部分だけがバイアスを持ち、加算後のバイアスが構成部分数に依存しないようにする。
 *
 * <p>ここでのFusionは学習モデル上の特徴合成を意味し、djl-rocmのFusion 実行計画ではない。凍結推論用のネイティブ実装は {@code
 * com.epsilon.major.ai.decision.policy.fusion} が所有する。
 */
public final class EpsilonFeatureFusion extends AbstractBlock {
  ...
  /** 各構成部分を宣言済み入力幅から共通幅へ投影し、ブロードキャスト可能な軸を保って加算する。 */
  NDArray fuse(
   ...
   * 宣言済み構成部分数を返す。
   *
   * <p>推論専用の永続実行計画が既存Linearを直接結び付けるためのパッケージ内境界であり、パラメーター自体は複製しない。
   */
  public int componentCount() {
    return projections.length;
  }

  /** 同じ候補軸を持ち、一つのGEMMへ連結できる先頭構成部分数を返す。 */
