 ...
 * {@link FusionOutputLease}を保持し、非同期D2Hが完了するまで実行枠の再利用を防ぐ。
 */
public final class DecisionPackedScores implements AutoCloseable {
  ...
  /**
   * バッチ-局所的な管理元が所有するEAGER テンソルを借用する。
   *
   * @param activeRows 有効行だけを持つまとめたスコアテンソル
  ...
  /** EAGER まとめたテンソルと、その計算が消費した上流利用権を束ねる。 */
  ...
  /**
   * 連結処理後の例外経路で、このスコアを生成するデバイス処理の完了を待つ。
   *
