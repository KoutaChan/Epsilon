...
/**
 * 合法手の確率分布から不確実性を測り、前回の収集結果から求めた累積分布関数（CDF）に基づいて探索倍率を決める。
 *
 * <p>収集中のヒストグラムは次の収集・更新単位でだけ利用する。したがってGPU完了順や収集処理スレッドの実行順は、現在の収集・更新単位の行動選択へ影響しない。
 */
public final class DecisionAdaptiveExploration {
  ...
  /** 次の収集・更新単位が参照するCDFをチェックポイント境界で固定する。 */
  ...
  public synchronized void restoreState(State state) {
    ...
    // 状態が変更されない配列を保持し、Calibrationは参照するだけなので、追加の複製は不要。
  ...
  /** 前の収集・更新単位の変更不可 CDFだけを参照する収集セッションを開始する。 */
  ...
  /** 当該収集・更新単位の集計をレポートへ固定し、そのヒストグラムを次の収集・更新単位のCDFへ切り替える。 */
  ...
  /** 収集失敗時は途中ヒストグラムを次の収集・更新単位へ持ち越さない。 */
  ...
  /** 選択時とエンジン結果確定時を結ぶ、対局中の行動履歴非永続の軽量観測値。 */
  record Observation(
  ...
  /** 一つの収集・更新単位中に全行動選択プレイヤープレイヤーが共有する収集セッション。CDF自体は変更不可。 */
  public static final class MacroSession {
    ...
    /** 最終的な確率下限適用後探索適用後のと実際の選択した候補の位置を記録する。 */
    ...
    /** 選択された応答がCAUSALかPREEMPTEDかを確定後に記録する。 */
    ...
    /** PendingDecisionが保持するコンパクトコードから種別/十分位別学習役割を記録する。 */
