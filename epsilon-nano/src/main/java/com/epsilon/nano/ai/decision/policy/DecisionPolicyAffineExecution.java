 ...
 *
 * <p>学習ブロックとパラメーターは所有せず、凍結済みパラメーターを参照する。投入スレッドは順伝播を直列に開始でき、枠ごとの{@link Forward}
 * を対応する連結したスコアのホスト転送が完了するまで保持する。
 */
public abstract class DecisionPolicyAffineExecution implements AutoCloseable {
   ...
   * 一つの方策順伝播を開始する。
   *
   * @param workingManager 最終連結したスコアのデバイス完了まで中間テンソルを所有するバッチ管理元
   * @return この順伝播で使うAFFINE_SUM 構成要素
   */
  ...
  /** この実行コンテキストを利用不能にした最初の失敗を返す。正常なら{@code null}を返す。 */
  public abstract Throwable failure();

  /**
   * 呼び出し側が同じストリーム末尾の完了を証明した後、失敗順伝播が保持した利用権を解放する。
   *
   * <p>実行単位の再利用禁止状態は解除せず、資源解放だけを可能にする。
   */
   ...
   *
   * <p>{@link #seal()}後は連結したスコア側へ所有権を渡す。正常時の{@link #close()}は最終D2H完了後にだけ呼ばれる。
   */
  public abstract static class Forward implements AutoCloseable {
    ...
    /** アフィン変換出力の所有権を最終連結したスコアへ渡せる状態に固定する。 */
    public abstract AutoCloseable seal();

    /**
     * 最終出力の完了証明へ到達できなかった失敗を記録する。
     *
     * <p>Fusion実装は既に投入した利用権を解放せず実行単位を再利用不可に設定する。EAGER実装は保持資源を持たない。
     */
  ...
  private static final class Eager extends DecisionPolicyAffineExecution {
    ...
    private final NDArray policyNodeIds;
    ...
    private Eager(
      ...
      this.policyNodeEmbedding = policyNodeEmbedding;
      this.alternativeEmbedding = alternativeEmbedding;
      this.policyNodeIds = policyNodeIds;
  ...
  private static final class EagerForward extends Forward {
    ...
    public NDArray alternative(
      ...
      NDArray policyNodes;
      NDArray alternatives;
      try (NDManager embeddingScope = batchManager.newSubManager()) {
        embeddingScope.tempAttachAll(owner.policyNodeIds, owner.alternativeIds);
        policyNodes =
            owner
                .policyNodeEmbedding
                .forward(parameterStore, new NDList(owner.policyNodeIds), false, runtimeParameters)
                .singletonOrThrow();
        alternatives =
            owner
                .alternativeEmbedding
                ...
                .singletonOrThrow();
        batchManager.attachAll(policyNodes, alternatives);
      }
      NDArray fixedContexts =
          policyNodes
              .concat(alternatives, 1)
              .reshape(1, DecisionAlternative.NETWORK_SIZE, owner.hiddenSize * 2L);
      return EpsilonMahjongStateEncoder.silu(
          EpsilonPartitionedLinear.apply(
              owner.alternativeHidden,
              parameterStore,
              false,
              stateContext,
              candidateContexts,
  ...
  /** 凍結推論の遷移別入力元を必要な要素数に合わせたバッファへ連結し、二つのGEMMで射影する。 */
