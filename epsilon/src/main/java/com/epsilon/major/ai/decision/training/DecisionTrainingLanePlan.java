...
/**
 * 一回のパラメーター更新に必要な小バッチを、推定処理量に応じて学習ワーカーへ分配する。
 *
 * <p>この型が所有するのは学習ワーカーごとの外側一覧と配分順だけであり、小バッチと記述情報は複製しない。返した一覧と配列は実行計画を実行し終えるまで読み取り専用で扱う。
 */
final class DecisionTrainingLanePlan {
   ...
   *
   * <p>{@code batches}と記述情報は並べ替えもコピーもしない。同処理量時の探索開始学習ワーカーは更新段階ごとに回転し、短い末尾を学習ワーカー 0へ固定しない。
   */
  ...
  /** 記述情報を必要としない純粋LPT。戻り値は元処理量インデックスごとの学習ワーカー番号である。 */
