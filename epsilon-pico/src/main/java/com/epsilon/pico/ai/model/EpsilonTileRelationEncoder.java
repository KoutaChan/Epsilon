 ...
 * フィードフォワード残差接続で構成し、最終ブロックの後の層正規化で候補スコア計算処理へ渡す牌トークンの尺度を固定する。
 */
public final class EpsilonTileRelationEncoder extends AbstractBlock {

  /** 積み重ねる牌種間の関係を使う注意機構ブロック数。 */
  ...
  /** 関係集合とエンコーダー構成を要約したチェックポイントの互換性識別子。 */
  ...
  /**
   * Fusion 実行へ渡す重みを固定したパラメーター参照を組み立てる。
   *
   ...
   *
   * <p>通常順伝播の定数ホストからデバイスへの転送を除き、HIP グラフ記録中にもpageable ホストメモリを参照しない。
   *
  ...
  private NDArray encodeInference(
    ...
    // 通常登録項目では共有入力を守り、owned 登録項目だけが呼び出し元から譲られたバッファを再利用する。
